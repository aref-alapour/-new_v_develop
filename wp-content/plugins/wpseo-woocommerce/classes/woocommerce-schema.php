...
class WPSEO_WooCommerce_Schema {
	...
	private function add_unit_price_specifications( &$data, $key, $price_specifications, $product ) {
		...
		foreach ( $price_specifications as &$price_specification ) {
			...
			if ( isset( $price_specification['price'] ) ) {
            $price_specification['price'] = wpseo_change_price_value( $currency, $price_specification['price'] );
        }
	...
	private function add_price_specifications( &$data, $key, $price ) {
		...
		$data['offers'][ $key ]['priceSpecification']['price'] = wpseo_change_price_value( $currency, $price );
...
add_filter( 'woocommerce_structured_data_product_offer', 'wpseo_fix_prices_in_structured_data', 10, 1 );

function wpseo_fix_prices_in_structured_data( $markup_offer ) {
    $currency = get_woocommerce_currency();
    $convert_currencies = array( 'IRT', 'IRHR', 'IRHT' );

    // Use a queue to traverse the array iteratively (avoids recursion issues)
    $queue = array( &$markup_offer );
    ...
    while ( ! empty( $queue ) ) {
        ...
        foreach ( $current as $key => &$value ) {
            if ( is_array( $value ) ) {
                // Add nested arrays to the queue for processing
                array_push( $queue, $value );
            } else {
                // Convert currency code to IRR
                if ( $key === 'priceCurrency' && in_array( $currency, $convert_currencies ) ) {
                    $value = 'IRR';
                }
                // Apply price conversion multipliers
                if ( in_array( $key, array( 'price', 'lowPrice', 'highPrice' ) ) ) {
                    $value = wpseo_change_price_value( $currency, $value );
                }
            }
        }
    }
...
function wpseo_change_price_value( $currency, $value ) {
    switch ( $currency ) {
        case 'IRT':
            $value *= 10; // 1 IRT = 10 IRR
            break;
        case 'IRHR':
            $value *= 1000; // 1 IRHR = 1000 IRR
            break;
        case 'IRHT':
            $value *= 10000; // 1 IRHT = 10,000 IRR
            break;
    }
    return $value;
}