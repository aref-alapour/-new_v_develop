=== zibal Gateway ===
author: pegah sarafrazi
author URI: http://www.zibal.ir/
Donate link: https://zibal.ir
Requires at least: 4.5
Requires PHP: 7.0.0
Tested up to: 6.4.2
Stable tag: 4.9

پرداخت اینترنتی وجه به وسیله درگاه پرداخت واسطه زیبال

== Description ==
** zibal Gateway** این افزونه شما را قادر می سازد تا براحتی اقدام به ایجاد درگاه پرداخت اینترنتی زیبال برای پرداخت های محصولات ووکامرس کنید

= امکانات =
 * اضافه شدن خودکار واحد های پولی ریال تومن هزار ریال و هزار تومان به ووکامرس
 * سازگار با ووکامرس 2.6 و قدیمی تر
 * پنل تنظیمات ساده و کاربرپسند
 * تنظیم پیام دلخواه در هنگام پرداخت موفقیت آمیز ، انصراف از پرداخت و یا لغو پرداخت
 * قابلیت نمایش کد رهگیری زیبال همراه با کد میانبر
 * نمایش خطاهای درگاه پرداخت




== Installation ==
1. پوشه `zibal-woocommerce` را در مسیر `/wp-content/plugins/` آپلود کنید
2. افزونه را از طریق منوی 'افزونه ها' در وردپرس فعال کنید
3. تنظیمات افزونه را می توانید از طریق قسمت تنظیمات ووکامرس / تسویه حساب انجام دهید

 == Changelog ==

= 4.3 =
* رفع مشکل ریدارکت پس از پرداخت
= 4.3.2 =
* رفع مشکل باز نشدن قسمت تنظیمات در ووکامرس جدید
= 4.3.5 =
* افزونه به صورت پیشفرض روی درگاه مستقیم است
* نمایش کد خطا 
* کم شدن یک مرحله از پرداخت جهت سریعتر شدن فرایند
= 4.4 =
 * اضافه شدن خودکار واحد های پولی ریال تومن هزار ریال و هزار تومان به ووکامرس
 = 4.5 =
 * افزودن تنظیم دستی درگاه برای انتقال به وب گیت و زرین گیت
 * تغییر نوع وب سرویس از soap به rest
 * رفع باگ تایید نشدن سفارش پس از پرداخت موفق در برخی حالات
  = 4.6 =
  بهینه سازی کد ها و سازگار سازی با نسخه های جدید ووکامرس

  = 4.6.1 =
رفع خطای -۱ در برخی شرایط


= 4.7 =
بروزرسانی به وب سرویس جدید زیبال
درج شماره همراه و ایمیل در درگاه بانکی

= 4.7.1 = 
خطاها


= 4.7.2 =

بروزرسانی واحد

= 4.7.3 =

هماهنگ سازی بخش واحد ارز با واحد ارز در زیبال
ارتقا به وب سرویس جدید زیبال

= 4.8.1 =

بروزرسانی به نسخه جدید و رفع خطاها

= 4.9 =
تغییرات در بخش تنظیمات درگاه
بهبود عملکرد در صورت خطای اتصال به سرور زیبال
سازگاری با نسخه جدید وکامرس و وردپرس


= 4.9.1 =

هماهنگ سازی ارسال شماره همراه و ایمیل خریدار
هماهنگ سازی ارسال شماره سفارش order_id


= 4.9.2 =

رفع مشکل ارسال شماره همراه با پیشوند های +98 و سایر و نواقص آن ها
افزوده شدن لوگو جدید زیبال

= 4.9.3 =
بروزرسانی در ارسال شماره همراه و سازگاری با سایر پلاگین های دریافت شماره همراه
