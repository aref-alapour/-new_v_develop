...
if ( ! defined( 'WC_ZPAL_TEXT_DOMAIN' ) ) {
	define( 'WC_ZPAL_TEXT_DOMAIN', 'wc-zpal' );
}

add_action('plugins_loaded', 'load_zarinpal_textdomain_co');
function load_zarinpal_textdomain_co() {
    load_plugin_textdomain(WC_ZPAL_TEXT_DOMAIN, false, dirname(plugin_basename(__FILE__)) . '/languages/');
}

function Load_ZarinPal_Gateway_co() {
    if (!function_exists('Woocommerce_Add_ZarinPal_Gateway_co') && class_exists('WC_Payment_Gateway') && !class_exists('WC_ZPal_co')) {
        add_filter('woocommerce_payment_gateways', 'Woocommerce_Add_ZarinPal_Gateway_co');
        function Woocommerce_Add_ZarinPal_Gateway_co($methods) {
            $methods[] = 'WC_ZPal_co';
            return $methods;
        }
        add_filter('woocommerce_currencies', 'add_IR_currency_co');
        function add_IR_currency_co($currencies) {
        ...
        add_filter('woocommerce_currency_symbol', 'add_IR_currency_symbol_co', 10, 2);
        function add_IR_currency_symbol_co($currency_symbol, $currency) {
            switch ($currency) {
                case 'IRR':
        ...
        class WC_ZPal_co extends WC_Payment_Gateway {
            public $merchantCode;
            public $sandbox;
            ...
            public function __construct() {
                $this->id = 'WC_ZPal_co';
                $this->method_title = __('پرداخت امن زرین‌پال (co)', WC_ZPAL_TEXT_DOMAIN);
                ...
                $this->accessToken = $this->sanitize_access_token_co($this->get_option('access_token'));
                ...
                $this->auto_detect_fee_payer_co();
                add_action('woocommerce_update_options_payment_gateways_' . $this->id, array($this, 'process_admin_options'));
                add_action('woocommerce_receipt_' . $this->id, array($this, 'Send_to_ZarinPal_Gateway_co'));
                add_action('woocommerce_api_' . strtolower(get_class($this)), array($this, 'Return_from_ZarinPal_Gateway_co'));
                add_action('woocommerce_email_after_order_table', array($this, 'email_instructions_co'), 10, 3);
                add_action('woocommerce_thankyou_' . $this->id, array($this, 'thankyou_page_co'));
                add_action('woocommerce_order_status_refunded', array($this, 'process_refund_co'), 10, 2);
                if (is_admin() && $this->sandbox) {
                    add_action('admin_bar_menu', array($this, 'add_sandbox_notice_to_admin_bar_co'), 100);
                }
                add_action('admin_notices', array($this, 'admin_notice_missing_merchantcode_co'));
                add_action('admin_notices', array($this, 'admin_notice_missing_accesstoken_co'));
                
                add_action('woocommerce_cart_calculate_fees', array($this, 'add_zarinpal_fee_to_cart_co'));
                add_action('woocommerce_checkout_update_order_meta', array($this, 'save_fee_to_order_co'));
                add_action('woocommerce_checkout_update_order_review', array($this, 'update_checkout_fees_co'));
                add_action('woocommerce_store_api_register_endpoint_data', array($this, 'register_store_api_data_co'));
                add_action('woocommerce_store_api_checkout_update_order_from_request', array($this, 'blocks_add_fee_co'), 10, 2);
                add_action('woocommerce_checkout_create_order', array($this, 'checkout_create_order_fee_co'), 10, 2);
                add_action('woocommerce_blocks_checkout_order_processed', array($this, 'blocks_order_processed_co'), 10, 1);
                add_action('woocommerce_store_api_cart_update_customer', array($this, 'blocks_payment_method_changed_co'), 10, 1);
                add_filter('woocommerce_get_price_decimals', array($this, 'adjust_decimals_for_zarinpal_fee_co'), 10, 1);
                
                add_action('wp_head', array($this, 'add_cart_css_co'));

            }
            public function init_form_fields_co() {
            ...
            public function admin_options_co() {
            ...
            public function tokenization_script_co() {
                if (!$this->supports('tokenization')) {
                    return;
                }
                wp_enqueue_script('wc-credit-card-form');
            }
            public function save_payment_method_checkbox_co() {
                ?>
                <p class="form-row">
            ...
            public function Send_to_ZarinPal_Gateway_co($order_id) {
                $order = wc_get_order($order_id);
                ...
                $callback_url = add_query_arg('wc_order', $order_id, WC()->api_request_url('WC_ZPal_co'));
            ...
            public function Return_from_ZarinPal_Gateway_co() {
                global $woocommerce;
            ...
            public function process_refund_co($order_id, $amount = null, $reason = 'CUSTOMER_REQUEST') {
                $order = wc_get_order($order_id);
                if (!$amount) {
            ...
            public function get_transaction_url_co($order) {
            ...
            public function email_instructions_co($order, $sent_to_admin, $plain_text = false) {
            ...
            public function thankyou_page_co() {
                if ($this->instructions) {
                    echo wpautop(wptexturize($this->instructions));
                }
            }
            public function add_sandbox_notice_to_admin_bar_co($wp_admin_bar) {
            ...
            public function generate_trust_logo_html_co($key, $data) {
            ...
            public function admin_notice_missing_merchantcode_co() {
            ...
            public function admin_notice_missing_accesstoken_co() {
                $accesstoken = $this->get_option('access_token');
                if (empty($accesstoken) && 'yes' === $this->get_option('enabled')) {
                }
            }
            private function sanitize_access_token_co($token) {
            ...
            public function add_zarinpal_fee_to_cart_co($cart) {
                ...
                if ($this->feePayer === 'customer') {
                    ...
                    if ($is_zarinpal_selected) {
                        ...
                        $this->remove_zarinpal_fees_co($cart);
                    }
                } else {
            ...
            public function register_store_api_data_co() {
                if (function_exists('woocommerce_store_api_register_endpoint_data')) {
                    woocommerce_store_api_register_endpoint_data(array(
                        'endpoint' => \Automattic\WooCommerce\StoreApi\Schemas\V1\CheckoutSchema::IDENTIFIER,
                        'namespace' => 'zarinpal',
                        'data_callback' => array($this, 'store_api_data_callback_co'),
                        'schema_callback' => array($this, 'store_api_schema_callback_co'),
                    ));
                }
            }
            
            public function store_api_data_callback_co() {
                return array(
            ...
            public function store_api_schema_callback_co() {
                return array(
                    'fee_payer' => array(
            ...
            public function blocks_add_fee_co($order, $request) {
            ...
            public function save_fee_to_order_co($order_id) {
                $order = wc_get_order($order_id);
                if ($order && $order->get_payment_method() === $this->id && $this->feePayer === 'customer') {
                }
            }
            
            public function checkout_create_order_fee_co($order, $data) {
            ...
            public function blocks_order_processed_co($order) {
                if ($order->get_payment_method() === $this->id && $this->feePayer === 'customer') {
                    $fees = $order->get_fees();
                }
            }
            
            public function blocks_payment_method_changed_co($customer) {
            ...
            public function remove_zarinpal_fees_co($cart) {
                $fees = $cart->get_fees();
                $fee_removed = false;
            ...
            public function update_checkout_fees_co() {
                if ($this->feePayer === 'customer') {
                    $chosen_payment_method = WC()->session->get('chosen_payment_method');
                    
                    if ($chosen_payment_method !== $this->id && WC()->cart) {
                        $this->remove_zarinpal_fees_co(WC()->cart);
                    }
            ...
            public function add_fee_notice_co() {
                $chosen_payment_method = WC()->session->get('chosen_payment_method');
                if ($chosen_payment_method === $this->id && $this->feePayer === 'customer') {
                    ...
                    echo '</div>';
                        ...
                        public function enqueue_zarinpal_scripts_co() {
                if (is_checkout() || is_cart()) {
                    wp_enqueue_script('jquery');
                }
            }
            
            public function add_cart_css_co() {
                return;
            }
            

            public function adjust_decimals_for_zarinpal_fee_co($decimals) {
            ...
            private function auto_detect_fee_payer_co() {
...
add_action('plugins_loaded', 'Load_ZarinPal_Gateway_co', 11);

add_action('wp_ajax_get_zarinpal_fee', 'zarinpal_ajax_get_fee_co');
add_action('wp_ajax_nopriv_get_zarinpal_fee', 'zarinpal_ajax_get_fee_co');

add_action('wp_ajax_zarinpal_update_payment_method', 'zarinpal_update_payment_method_co');
add_action('wp_ajax_nopriv_zarinpal_update_payment_method', 'zarinpal_update_payment_method_co');

function zarinpal_update_payment_method_co() {
...
add_action('upgrader_process_complete', 'zarinpal_plugin_updated_co', 10, 2);
function zarinpal_plugin_updated_co($upgrader_object, $options) {
...
add_action('wp_ajax_zpal_transaction_info', 'zpal_display_transaction_info_co');
add_action('wp_ajax_nopriv_zpal_transaction_info', 'zpal_display_transaction_info_co');
function zpal_display_transaction_info_co() {
    function gregorian_to_jalali_co($g_y, $g_m, $g_d) {
    ...
    function format_jalali_date_co($date_str) {
        if (empty($date_str)) {
            return '-';
        }
        ...
        list($j_y, $j_m, $j_d) = gregorian_to_jalali_co($g_y, $g_m, $g_d);
    ...
    try {
        ...
        if ($transactions) {
            ...
            function render_row_co($title, $value) {
                echo '<tr>';
                echo '<th>' . $title . '</th>';
                echo '<td>' . $value . '</td>';
                echo '</tr>';
            }
            render_row_co(__('شناسه پیگیری', WC_ZPAL_TEXT_DOMAIN), esc_html($transaction_info['id']));
            render_row_co(__('کد مرجع', WC_ZPAL_TEXT_DOMAIN), esc_html($transaction_info['reference_id'] ?? '-'));
            render_row_co(__('کد آتوریتی', WC_ZPAL_TEXT_DOMAIN), esc_html($transaction_info['authority'] ?? '-'));
            render_row_co(__('وضعیت تراکنش', WC_ZPAL_TEXT_DOMAIN), esc_html($transaction_info['status']));
            render_row_co(__('مبلغ', WC_ZPAL_TEXT_DOMAIN), esc_html(number_format($transaction_info['amount']) . ' ریال'));
            render_row_co(__('کارمزد', WC_ZPAL_TEXT_DOMAIN), esc_html(number_format($transaction_info['fee']) . ' ریال'));
            render_row_co(__('توضیحات', WC_ZPAL_TEXT_DOMAIN), esc_html($transaction_info['description']));
            $shamsi_date_created = !empty($transaction_info['created_at']) ? format_jalali_date_co($transaction_info['created_at']) : '-';
            render_row_co(__('تاریخ ایجاد', WC_ZPAL_TEXT_DOMAIN), esc_html($shamsi_date_created));
            $shamsi_date_reconciled = !empty($transaction_info['reconciled_at']) ? format_jalali_date_co($transaction_info['reconciled_at']) : '-';
            render_row_co(__('تاریخ تسویه', WC_ZPAL_TEXT_DOMAIN), esc_html($shamsi_date_reconciled));
            ...
            render_row_co(__('شماره کارت', WC_ZPAL_TEXT_DOMAIN), esc_html($card_pan));
            render_row_co(__('RRN', WC_ZPAL_TEXT_DOMAIN), esc_html($rrn));
            render_row_co(__('آیپی پرداخت کننده', WC_ZPAL_TEXT_DOMAIN), esc_html($payer_ip));
            echo '</table>';
            echo '</div>';
        } else {
...
add_action('wp_ajax_zpal_manual_verify', 'zpal_manual_verify_transaction_co');
function zpal_manual_verify_transaction_co() {
...
add_action('woocommerce_admin_order_data_after_order_details', 'zpal_manual_verify_button_co');
function zpal_manual_verify_button_co($order) {
    if ($order->get_payment_method() !== 'WC_ZPal_co') {
        return;
    }
    $order_id = $order->get_id();
    ?>
