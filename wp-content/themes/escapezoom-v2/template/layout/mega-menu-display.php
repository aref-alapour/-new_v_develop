...
add_action('wp_enqueue_scripts', function () {
    // Check if any menu location has items
    $locations = get_mega_menu_locations();
    $has_menu = false;

    foreach ($locations as $location_key => $location_name) {
        $menu_items = get_option('ez_mega_menu_' . $location_key, []);
        if (!empty($menu_items)) {
            $has_menu = true;
            break;
        }
    ...
    if ($has_menu) {
        // استفاده از get_asset_version برای بروزرسانی خودکار
        wp_enqueue_style('mega-menu-style', Theme_URL . 'assets/css/mega-menu-frontend.css', [], get_asset_version('assets/css/mega-menu-frontend.css'));
        wp_enqueue_script('mega-menu-script', Theme_URL . 'assets/js/mega-menu-frontend.js', ['jquery'], get_asset_version('assets/js/mega-menu-frontend.js'), true);
    }
});
