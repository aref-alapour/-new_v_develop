...
/* ============================================================================
 * عملیات جدید (۱): دریافت اطلاعات سفارش برای فرم ویرایش مودال مالی
 *  - پرداخت کامل: completed-paid + order_payment_type = complete
 *  - پیش‌پرداخت: partially-paid + order_payment_type = partial
 * ============================================================================ */
elseif ($operation === 'get_order_for_edit') {
    ...
    $is_full_payment_edit   = ($status_norm === 'completed-paid' && $payment_type === 'complete');
    $is_partial_payment_edit = ($status_norm === 'partially-paid' && $payment_type === 'partial');

    if (!$is_full_payment_edit && !$is_partial_payment_edit) {
        wp_send_json_error('این سفارش در وضعیت قابل ویرایش (مودال مالی) نیست؛ فقط پرداخت کامل یا پیش‌پرداخت مجاز است.');
    }
    ...
    wp_send_json_success([
        ...
        'edit_mode'       => $is_full_payment_edit ? 'full' : 'partial',
    ]);
}

/* ============================================================================
 * عملیات جدید (۲): ذخیره‌سازی ویرایش سفارش (مودال مالی)
 *  - پرداخت کامل یا پیش‌پرداخت (همان دو حالت بالا)
 *  - پیش‌پرداخت: تغییر وضعیت با این فرم فقط به پرداخت کامل یا ماندن روی پیش‌پرداخت
 *  - تعداد نفرات، تیکت بیعانه، مبلغ (order_paid / متاها)، نوع پرداخت
 * ============================================================================ */
elseif ($operation === 'edit_order') {
    ...
    $existing_pt = isset($existing['order_payment_type']) ? (string) $existing['order_payment_type'] : '';

    $is_full_payment_edit = ($current_status === 'completed-paid' && $existing_pt === 'complete');
    $is_partial_payment_edit = ($current_status === 'partially-paid' && $existing_pt === 'partial');

    if (!$is_full_payment_edit && !$is_partial_payment_edit) {
        wp_send_json_error('سفارش در وضعیت مجاز برای این ویرایش نیست؛ فقط پرداخت کامل یا پیش‌پرداخت مجاز است.');
    }
    ...
    try {
        ...
        if ($new_payment_type !== null) {
            ...
            if ($is_partial_payment_edit && $new_payment_type === 'partial' && $order && $order->has_status('completed-paid')) {
                wp_send_json_error('سفارش در ووکامرس «پرداخت کامل» است؛ از این فرم فقط ارتقا به پرداخت کامل یا ویرایش مبلغ/تعداد هم‌راستا با پیش‌پرداخت مجاز است. وضعیت را با پشتیبانی هماهنگ کنید.');
            }
...
elseif ($operation === 'confirm_verified_payment') {

    $current_user = wp_get_current_user();
    if (!array_intersect(['administrator', 'accounting', 'poshtiban'], (array) $current_user->roles)) {
        wp_send_json_error('شما اجازه‌ی این عملیات را ندارید.');
    }

    $mrow = $medoo->get('wp_markting', '*', ['order_id' => $order_id]);
    ...
    $actor_id = (int) $current_user->ID;
    $uname    = sanitize_text_field($current_user->user_login);

    $lock_key = '_ez_confirm_verified_pay_lock';
    $lock_ts  = (int) get_post_meta($order_id, $lock_key, true);
    if ($lock_ts && (time() - $lock_ts) < 45) {
        wp_send_json_error('این عملیات همین حالا در حال انجام است؛ چند ثانیه بعد دوباره تلاش کنید.');
    }
    update_post_meta($order_id, $lock_key, time());

    try {
        if (!function_exists('ez_team_finalize_order_sans_run')) {
            wp_send_json_error('تابع بستن سانس تیم در دسترس نیست.');
        }

        $run = ez_team_finalize_order_sans_run($order_id, $actor_id, 'confirm_payment');

        $order = wc_get_order($order_id);
        if ($order) {
            $order->add_order_note(
                sprintf(
                    'تأیید پرداخت / بستن سانس توسط همکار %s — نتیجه: %s',
                    $uname,
                    $run['message'] ?? '—'
                )
            );
        ...
        error_log('[confirm_verified_payment] order=' . $order_id . ' ' . $e->getMessage());
        wp_send_json_error('خطا هنگام بستن سانس؛ لاگ سرور را ببینید.');
    } finally {
        delete_post_meta($order_id, $lock_key);
    }
    ...
    $current_user = wp_get_current_user();
    if (!array_intersect(['administrator', 'poshtiban'], (array) $current_user->roles)) {
        wp_send_json_error('شما اجازه‌ی این عملیات را ندارید.');
    }
    ...
    if (!function_exists('ez_markting_row_can_run_booking_recovery') || !ez_markting_row_can_run_booking_recovery($mrow)) {
        $reason = function_exists('ez_markting_team_ops_ineligible_reason')
            ? ez_markting_team_ops_ineligible_reason($mrow, 'booking_recovery')
            : 'بررسی سانس فقط برای processing، پیش‌پرداخت یا پرداخت کامل مجاز است.';
        wp_send_json_error($reason);
    }

    if (!function_exists('ez_team_finalize_order_sans_run')) {
        wp_send_json_error('تابع بستن سانس در محیط بارگذاری نشده است.');
    }

    $run = ez_team_finalize_order_sans_run($order_id, $actor_id, 'recover_booking');
