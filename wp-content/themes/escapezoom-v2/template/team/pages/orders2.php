...
<div class="flex justify-between items-center gap-4">
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="relative">
            ...
            <div id="problematicSessionsMenu" class="dropdown-menu hidden absolute top-full left-0 mt-2 w-48 bg-white border border-[#E4EBF0] rounded-xl shadow-lg z-10">
                <div class="py-2">
                    ...
                    <a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" data-value="problematic">سفارشات بد</a>
                </div>
            </div>
        </div>
        ...
        <div class="relative flex items-center gap-2 shrink-0">
            <input type="text"
                id="couponCodeFilter"
                class="w-36 lg:w-44 h-12 px-3 border border-[#E4EBF0] rounded-xl text-sm font-yekan-bold text-navyBlue bg-[#FAFDFF] outline-none focus:border-[#3F7FF5] placeholder:text-gray-400"
                placeholder="کد تخفیف"
                autocomplete="off"
                dir="ltr" />
            <button type="button" id="couponCodeFilterApply" class="shrink-0 h-12 px-3 rounded-xl text-sm font-yekan-bold bg-white border border-[#E4EBF0] text-navyBlue hover:bg-gray-50 transition-colors">
                اعمال
            </button>
        </div>
...
<input id="current_user_roles" type="hidden" value="<?php echo esc_attr( wp_json_encode( array_values( (array) wp_get_current_user()->roles ) ) ); ?>">
...
<script>
    jQuery(document).ready(function($) {
        ...
        $('#couponCodeFilterApply').on('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            currentFilters.couponCode = $('#couponCodeFilter').val().trim();
            applyFilters();
        });
        ...
        $('#couponCodeFilter').on('keypress', function(e) {
            if (e.which === 13) {
                e.preventDefault();
                currentFilters.couponCode = $(this).val().trim();
                applyFilters();
            }
        });
        ...
        function fetch_orders_data(showMessages = false, page = 1, term = '') {
            ...
            $.ajax({
                type: 'POST',
                url: "<?php echo esc_url( get_template_directory_uri() ); ?>/template/team/ajax/callbacks/orders_get2.php",
                data: {
                    'state': 'all',
                    'page': page,
        ...
        $('body').on('click', '.confirm-verified-payment-btn', function() {
            ...
            Swal.fire({
                title: 'پرداخت را تأیید می‌کنید؟',
                html: 'مثل تأیید درگاه عمل می‌شود (بستن سانس یا تداخل و عودت کیف در صورت نیاز؛ پیامک‌ها طبق وضعیت نهایی).',
                icon: 'question',
                showCancelButton: true,
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        nonce: "<?php echo wp_create_nonce('team-ajax-nonce') ?>",
                        callback: 'orders_actions',
                        operation: 'confirm_verified_payment',
                        order_id: order_id,
                    },
                    dataType: 'json',
                    success: function(data) {
                        if (data && data.success) {
                            ...
                            Swal.fire({
                                title: 'خطا',
                                text: (data && data.data) ? data.data : 'عملیات ناموفق بود.',
                                icon: 'error',
                                confirmButtonColor: '#6B7280',
                                customClass: { confirmButton: 'swal2-confirm-custom' }
                            });
                            $button.prop('disabled', false).css('opacity', '1').css('cursor', 'pointer').html(originalHtml);
                        }
                    },
                    error: function() {
                        Swal.fire({
        ...
        // ============================================================================
        // بررسی سانس (بازیابی بوکینگ / تداخل / همگام‌سازی مارکتینگ)
        // ============================================================================
        $('body').on('click', '.recover-booking-sans-btn', function() {
            let $button = $(this);
            let originalHtml = $button.html();
            let order_id = $button.data('order-id');
            if (!order_id) return;
            ...
            Swal.fire({
                title: 'بررسی سانس را اجرا کنم؟',
                html: 'سیستم وضعیت <code class="text-xs">wp_zb_booking_history</code> و اسلات سانس را بررسی می‌کند؛ در صورت تداخل مبلغ به کیف پول مشتری برمی‌گردد؛ در غیر این صورت رزرو/مارکتینگ اصلاح می‌شود.',
                icon: 'question',
                showCancelButton: true,
        ...
        $('body').on("click", "#openEditOrderForm", function() {
            ...
            if (!$box.hasClass('hidden')) {
                ...
                $('#editOrderPaymentHint').addClass('hidden').text('');
                return;
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (info.edit_mode === 'partial') {
                        $('#editOrderPaymentHint').removeClass('hidden').text(
                            'برای ارتقای سفارش به «پرداخت کامل»، گزینه «پرداخت کامل» را انتخاب کنید؛ در غیر این صورت وضعیت همان پیش‌پرداخت می‌ماند.'
                        );
                    } else {
                        $('#editOrderPaymentHint').addClass('hidden').text('');
                    }
        ...
        function ezResetDirectCancellationRefundForm() {
            $('input[name="direct_cancel_side"][value="customer"]').prop('checked', true);
            $('input[name="direct_cancel_reason"]').prop('checked', false);
            $('#directCancelOwnerReasons').addClass('hidden');
        }

        function ezDirectCancelAllowedStatuses() {
            return ['partially-paid', 'wc-partially-paid', 'completed-paid', 'wc-completed-paid', 'walletx', 'wc-walletx', 'processing', 'wc-processing', 'completed', 'wc-completed'];
        }

        $(document).on('change', 'input[name="direct_cancel_side"]', function() {
            if ($(this).val() === 'owner') {
                $('#directCancelOwnerReasons').removeClass('hidden');
            } else {
                $('#directCancelOwnerReasons').addClass('hidden');
                $('input[name="direct_cancel_reason"]').prop('checked', false);
            }
        ...
        $(document).on('click', '#btnMasterRefund', function() {
            const $btn = $(this);
            const order_id = $('#maliModal').attr('data-id');
            if (!order_id) {
                return;
            }
            ...
            Swal.fire({
                title: 'مسترد سفارش؟',
                html: 'سانس (در صورت بسته بودن) باز می‌شود، مبلغ به کیف پول پلیر برمی‌گردد' +
                    ( $('#maliModal').data('order-status') === 'walletx' || $('#maliModal').data('order-status') === 'wc-walletx'
                        ? ' و واریز کیف پول مجموعه‌دار با اصلاحیه برگردانده می‌شود' : '' ) +
                    '.<br><b>پیامک ارسال نمی‌شود.</b>',
                icon: 'warning',
                iconColor: '#F21543',
                showCancelButton: true,
                confirmButtonText: 'بله، مسترد کن',
                cancelButtonText: 'انصراف',
                ...
                $btn.prop('disabled', true).html('در حال پردازش...');
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.success) {
                            Swal.fire({
                                title: 'موفق',
                                text: data.data || 'سفارش با موفقیت مسترد شد.',
                                icon: 'success'
                            });
                            $('#maliModal').hide();
                            $('#refreshButton').click();
                            ...
                            Swal.fire({
                                title: 'خطا',
                                text: (data.data && typeof data.data === 'string') ? data.data : 'عملیات ناموفق بود.',
                                icon: 'error'
                            });
        ...
        $(document).on('click', '#btnDirectCancellationRefund', function() {
            const $btn = $(this);
            const order_id = $('#maliModal').attr('data-id');
            if (!order_id) {
                return;
            ...
            const requester_type = $('input[name="direct_cancel_side"]:checked').val();
            const reason_id = $('input[name="direct_cancel_reason"]:checked').val();

            if (requester_type === 'owner' && !reason_id) {
                Swal.fire({
                    title: 'خطا',
                    text: 'لطفاً دلیل کنسلی مجموعه را انتخاب کنید.',
                    icon: 'error'
                });
                return;
            }

            const sideLabel = requester_type === 'owner' ? 'مجموعه' : 'پلیر';
            Swal.fire({
                title: 'ثبت کنسلی و استرداد؟',
                html: 'سفارش <b>مسترد</b> می‌شود؛ رضایت بر اساس کنسلی از طرف <b>' + sideLabel + '</b> ثبت می‌شود.<br>عودت کیف پول و پیامک استرداد ارسال می‌شود.',
                icon: 'warning',
                iconColor: '#F21543',
                showCancelButton: true,
                confirmButtonText: 'بله، ثبت کن',
                ...
                const payload = {
                    ...
                    nonce: "<?php echo wp_create_nonce('team-ajax-nonce') ?>",
                    callback: 'cancellation_actions',
                    function: 'direct_cancellation_refund',
                    order_id: order_id,
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.success) {
                            Swal.fire({
                                title: 'موفق',
                                text: data.data || 'کنسلی و استرداد با موفقیت ثبت شد.',
                                icon: 'success'
                            });
                            $('#maliModal').hide();
                            $('#refreshButton').click();
        ...
        $(document).on("click", ".openMaliModal", function(e) {
            e.preventDefault();
            e.stopPropagation();
            ...
            if (!$mali.length) {
                console.error('maliModal not in DOM — احتمالاً خطای PHP در orders_get2.php هنگام لود لیست.');
                Swal.fire({
                    title: 'خطا',
                    text: 'مودال مالی بارگذاری نشد. دکمه بروزرسانی را بزنید؛ اگر ادامه داشت به پشتیبانی بگویید.',
                    icon: 'error'
                });
                return;
            }

            $mali.attr('data-id', order_id).attr('data-order-status', order_status).show();
            ezResetDirectCancellationRefundForm();

            let isCompletedPaid = (order_status === 'completed-paid' || order_status === 'wc-completed-paid');
            let isPartiallyPaid   = (order_status === 'partially-paid' || order_status === 'wc-partially-paid');
            let showConvertAmendment = isCompletedPaid && payment_type === 'complete';
            let showEditOrderForm = (isCompletedPaid && payment_type === 'complete')
                || (isPartiallyPaid && payment_type === 'partial');

            if (showConvertAmendment) {
                $mali.find("#convert_with_amendment").show();
            } else {
                $mali.find("#convert_with_amendment").hide();
            }
            ...
            if (order_status === 'trash' || order_status === 'wc-trash') {
                $mali.find("button[data-action='trash']").hide();
            } else {
                $mali.find("button[data-action='trash']").show();
            }

            // اگر سفارش در وضعیت walletx است، دکمه کیف پول را مخفی کن
            if (order_status === 'walletx' || order_status === 'wc-walletx') {
                $mali.find("button[data-action='walletx']").hide();
            } else {
                $mali.find("button[data-action='walletx']").show();
            }

            const stNorm = order_status.replace(/^wc-/, '');
            const hideRefundActions = order_status === 'refunded' || order_status === 'wc-refunded'
                || order_status === 'trash' || order_status === 'wc-trash'
                || stNorm === 'cancelled' || stNorm === 'conflict' || stNorm === 'failed'
                || ezDirectCancelAllowedStatuses().indexOf(order_status) === -1;

            // اگر سفارش در وضعیت refunded است، دکمه مسترد را مخفی کن
            if (hideRefundActions) {
                $mali.find("button[data-action='refunded']").hide();
                $('#btnMasterRefund').hide();
            } else {
                $mali.find("button[data-action='refunded']").show();
                $('#btnMasterRefund').show().prop('disabled', false).text('مسترد');
            }
            ...
            if (order_status === 'admin-cancelled' || order_status === 'wc-admin-cancelled') {
                $mali.find("button[data-action='admin-cancelled']").hide();
            } else {
                $mali.find("button[data-action='admin-cancelled']").show();
            }

            const $directBox = $('#directCancellationRefundBox');
            if ($directBox.length) {
                if (hideRefundActions) {
                    $directBox.hide();
                } else {
                    $directBox.show();
                    $('#btnDirectCancellationRefund').prop('disabled', false).text('ثبت کنسلی و استرداد');
                }
            }
        });
