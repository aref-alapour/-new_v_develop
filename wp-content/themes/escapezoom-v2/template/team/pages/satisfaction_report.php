...
<div class="space-y-6">
	...
	<form id="satisfaction-report-form" class="grid grid-cols-1 lg:grid-cols-16 gap-4 bg-slate-50 border border-slate-200 rounded-xl p-4">
		...
		<div class="lg:col-span-4">
			...
			<div class="relative">
				<input id="gameSearch" type="text" autocomplete="off" class="w-full h-11 border border-[#E4EBF0] bg-white rounded-xl outline-none px-4 text-xs font-bold text-navyBlue" placeholder="نام بازی را جستجو کنید">
				<div id="lg-search-result-list" class="hidden absolute z-50 mt-1 w-full max-h-72 overflow-y-auto divide-y divide-[#E4EBF0] bg-white border border-slate-200 rounded-xl shadow-sm"></div>
			</div>
		...
		<div class="lg:col-span-4">
			...
			<div class="date-range-container relative">
				<div class="date-range-trigger h-11 px-3 bg-white border border-[#E4EBF0] rounded-xl cursor-pointer flex items-center justify-between" id="date-range-trigger">
		...
		<div class="lg:col-span-4">
			...
			<div class="date-range-container relative">
				<div class="date-range-trigger h-11 px-3 bg-white border border-[#E4EBF0] rounded-xl cursor-pointer flex items-center justify-between" id="compare-date-range-trigger">
		...
		<div class="lg:col-span-4 flex items-center justify-center gap-2">
			<button type="submit" class="h-11 px-5 rounded-lg bg-[#155DFC] text-white hover:bg-[#1447E6] transition-colors">نمایش گزارش</button>
	...
	<div id="satisfaction-detail-modal-overlay" class="fixed inset-0 z-50 hidden bg-black/40 backdrop-blur-sm">
		<div class="w-full h-full flex items-center justify-center p-4">
			<div id="satisfaction-detail-modal" class="bg-white rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-hidden">
	...
	if (!rows.length) {
		...
		rows.forEach((row) => {
			...
			tableRows += `<tr class="border-b border-slate-100 text-sm">
				...
				<td class="px-4 py-3 max-w-[380px] whitespace-pre-wrap break-words text-slate-600">
...
async function searchGames(term) {
	...
	try {
		...
		gameSearchList.innerHTML = items.map((item) => {
			...
			const typeLabel = item.game_type ? `<p class="text-[11px] text-slate-500 mt-0.5">نوع: ${escHtml(item.game_type)}</p>` : '';
			const historyBadge = item.has_satisfaction_data
				? '<span class="text-[10px] px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700">دارای لاگ رضایت</span>'
				: '<span class="text-[10px] px-2 py-0.5 rounded-full bg-amber-50 text-amber-700">بدون لاگ رضایت</span>';
