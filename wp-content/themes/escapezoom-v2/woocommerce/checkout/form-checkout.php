...
$ez_book_ts     = isset($_GET['book']) ? (int) $_GET['book'] : 0;
$ez_product_id  = isset($_GET['add-to-cart']) ? (int) $_GET['add-to-cart'] : 0;
$ez_sans_state  = ( $ez_book_ts > 0 && $ez_product_id > 0 && function_exists( 'ez_sans_checkout_slot_state' ) )
    ? ez_sans_checkout_slot_state( $ez_product_id, $ez_book_ts )
    : ( ( $ez_book_ts > 0 && $ez_book_ts < time() ) ? 'past' : 'bookable' );

if ( $ez_sans_state !== 'bookable' ) {
    $ez_sans_block_msg = function_exists( 'ez_sans_checkout_slot_message' )
        ? ez_sans_checkout_slot_message( $ez_product_id, $ez_book_ts, $ez_sans_state )
        : 'زمان این سانس برای رزرو در دسترس نیست.';
    $reorder_url = home_url($_GET['add-to-cart']); ?>

    <section
        class="mt-5 rounded-xl border border-borderDefault px-6 pb-9 pt-7 shadow-[0px_2px_0px_0px_#E8EDF1] lg:p-13 bg-[#F9FAFB]">
        <div class="flex flex-col justify-center items-center">
            ...
            <div
                class="text-text-bold-s3 font-bold flex justify-center items-center border border-borderDefault sahdow-1 lg:w-[556px] w-[326px] h-[86px] rounded-xl shadow-1 mt-6">
                <?php echo esc_html( $ez_sans_block_msg ); ?></div>
            <div class="flex flex-col lg:flex-row gap-5 mt-9">
                <a href="<?php echo $reorder_url; ?>" class="bg-bgPrimary shadow-btn-org w-[326px] lg:w-[268px] h-[56px] flex justify-center items-center gap-11 text-white rounded-lg">
                    اقدام مجدد <svg xmlns="http://www.w3.org/2000/svg" width="24" height="25"
                ...
                <button
                    class="bg-white shadow-1 w-[326px] lg:w-[268px] h-[56px] flex justify-center items-center gap-11 rounded-lg">
                    تماس با پشتیبانی <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25"
        ...
        <div class="md:flex md:gap-x-4 mt-5">
            <div class="border border-border-1 rounded-xl p-5 md:py-4 flex flex-wrap items-center justify-between lg:justify-around bg-white grow font-medium text-lg shadow-13">
                <div class="w-full lg:w-auto flex max-lg:flex-col gap-x-4 mb-5 lg:mb-0">
                    <div class="text-[#7E7E7E] font-semibold">
            ...
            <div class="hidden md:border md:border-border-1 md:rounded-xl px-1 md:px-4 md:py-4 md:flex md:items-center md:bg-white shadow-13">
                <a href="<?php echo get_permalink($product->get_id()); ?>" class="flex items-center gap-x-1 lg:gap-x-4 text-[#888888] hover:text-primary-2 transition">
            ...
            <div class="md:w-1/2 lg:w-7/12 flex flex-col gap-y-6">
                ...
                <div class="border border-border-1 rounded-xl px-4 py-4 lg:py-6 bg-white shadow-13">
                    ...
                    <div class="grid lg:grid-cols-2 mt-5">
                        <div class="flex flex-col">
                            <span class="text-[#889BAD] text-sm font-light-yekanbakh font-light">نام و نام خانوادگی</span>
                            <span>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-end gap-x-2">
                                <div class="flex flex-col">
                                    <span class="text-[#889BAD] text-sm font-light-yekanbakh font-light">شماره همراه</span>
                                    <bdo dir="ltr"><?php echo $user->user_login ?></bdo>
                                </div>
                                <div class="bg-[#E1F9EE] rounded-md flex items-center justify-center gap-x-1.5 px-2 w-fit h-6 mb-2">
                                <span>
                                    ...
                                    <span class="text-xs text-[#02C96F]">تأیید شده</span>
                                </div>
                            </div>
                            <a href="<?php echo esc_url(site_url('panel/settings')); ?>" class="border border-[#E4EBF0] rounded-[10px] w-11 h-11 flex items-center justify-center">
                ...
                <div class="border border-border-1 rounded-xl px-4 py-4 lg:py-6 bg-white shadow-13">
                    <div class="mb-6">
                        ...
                        <p class="text-[#888888] font-medium text-sm mt-2">
                    ...
                    <div id="other-players" class="mt-5" data-count="<?php echo esc_attr($quantity); ?>">
                        <div class="mt-5">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                <label class="relative bg-white border border-border-1 rounded-xl h-12 shadow-13 px-2 has-[input:placeholder-shown]:!border-border-1 has-[input:valid]:border-green-500 has-[input:invalid]:border-[#FD7013]">
                    ...
                    <?php if ($quantity > 2) { ?>
                        <div class="mt-6 flex items-center gap-x-5 relative after:relative after:block after:content-[''] after:w-full after:h-px after:bg-[#DADADA]">
            ...
            <div class="border border-border-1 rounded-xl px-4 py-4 lg:py-6 bg-white md:w-1/2 lg:w-5/12 shadow-13">
                ...
                <div class="my-4 lg:my-8 relative">
                    <form id="submit-coupon" method="post" class="transition-all duration-300">
                        <div class="bg-[#FFFFFF] rounded-xl border border-[#E8EDF1] shadow-13 flex justify-between items-center px-1.5 py-2 md:p-2 gap-x-2 md:gap-x-4">
                            <div class="font-medium text-sm text-nowrap">اعمال کد تخفیف</div>
                            <div class="grow">
                                <input class="w-full focus:outline-0 bg-white border border-[#DADADA] rounded-md h-8 px-2" type="text" name="coupon-code" id="coupon-code">
                            </div>
                            <div>
                ...
                <form name="checkout" id="payment-form" method="post" class="checkout woocommerce-checkout" action="<?php echo esc_url(wc_get_checkout_url()); ?>" enctype="multipart/form-data" aria-label="<?php echo esc_attr__('Checkout', 'woocommerce'); ?>">
                    ...
                    <p class="text-sm text-[#888888] font-medium">
                        ثبت رزرو شما به معنای پذیرش
                        <a href="https://escapezoom.ir/terms" class="underline underline-offset-4 text-[#158EFF] hover-[#0084FF] transition">قوانین و
                            مقررات
                        </a>
