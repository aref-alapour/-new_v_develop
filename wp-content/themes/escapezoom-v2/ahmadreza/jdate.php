...
function jdate(
    ...
    if ($time_zone != "local") {
        date_default_timezone_set(
            $time_zone === "" ? "Asia/Tehran" : $time_zone
        );
    }
    ...
    for ($i = 0; $i < $sl; $i++) {
        ...
        switch ($sub) {
            ...
            case "k":
                $out .= tr_num(
                    100 - (int) (($doy / ($kab + 365.24)) * 1000) / 10,
                    $tr_num
                );
                break;
...
function jstrftime(
    ...
    $ts = $T_sec + (int)($timestamp === "" ? time() : tr_num($timestamp));
    ...
    for ($i = 0; $i < $sl; $i++) {
        ...
        switch ($sub) {
            ...
            case "U":
                $avs =
                    ($date[6] < 5 ? $date[6] + 2 : $date[6] - 5) - ($doy % 7);
                if ($avs < 0) {
                    $avs += 7;
                }
                $num = (int) (($doy + $avs) / 7) + 1;
            ...
            case "r":
                $out .=
                    ...
                    $date[5] .
                    " " .
                    ($date[1] < 12 ? "قبل از ظهر" : "بعد از ظهر");
                break;
...
function jmktime(
    ...
    if ($h === "") {
        ...
        list($h, $m, $s, $jm, $jd, $jy) = explode(
            "_",
            tr_num($h . "_" . $m . "_" . $s . "_" . $jm . "_" . $jd . "_" . $jy)
        );
        if ($m === "") {
            return mktime($h);
        } else {
            if ($s === "") {
                ...
                if ($jm === "") {
                    ...
                    $jdate = explode(
                        "_",
                        jdate("Y_j", "", "", $timezone, "en")
                    );
                    if ($jd === "") {
                        list($gy, $gm, $gd) = jalali_to_gregorian(
                            $jdate[0],
                            $jm,
                            $jdate[1]
                        );
...
function jgetdate(
    ...
    $jdate = explode(
        "_",
        jdate("F_G_i_j_l_n_s_w_Y_z", $ts, "", $timezone, $tn)
    );
    return [
