...
add_action('init', function () {
    ...
    if (get_option('escapezoom_my_reviews_endpoint_v1') !== '1') {
        flush_rewrite_rules(false);
        update_option('escapezoom_my_reviews_endpoint_v1', '1');
    }
});
...
add_filter('woocommerce_account_menu_items', function ($items) {
    ...
    $has_room      = function_exists( 'ez_account_user_has_managed_room' ) && ez_account_user_has_managed_room( $uid );
    $is_compiler   = function_exists( 'ez_user_has_role' ) && ez_user_has_role( $user, 'compiler' );
    $is_sans_mgr   = function_exists( 'ez_user_has_role' ) && ez_user_has_role( $user, 'sans_manager' );
    $is_customer   = function_exists( 'ez_user_has_role' ) && ez_user_has_role( $user, 'customer' );
    $is_admin_cap  = current_user_can( 'administrator' );

    $show_staff_hub = $is_admin_cap || ( ( $is_compiler || $is_sans_mgr ) && $has_room );
...
add_action('wp_enqueue_scripts', function () {
    wp_register_style('select2-css', Theme_URL . 'assets/vendor/select2/select2.min.css', [], null);
    wp_register_style('persian-datepicker', Theme_URL . 'assets/vendor/persian-datepicker/persian-datepicker.css', [], '1.2.0');
    wp_register_style('lightbox-css', Theme_URL . 'assets/vendor/lightbox/css/lightbox.css', [], '1.2.0');

    wp_enqueue_style('tippy-perspective-extreme-css', Theme_URL . 'assets/vendor/tippy/perspective-extreme.css', [], null);
    wp_enqueue_style('custom-styles-css', Theme_URL . 'assets/css/custom-styles.css', [], '1.0.2', 'all');

    wp_register_script('select2-js', Theme_URL . 'assets/vendor/select2/select2.min.js', ['jquery'], null, true);
    wp_register_script('persian-date-jquery', Theme_URL . 'assets/vendor/persian-datepicker/jquery-3.7.1.min.js', [], '3.7.1', true);
    wp_register_script('persian-date', Theme_URL . 'assets/vendor/persian-datepicker/persian-date.min.js', ['persian-date-jquery'], '1.1.0', true);
    wp_register_script('lightbox-js', Theme_URL . 'assets/vendor/lightbox/js/lightbox.min.js', [], null);
    wp_enqueue_script('qrcode-js', Theme_URL . 'assets/vendor/qrcode/qrcode.js', [], null);
    wp_enqueue_script('popper-js', Theme_URL . 'assets/vendor/tippy/popper.min.js', [], '2.11.8');
    wp_enqueue_script('tippy-js', Theme_URL . 'assets/vendor/tippy/tippy-bundle.umd.min.js', [], null);
    wp_register_script('persian-datepicker', Theme_URL . 'assets/vendor/persian-datepicker/persian-datepicker.min.js', [
        'persian-date-jquery',
        'persian-date',
    ], '1.2.0', true);

    wp_add_inline_script('persian-datepicker', 'jQuery(document).ready(function($) { 
...
add_action('wp_head', function () {
    ...
    if ($comments_count != 0)
        $final_score = (int)$final_score / (int)$comments_count / 20;
    else
        $final_score = 0;
    ...
    $final_score = $product_type !== 'اتاق فرار' ? $final_score : $final_score / 5;
