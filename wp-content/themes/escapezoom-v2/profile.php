...
// دریافت HTML کروسل از وب‌سرویس
if (!empty($latest_20_products)) {
    $recent_played_response = json_decode(ez_webservice([
        'type' => 'get_by_products_id',
        'data' => [
            'products_id' => $latest_20_products,
            'format'      => 'html_swiper'
        ]
    ]));
}
...
get_header(); ?>
    ...
    <div class="grid-cols-12 pb-4 max-md:mt-6 lg:mt-10 lg:grid lg:gap-10">
                        ...
                        <section class="relative max-w-full py-4 md:py-5 lg:py-9 max-md:px-4">
                            <div class="relative overflow-hidden embla_normal">
                                <div class="embla__viewport">
                                    <div id="trends-rooms-slider" class="embla__container first:child:before:hidden child:before:w-px child:before:absolute child:before:bg-gradient-to-t child:before:from-white child:before:via-slate-110 child:before:to-white child:before:-right-3.5 child:lg:before:-right-6 child:before:h-full min-h-[200px] child:box-content lg:min-h-[300px] flex child:ml-7 md:child:ml-12  last:child:ml-0 child:relative child:shrink-0 child:grow-0 child:w-[156px] md:child:w-[200px] child:py-2.5"> <?= $data['recent_played']; ?> </div>
                                </div>
        ...
        <div id="ez-bio" class="col-span-3 max-3xl:col-span-4 max-2xl:col-span-5">
            ...
            <?php endif; ?>
            <?php
            $viewer_roles         = is_array( $user_roles ) ? $user_roles : [];
            $can_view_owner_notes = ! empty( $owner_feedback ) && (
                in_array( 'compiler', $viewer_roles, true )
                || in_array( 'administrator', $viewer_roles, true )
                || in_array( 'poshtiban', $viewer_roles, true )
                || in_array( 'supervisor', $viewer_roles, true )
            );
            ?>
            <?php if ( $can_view_owner_notes ) : ?>
