...
function checkout_get_api($request)
{
    ...
    $ez_slot_err = function_exists( 'ez_api_sans_slot_error_message' )
        ? ez_api_sans_slot_error_message( (int) $product_id, (int) $sans_time )
        : null;
    if ( $ez_slot_err !== null ) {
    ...
    elseif ($sans_obj['status'] == 1) {
        $msg_booked = function_exists('ez_api_message_sans_already_booked_confirmed')
            ? ez_api_message_sans_already_booked_confirmed((array) $sans_obj, (int) $user_id, '')
...
function checkout_place_order_api2($request)
{
    ...
    elseif ($sans_obj['status'] == 1) {
        $msg_booked = function_exists('ez_api_message_sans_already_booked_confirmed')
            ? ez_api_message_sans_already_booked_confirmed((array) $sans_obj, $wc_customer_id, $phone_digits)
            : 'سانس توسط شخص دیگری رزرو شده است.';
        wp_send_json_error($msg_booked);
    }
