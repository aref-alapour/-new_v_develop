...
function ez_compute_review_rate_average( array $rate, bool $missing_1097 = false ): float {
	$rate_1098 = $rate[1098] ?? 0;
	$rate_1097 = $rate[1097] ?? 0;
	$rate_1096 = $rate[1096] ?? 0;
	$rate_1095 = $rate[1095] ?? 0;
	$rate_1094 = $rate[1094] ?? 0;

	$rate_average = ( $rate_1098 + $rate_1097 + $rate_1096 + $rate_1095 + $rate_1094 ) / 5 / 20;
	if ( $missing_1097 ) {
		$rate_average = $rate_average * 5;
	}
	return (float) $rate_average;
}
...
/**
 * Decode wp_markting.order_phones (PHP-serialized array or JSON string).
 *
 * @param mixed $raw Value from wp_markting.order_phones or players_phone meta.
 * @return array<int, array<string, mixed>>
 */
function ez_decode_order_phones_row( $raw ): array {
	if ( is_array( $raw ) ) {
		return $raw;
	}
	...
	if ( $potential_orders ) {
		foreach ( $potential_orders as $row ) {
			...
			if ( ! empty( $row->order_phones ) && ! empty( $user_phone_10 ) ) {
				foreach ( ez_decode_order_phones_row( $row->order_phones ) as $teammate ) {
