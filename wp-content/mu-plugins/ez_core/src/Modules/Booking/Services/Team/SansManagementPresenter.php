...
/**
 * HTML presentation for team/panel sans-management grids.
 */
final class SansManagementPresenter
{
	/**
	 * @param array<string, mixed> $data
	 */
	public static function render( array $data, int $productId, int $dayStartTime ): string {
		if ( array() === $data ) {
			return '';
		}

		$isAllClosed  = ! empty( $data['is_all_closed'] );
		$closeChecked = $isAllClosed ? 'checked' : '';
		$openChecked  = ! $isAllClosed ? 'checked' : '';
		$reservation  = is_array( $data['reservation_data'] ?? null ) ? $data['reservation_data'] : array();

		$html  = self::renderBulkRadioTemplate( $isAllClosed, $openChecked, $closeChecked );
		$html .= self::renderSlots( $reservation, $productId, $dayStartTime );
	 ...
	 */
	public static function buildMojavezedarMap( array $reservationData ): array {
		$ids = array();
	 ...
	 */
	public static function resolveTheme( array $reservedData, bool $isMoj ): array {
		if ( $isMoj ) {
			return array(
	...
	/**
	 * @param list<array{time: int, status: string, reserved_data: array<string, mixed>|null, time_lbl?: string}> $reservationData
	 */
