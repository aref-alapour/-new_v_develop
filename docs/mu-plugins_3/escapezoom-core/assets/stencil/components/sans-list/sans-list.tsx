...
export class EzSansList {
  ...
  render() {
    ...
    return (
      <div class="block w-full bg-white rounded-2xl border border-[#E4EBF0] overflow-hidden">
        <div class="flex overflow-x-auto pb-2 scrollbar-hide border-b border-[#E4EBF0]">
          {this.dates.map((date: any, index: number) => (
            <button
              class={`
                flex-shrink-0 flex flex-col items-center justify-center min-w-[80px] p-4 gap-1 transition-all relative
                ${this.selectedDate === date.date ? 'text-primary-600 bg-primary-50' : 'text-gray-500 hover:bg-gray-50'}
                ${index !== this.dates.length - 1 ? 'border-l border-[#E4EBF0]' : ''}
              `}
