...
final class GenreScreen extends BaseCrudScreen
{
    ...
    {
        return __('ژانرها', 'escapezoom-core');
    }
    ...
    {
        return __('افزودن ژانر', 'escapezoom-core');
    }

    protected static function getEditTitle(): string
    {
        return __('ویرایش ژانر', 'escapezoom-core');
    }
    ...
    protected static function renderList(): void
    {
        ...
        $refreshUrl = admin_url('admin-ajax.php') . '?action=ez_gen_refresh_table';
        echo '<div class="wrap" id="ez-genres-table-container">';
        ...
        echo '<table class="wp-list-table widefat fixed striped" hx-trigger="refreshGenresTable from:body" hx-get="' . esc_url($refreshUrl) . '" hx-target="#ez-genres-table-container" hx-swap="innerHTML"><thead><tr><th>ID</th>';
    ...
    public static function ajaxSave(): void
    {
        ...
        static::assertAjaxNonce('ez_gen_ajax_save');
        ...
        // یونیک بودن نام
        if (Genre::query()->where('name', $name)->exists()) {
            wp_send_json_error(['name' => __('این نام قبلاً استفاده شده است.', 'escapezoom-core')], 400);
        }

        // یونیک بودن اسلاگ
        if ($slug !== '' && Genre::query()->where('slug', $slug)->exists()) {
        ...
        wp_send_json_success(['id' => (int) $genre->id, 'name' => $genre->name]);
    }

    public static function ajaxUpdate(): void
    {
        static::assertAjaxCapability();
        static::assertAjaxNonce('ez_gen_ajax_update');
        ...
        if (Genre::query()->where('name', $name)->where('id', '!=', $id)->exists()) {
            wp_send_json_error(['name' => __('این نام قبلاً استفاده شده است.', 'escapezoom-core')], 400);
        }
        if ($slug !== '' && Genre::query()->where('slug', $slug)->where('id', '!=', $id)->exists()) {
            wp_send_json_error(['slug' => __('این اسلاگ قبلاً استفاده شده است.', 'escapezoom-core')], 400);
        }
