...
class Jwt_Auth_Public
{
    ...
    public function generate_token($request)
    {
        ...
        if (!$secret_key) {
            return new WP_Error(
                'jwt_auth_bad_config',
                __('JWT TOKEN ERROR: JWT is not configurated properly, please contact the admin', 'wp-api-jwt-auth'),
                array(
                    'status' => 401,
                )
            );
        ...
        if (is_wp_error($user)) {
            ...
            return new WP_Error(
                '[jwt_auth] ' . $error_code,
                'JWT TOKEN ERROR: ' . $user->get_error_message($error_code),
                array(
                    'status' => 401,
                )
            );
...
//        $token = $this->validate_token(false);
     ...
     */
    public static function validate_token($output = true, $required = false)
    {
        $token = get_token_from_header();

        $raw_token = $token;

        if ( !is_string($raw_token) ) // when the token is not string means there is an error from get_token_from_header() function
            if ( $raw_token->has_errors() )
                if ( $required )
                    wp_send_json_error($raw_token->get_error_messages(), 401); // if the api needs token so I show the error
                else
                    return; // if token is not required for the api but still there is an error     so I prevent executing rest of codes and return to that part
        ...
        if (!$secret_key) {
            return new WP_Error(
                'jwt_auth_bad_config',
                'JWT TOKEN ERROR: JWT is not configured properly, please contact the admin',
                array(
                    'status' => 401,
                )
            );
        ...
        try {
            ...
            if ($token->iss != get_bloginfo('url')) {
                ...
                return new WP_Error(
                    'jwt_auth_bad_iss',
                    'JWT TOKEN ERROR: The iss do not match with this server',
                    array(
                        'status' => 401,
                    )
                );
            ...
            if (!isset($token->data->user->id)) {
                ...
                return new WP_Error(
                    ...
                    'JWT TOKEN ERROR: User ID not found in the token',
                    array(
                        'status' => 401,
                    )
                );
