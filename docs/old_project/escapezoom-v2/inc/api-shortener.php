...
class EZ_API_Shortener
{
    ...
    private function init_hooks()
    {
        ...
        add_action('created_yith_product_brand', array($this, 'handle_product_brand_create'), 10, 2);
        add_action('edited_yith_product_brand', array($this, 'handle_product_brand_update'), 10, 2);
        ...
        add_action('yith_product_brand_edit_form_fields', array($this, 'add_taxonomy_shortlink_field'), 10, 2);
    }
    ...
    /**
     * Handle product brand creation
     */
    public function handle_product_brand_create($term_id, $tt_id)
    {
        $this->call_api(array(
            'original_url' => get_term_link($term_id, 'yith_product_brand'),
            'type' => 'brand',
            'item_id' => $term_id
        ));
    }
    ...
    /**
     * Handle product brand update
     */
    public function handle_product_brand_update($term_id, $tt_id)
    {
        $this->call_api(array(
            'original_url' => get_term_link($term_id, 'yith_product_brand'),
            'type' => 'brand',
...
?>
    ...
    public function enqueue_admin_scripts($hook)
    {
        ...
        if (
            ...
            (in_array($post_type, array('post', 'page', 'product')) ||
                isset($_GET['taxonomy']) && in_array($_GET['taxonomy'], array('category', 'post_tag', 'product_cat', 'product_tag', 'yith_product_brand')))
        ) {

            wp_enqueue_script(
                'ez-shortlink-admin',
                get_template_directory_uri() . '/js/shortlink-admin.js',
                array('jquery'),
                '1.0.0',
                true
            );
    ...
    private function sync_brands($limit, $offset = 0)
    {
        ...
        if (!is_wp_error($brands)) {
            foreach ($brands as $brand) {
                $this->handle_product_brand_create($brand->term_id, $brand->term_taxonomy_id);
            }
        }
    }
    ...
    private function call_api_for_custom($original_url)
    {
        ...
        error_log('EZ API Custom: Token - ' . substr($this->token, 0, 10) . '...');
