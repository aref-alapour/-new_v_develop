...
$team =  get_field('team', "{$taxonomy}_{$id}");
$brand_type_games = get_field('brand_type_games', "{$taxonomy}_{$id}");
$brands_location_add = get_field('brands_location_add', "{$taxonomy}_{$id}");
        ...
        <ol class="inline-flex items-center">
            <li class="group">
                <div class="flex items-center">
                    <a class="text-2xs font-medium text-slate-310 hover:text-primary-600" href="<?php echo site_url() ?>">صفحه اصلی</a>
                </div>
            </li>
            <li class="group">
                <div class="flex items-center">
                    <div class="mx-5 h-2 w-px bg-slate-110"></div>
                    <a class="text-2xs font-medium text-slate-310 hover:text-primary-600" href="<?php echo site_url('/brands/'); ?>">برندها</a>
                </div>
            </li>
            <li class="group">
                <div class="flex items-center">
    ...
    <div>
        ...
        <?php if ($brand_type_games): ?>
            <div class="text-primaryColor text-base lg:text-lg font-bold my-4 lg:my-8">
                ...
                foreach ($brand_type_games as $game) {
                    if (isset($game['عنوان_تایپ'])) {
                        $titles[] = esc_html($game['عنوان_تایپ']);
                    }
                }
                $result = implode('<span class="bg-[#90A1B9] mx-2 w-[3px] h-[3px] inline-block rounded-full"></span>', $titles);
                echo $result;
                ?>
            </div>
        <?php endif; ?>
        <?php if ($brands_location_add): ?>
            <div class="flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="18" viewBox="0 0 16 18" fill="none" class="mx-0">
    ...
    <section class="max-w-full py-4 md:py-5 lg:py-9 md:mt-7.5">
        ...
        <div class="relative overflow-hidden embla_normal horizontal dragFree">
            <div class="embla__viewport">
                <div id="brand-slider" class="embla__container first:child:before:hidden child:before:w-px child:before:absolute child:before:bg-gradient-to-t child:before:from-white child:before:via-slate-110 child:before:to-white child:before:-right-3.5 child:lg:before:-right-6 child:before:h-full min-h-[200px] child:box-content lg:min-h-[300px] flex child:ml-7 md:child:ml-12  last:child:ml-0 child:relative child:shrink-0 child:grow-0 child:w-[156px] md:child:w-[200px] child:py-2.5"> <?= $brand_products->products ?> </div>
            </div>
            <button class="embla__button embla__button--prev brand-btn absolute right-0 top-1/2 translate-y-[-115px] rotate-180 z-50 cursor-pointer touch-manipulation appearance-none -mr-px hidden" type="button">
            ...
            <button class="embla__button embla__button--next brand-btn absolute left-0 top-1/2 translate-y-[-115px] z-50 cursor-pointer touch-manipulation appearance-none -ml-px hidden" type="button">
                <svg xmlns="http://www.w3.org/2000/svg" width="30" fill="none" viewBox="0 0 30 113">
                    <g clip-path="url(#arrow_aa)">
                        <path fill="#BFCBD9" fill-rule="evenodd" d="M0 3.75c0 28.814 30 32.928 30 52.823 0 21.023-30 26.414-30 56.595V3.75Z" clip-rule="evenodd"></path>
                        <path fill="#fff" fill-rule="evenodd" d="M0 1c0 28.814 27 33.679 27 53.573 0 21.022-27 23.914-27 54.094V1Z" clip-rule="evenodd"></path>
...
<?php if ($brand->description): ?>
    <div class="container">
        ...
        <p class="text-justify mt-8"><?php echo $brand->description; ?></p>
    </div>
<?php endif; ?>
<?php if ($team): ?>
    <section class="flex flex-col mt-10">
        ...
        <div class="members flex gap-9 overflow-x-auto no-scrollbar">
            <?php foreach ($team as $member): ?>
                <div class="flex flex-col text-center shrink-0 max-lg:w-[124px] w-[154px]">
                    <?php echo wp_get_attachment_image($member['avatar']['ID'], 'large', false, [
                        'class' => 'aspect-square rounded-full object-cover mb-3'
                    ]); ?>
                    <span class="text-xl"><?php echo esc_html($member['name']); ?></span>
                    <span class="text-gray-400"><?php echo esc_html($member['position']); ?></span>
                </div>
            <?php endforeach; ?>
        </div>
    </section>
