...
if ((int) $order->get_user_id() == 3325 || 1) {
    ...
    if ($row_check_conflict) { // اگر سانس جاری قبلا رزرو شده است پس تداخل اتفاق افتاده
    ...
    /**********************************************************************************************************************************/
    // جلوگیری از رفرش تنکیوپیج

    if ($_SERVER['HTTP_HOST'] !== 'dev.escapezoom.local') {
        if (get_post_meta($order_id, 'seen_tnx_page', true)) { // از رفرش شدن تنکیو پیج جلوگیری میکند.

            wp_redirect('https://escapezoom.ir/t/' . str_replace("wc_order_", "", get_post_meta($order_id, '_order_key', true)));
            exit;

        } else update_post_meta($order_id, 'seen_tnx_page', time());
    } ?>

    <section class="container mx-auto my-10">

        <div class="flex items-center justify-between gap-x-4 rounded-2xl border border-[#E8EDF1] px-5 py-6 shadow-[0px_2px_0px_0px_#E8EDF1] lg:hidden">
            <div>
                <p class="text-lg font-medium">
                    رزرو شما با
                    <span class="text-xl font-bold text-[#049654]">موفقیت</span>
                    ثبت شد
                </p>
                <div class="mt-3 space-x-2 space-x-reverse">
                    <span class="text-[#889BAD]">شماره رزرو</span>
                    <span class="font-bold"><?php echo esc_html($order_id); ?></span>
                </div>
            </div>
            <div class="success-payment flex h-[83px] w-[83px] items-center justify-center rounded-full text-center">
        ...
        <section class="mt-5 rounded-xl border border-[#E8EDF1]  px-6 pb-9 pt-7 shadow-[0px_2px_0px_0px_#E8EDF1] lg:p-13">
            <div class="overflow-hidden pb-0.5">
                <div class="flex shrink-0 items-center justify-between gap-x-4 rounded-2xl border border-[#E8EDF1] px-5 py-6 shadow-[0px_2px_0px_0px_#E8EDF1] max-lg:hidden lg:flex-col lg:justify-center lg:gap-y-6">
                    <div class="lg:order-1 lg:text-center">
                        <p class="text-lg font-medium lg:text-xl">
                            رزرو شما با
                            <span class="text-xl font-bold text-[#049654] lg:text-2xl">موفقیت</span>
                            ثبت شد
                        </p>
                        <div class="mt-3 space-x-2 space-x-reverse">
                            <span class="text-[#889BAD]">شماره رزرو</span>
                            <span class="font-bold lg:text-lg"><?php echo esc_html($order->get_id()); ?></span>
                        </div>
                    </div>
                    <div class="success-payment flex h-[132px] w-[132px] items-center justify-center rounded-full text-center">
            ...
            <div class="mt-9 lg:flex lg:justify-between lg:gap-x-20">
                <div class="mt-10 w-full lg:mt-14">
                    <a id="redirectLink" class="mx-auto flex w-fit items-center gap-x-4" href="<?php echo home_url('/t/' . str_replace('wc_order_', '', get_post_meta($order_id, '_order_key', true))); ?>">
                    <span id="redirectText" class="text-lg font-bold text-primary-500 underline underline-offset-2 transition hover:text-primary-600">تا <span id="countdown">10</span> ثانیه دیگر به صفحه چاپ بلیط منتقل می‌شوید (یا اینجا را کلیک کنید)</span>
            ...
            <script>
                ...
                const textEl = document.getElementById('redirectText');
                ...
                const interval = setInterval(() => {
                    ...
                    if (seconds > 0) {
                        ...
                        textEl.textContent = 'در حال انتقال به صفحه چاپ بلیط…';
...
/**********************************************************************************************************************************/
// اگر قیمت کل 0 بود پس اختلالی وجود داشته.
...
if (!$total_amount and 0) {
    ...
    echo '<h2 class="tac" style=" font-size: 1.3em; " id="err_reserve">خطا: سانس شما رزرو نشد لطفا مجددا تلاش بفرمایید <br><a href="' . $product_url . '">بازگشت به صفحه بازی ' . $product_title . '</a></h2>';
    return;
}
...
$game_brand     = get_the_terms($product_id, 'yith_product_brand')[0]->name;
$game_brand     = $game_brand ?: null;
...
$rest = (int)$item_total - $prepaid; // صرفا برای نمایش در بلیط
...
while ($attempt < $max_attempts && !$success) {
    ...
    $check_res = ez_reservation(['type' => 'query_execution', 'data' => $check_data]);
    ...
    if (!empty($decoded['cnt']) && intval($decoded['cnt']) > 0) {
        ...
        $attempt++;
        saeed_store("Insert attempt {$attempt} failed for customer_id={$user_id}, order_id={$order_id}");
        usleep(200000); // کمی مکث
    }
}

if (!$success)
    saeed_store("Insert failed after {$max_attempts} attempts for customer_id={$user_id}, order_id={$order_id}");
...
$order_id_persian = englishToPersian($order_id);
...
<section class="container mx-auto my-10">
    ...
    <div class="flex items-center justify-between gap-x-4 rounded-2xl border border-[#E8EDF1] px-5 py-6 shadow-[0px_2px_0px_0px_#E8EDF1] lg:hidden">
        <div>
            ...
            <div class="mt-3 space-x-2 space-x-reverse">
                <span class="text-[#889BAD]">شماره رزرو</span>
                <span class="font-bold"><?php echo esc_html($order->get_id()); ?></span>
            </div>
        </div>
        <div class="success-payment flex h-[83px] w-[83px] items-center justify-center rounded-full text-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="46" height="38" viewBox="0 0 74 62" fill="none">
    ...
    <section class="mt-5 rounded-xl border border-[#E8EDF1]  px-6 pb-9 pt-7 shadow-[0px_2px_0px_0px_#E8EDF1] lg:p-13">
        <div class="overflow-hidden pb-0.5 lg:flex lg:gap-x-8">
            <div class="flex w-[300px] shrink-0 items-center justify-between gap-x-4 rounded-2xl border border-[#E8EDF1] px-5 py-6 shadow-[0px_2px_0px_0px_#E8EDF1] max-lg:hidden lg:flex-col lg:justify-center lg:gap-y-6">
                <div class="lg:order-1 lg:text-center">
                    <p class="text-lg font-medium lg:text-xl">
                        رزرو شما با
                        <span class="text-xl font-bold text-[#049654] lg:text-2xl">موفقیت</span>
                ...
                <div class="relative justify-between rounded-xl border border-slate-120 shadow-13 max-sm:pb-8 max-sm:pt-12 sm:flex sm:px-8">
                    <div class="lg:flex lg:flex-wrap lg:items-center lg:justify-between lg:py-12">
                        <div class="flex gap-x-5 px-6">
                            ...
                            <div class="space-y-4">
                                <div class="text-xl font-bold text-[#889BAD]">اتاق فرار</div>
                                <div class="space-y-4 lg:flex lg:items-center">
                                    <a class="text-4xl font-extrabold text-[#09192D] lg:mt-1.5" href="<?php echo $product_url ?>">
                                    ...
                                    <div class="flex items-center gap-x-4 lg:-mt-2 lg:mr-4">
                                        ...
                                        <span class="text-4xl font-extrabold text-[#FD7013]"><?php echo $product_quantity; ?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="my-5 px-6">
                            <div class="flex items-center justify-center gap-x-12 max-lg:border-b max-lg:border-t max-lg:border-b-[#E4EBF0] max-lg:border-t-[#E4EBF0] max-lg:py-5 lg:flex-col lg:items-end lg:gap-y-2">
                                <bdo dir="ltr" class="text-2xl font-bold text-[#FD7013] lg:order-2"><?php echo jdate('Y . m . d', $sans_time) ?></bdo>
                                <bdo dir="ltr" class="text-2xl font-bold text-[#09192D] lg:order-1"><?php echo jdate('H:i', $sans_time) ?></bdo>
                            </div>
                        </div>
                        <div class="flex justify-between gap-x-4 px-6 pt-12 lg:mt-12.5 lg:w-full lg:border-t lg:border-t-[#E4EBF0]">
                            <div class="flex h-[100px] w-[100px] shrink-0 items-center justify-center rounded-md bg-[#E4EBF0] p-1.5 max-lg:hidden" id="qrcode"></div>
                            <script>
                            ...
                            <div class="lg:w-full">
                                <div class="lg:flex lg:items-center lg:gap-x-5">
                                    <h2 class="text-xl font-bold text-[#09192D] max-lg:mb-4"><?php echo $brand_data->name; ?></h2>
                                ...
                                <p class="font-bold lg:my-2"><?php echo $product_meta->city_name . ' ' . get_field('room_address', $product_id); ?></p>
                                <p class="w-fit bg-black px-1 text-md font-bold text-white max-lg:hidden">
                                    QR کد کنار
                                    را برای مشاهده لوکشین با تلفن همراه اسکن کنید</p>
                            </div>
                            <div class="shrink-0 max-lg:w-25 lg:w-[70px]">
                                <img src="<?php echo wp_get_attachment_url(get_term_meta($brand_data->term_id, 'thumbnail_id', true)); ?>">
                            </div>
                        </div>
                    </div>
        ...
        <div class="mt-9 lg:flex lg:justify-between lg:gap-x-20">
            ...
            <div class="grid grid-cols-2 gap-x-4">
                <a href="<?php echo home_url("/t/" . str_replace("wc_order_", "", get_post_meta($order_id, '_order_key', true) . '?download')) ?>" class="flex gap-4 items-center justify-center relative text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 transition-all duration-300 ease-in-out disabled:bg-slate-110 disabled:text-[#ccc] disabled:cursor-not-allowed disabled:shadow-none bg-primary-600 text-white shadow-14 hover:bg-primary-500 focus-visible:outline-primary-600 min-w-16 py-2 h-14 rounded-lg px-0 lg:px-6">
                ...
                <button type="button" class="flex gap-4 items-center justify-center relative text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 transition-all duration-300 ease-in-out disabled:bg-slate-110 disabled:text-[#ccc] disabled:cursor-not-allowed disabled:shadow-none bg-gray-20 text-gray-900 shadow-13 border border-gray-100 min-w-16 py-2 h-14 rounded-lg px-0 lg:px-6 lg:py-3">
    ...
    <div class="mt-10 flex flex-col items-center gap-y-5 lg:hidden">
        <a target="blank" class="flex h-14 w-[230px] items-center justify-between gap-x-4 rounded-lg border border-[#E8EDF1] px-5 drop-shadow lg:py-3" href="<?php echo $product_url . '#address' ?>">
            مشاهده لوکیشن
        ...
        <button type="button" class="text-gray-900 relative flex h-14 w-[230px] min-w-16 items-center justify-between gap-4 rounded-lg border border-gray-100 bg-gray-20 px-5 py-2 text-sm font-semibold shadow-13 transition-all duration-300 ease-in-out focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:bg-slate-110 disabled:text-[#ccc] disabled:shadow-none lg:py-3">
            افزودن به تقویم گوگل
            <span>
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="25" viewBox="0 0 24 25" fill="none">
                    <g clip-path="url(#clip0_4133_8923)">
...
<script>
    jQuery(document).ready(function($) {
        setTimeout(() => {
            let productTitle = '<?= esc_js($product_title) ?>';
            let productCity = '<?= esc_js($game_city) ?>';
            let productArea = '<?= esc_js($game_area) ?>';
            let productType = '<?= esc_js($game_product_type) ?>';
            let orderSansTime = '<?= esc_js($order_sans_time) ?>';
            let orderSansDay = '<?= esc_js($order_sans_day) ?>';
            let orderSansDate = '<?= esc_js($order_sans_date) ?>';
            let quantity = <?= esc_js($product_quantity) ?>;
            let orderId = '<?= esc_js($order_id) ?>';
            let userPhone = '<?= esc_js($user_phone) ?>';
            let playerFname = '<?= esc_js($player_fname) ?>';
            let playerLname = '<?= esc_js($player_lname) ?>';
            let prepaid = <?= esc_js($prepaid) ?>;
            ...
            // Track coupon usage if any coupon was applied
            <?php
            $coupons = $order->get_items('coupon');
            if (!empty($coupons)) {
                foreach ($coupons as $coupon_item) {
                    $coupon_code = $coupon_item->get_code();
                    echo "zebline.event.track('apply_coupon', {
                        'order_id': orderId,
                        'coupon_code': '" . esc_js($coupon_code) . "',
                        'game_name': productTitle,
                        'game_city': productCity,
                        'game_area': productArea,
                        'game_product_type': productType
