        ...
        <form class="w-full" method="post" action="#" id="profile-form">
            <div class="grid gap-3 lg:gap-5.5 grid-cols-3">
                ...
                <div class="col-span-3">
                    ...
                    <?php } else { ?>
                        <div class="flex gap-x-2.5 rounded-lg bg-[#FDF7DE] px-3 py-2 lg:mr-auto lg:items-center">
                            <svg class="mx-0 shrink-0 max-lg:mt-1" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
                            ...
                            <p class="text-md font-bold text-[#BF9A00]">
                ...
                <p class="text-md font-bold text-[#BF9A00] col-span-3 flex items-center ">
                    ⚠️
                ...
                <div class="col-span-3">
                    <button type="button" id="set-static-password" class="bg-[#FD7013] hover:bg-[#CA5608] text-white px-6 py-3 rounded-lg text-base font-yekan-bold mt-4">
                        تنظیم رمز ثابت
                    </button>
        ...
        <form id="staticPasswordForm">
            <?php if ($has_password): ?>
                <div class="mb-4">
                    ...
                    <input type="password" id="old_password" name="old_password" class="w-full h-12 border border-[#E8EDF1] rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-[#FD7013]">
                </div>
            <?php endif; ?>
            <div class="mb-4">
                <label class="block text-sm font-yekan-bold text-navyBlue mb-2">رمز جدید</label>
                <input type="password" id="new_password" name="new_password" class="w-full h-12 border border-[#E8EDF1] rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-[#FD7013]">
            </div>
            <div class="mb-4">
                <label class="block text-sm font-yekan-bold text-navyBlue mb-2">تکرار رمز جدید</label>
                <input type="password" id="confirm_password" name="confirm_password" class="w-full h-12 border border-[#E8EDF1] rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-[#FD7013]">
            </div>
...
<script>
    jQuery(document).ready(function($) {
        ...
        // Initialize regular select boxes
        $(".select-box:not(#user-city-select)")
            .select2()
            .one('select2:open', function(e) {
                $('input.select2-search__field').prop('placeholder', 'جست و جو کنید...');
            });
        
        // Initialize city select with custom search
        $("#user-city-select").select2({
            ...
            matcher: function(params, data) {
                ...
                var cityName = data.text.toLowerCase(); // فارسی
                var citySlug = $(data.element).val().toLowerCase(); // انگلیسی (slug)
                // Search in both Persian name and English slug
                if (cityName.indexOf(searchTerm) > -1 || citySlug.indexOf(searchTerm) > -1) {
                    return data;
                }
