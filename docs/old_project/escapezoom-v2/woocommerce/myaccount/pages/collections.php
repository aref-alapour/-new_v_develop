        ...
        <div class="md:mb-8 mb-0 lg:mb-8 max-lg:border-b max-lg:border-slate-120 max-lg:pb-6">
            <div class="grid grid-cols-2 gap-4 lg:flex">
                ...
                <div class="flex items-center justify-end order-2 lg:order-3 gap-6">
                    ...
                    <div class="delete-collection-modal hidden">
                    ...
                    <?php if (!empty($collections)): ?>
                        <button type="button" id="delete-collection-button" class="text-red-500 hover:text-red-700 transition-colors">
...
<script>
    jQuery(document).ready(function($) {
        ...
        $('#delete-collection-button').on('click', function(e) {
            ...
            let activeTab = $('button[data-status].bg-primary-500, button[data-status]:not(.text-\\[\\#889BAD\\])').first();
            if (activeTab.length === 0) {
                activeTab = $('button[data-status]').first();
            }

            if (activeTab.length > 0) {
                let tabStatus = activeTab.data('status');
                let collectionTitle = activeTab.text().trim();

                // Extract collection ID from the status (assuming format like 'collection-1')
                currentCollectionId = tabStatus.replace('collection-', '');
