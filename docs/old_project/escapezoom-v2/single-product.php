...
$satisfaction_positive_count_display = null;
if ($satisfaction_count)
    $satisfaction_positive_count_display = ($satisfaction_positive_count / $satisfaction_count) * 100;
...
<div class="flex justify-between items-center my-5 <?= !$is_tehran ? 'lg:mt-8' : '' ?>">
    ...
    <div class="flex items-center">
        ...
        ?>
                <a href="<?= !empty($breadcrumb_item['url']) ? esc_url($breadcrumb_item['url']) : '#' ?>" class="text-sm font-bold text-[#9AA8B7] <?= $is_hidden ? 'max-lg:hidden' : '' ?>"><?= esc_html($breadcrumb_item['title']) ?></a>
                <?php if (!$is_last): ?>
                    ...
                    <div class="border border-[#E2E8F0] px-3 py-4 rounded-lg flex gap-x-15">
                        ...
                        <?php if ($satisfaction_count): ?>
                            <div class="flex items-center">
                                ...
                                <p class="text-lg font-extrabold"><span class="animate-counter" data-target="<?= $satisfaction_positive_count_display ?>" data-decimals="0">0</span>%</p>
                            </div>
                        <?php endif; ?>
                        <div class="flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <div id="menuContainer" class="bg-white flex flex-col px-4 py-3 rounded-xl overflow-hidden transition-all duration-500 ease-in-out">
        ...
        <div class="menu-item flex justify-between items-center extra-item hidden">
            ...
            <p class="shrink-0">
                <?php
                $level = $data['level'];
                if ($level == 1) echo 'آسان';
                elseif ($level == 2) echo 'متوسط';
                elseif ($level == 3) echo 'سخت';
                elseif ($level == 4) echo 'خیلی سخت';
                else echo 'نامشخص';
                ?>
            </p>
        </div>
                            ...
                            <div class="flex justify-between items-center max-lg:px-7">
                                <div class="flex items-center gap-3">
                                    <div class="w-[40px] h-[40px] rounded-xl overflow-hidden bg-[#EEE8E8] flex items-center justify-center">
                                        <?php
                                        echo get_avatar(
                                            $comment['author_id'] ?: $comment['author_title'],
                                            64,
                                            '',
                                    ...
                                    <p class="text-sm font-bold"><?php echo $comment['author_title']; ?></p>
                                </div>
                                <span class="max-lg:text-base">
                                    <?php if ($comment['author_id']) {
                                        $instant_user_level = get_comment_meta($comment['id'], 'user_level', true) ?: 1;
                                        if ($comment['date'] > COMMENT_NEW_VER_TIMESTAMP)
                                            user_badge_by_level($instant_user_level, 'px-2 py-0.5 text-xs font-bold mr-2.5 rounded-xl', 'user_level');
                                        else
    ...
    <div id="comment-modal-desktop" class="hidden fixed inset-0 z-[9999] items-center justify-center max-lg:hidden">
        <div class="relative bg-white rounded-2xl p-8 w-full max-w-[800px] max-h-[90vh] overflow-y-auto mx-4 transform transition-all duration-300 scale-95 opacity-0">
            ...
            <div class="flex items-center justify-between mb-6 mt-4">
                <h3 class="text-2xl font-black">ارسال نظر</h3>
            ...
            <form id="comment-form" class="send-comment" method="post">
                ...
                <?php } ?>
                <button type="submit" class="w-full bg-[#2B7FFF] text-white py-4 max-lg:py-3.5 rounded-xl font-bold text-lg max-lg:text-base hover:bg-[#1e5fbf] transition-colors flex items-center justify-center">
                    ثبت نظر
                </button>
            </form>
        </div>
    ...
    <div id="comment-modal-mobile" class="fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-[0_-4px_10px_rgba(0,0,0,0.1)] translate-y-full transition-transform duration-300 ease-in-out z-[9999] lg:hidden px-7 pt-9 max-h-[90vh]" style="display: none;">
        <div class="relative">
            ...
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-xl font-black">ارسال نظر</h3>
                <span class="text-sm text-[#62748E]">برای <?php echo $data['title'] ?></span>
            </div>
