...
$total_comments = $wpdb->get_var( "SELECT COUNT(*) FROM wp_comments WHERE comment_post_ID IN (" . implode( ",", $active_products ) . ") AND comment_approved = 1 AND comment_parent = 0" );
...
if ( ! empty( $query->comments ) ) {
    foreach ( $query->comments as $comment ) {
        ...
        $rate = get_comment_meta( $comment->comment_ID, 'comment_rating' )[0];
        ...
        $item = [
            ...
            'comment'   => $comment,
            'rating'    => [
                'فضاسازی'          => (int)$rate[1094] / 20,
                'کیفیت معما'       => (int)$rate[1095] / 20,
                'تازگی و خلاقیت'   => (int)$rate[1098] / 20,
                'بازیگردانی و اکت' => (int)$rate[1096] / 20,
                'برخورد پرسنل'     => (int)$rate[1097] / 20,
            ],
...
if ( ! empty( $data ) ) :
    ...
    foreach ( $data as $comment ) : ?>
        ...
        <div class="lg:hidden flex flex-col border-b pb-7 mb-7">
            <div class="flex justify-between">
                <div class="flex flex-row items-center gap-[8px]">
                    <span class="font-bold"><?php echo $user->display_name; ?></span>
                    <?php user_badge_by_level( $comment['id'] ); ?>
                </div>
