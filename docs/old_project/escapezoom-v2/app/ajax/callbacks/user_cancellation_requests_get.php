...
// Function to get user level display with colors
...
foreach ($requests as $request) :
    $hours_to_sans = ($request['sans_time'] - $now) / 3600;

    // Convert expired requests
    if ($hours_to_sans <= 2 && $request['status'] === 'pending') {
...
?>
    ...
    <section class="request-card flex justify-center items-center mt-5">
        ...
        <div class="rounded-[20px] overflow-hidden w-full pt-[20px] border shadow-[0px_1px_0px_0px_#DBE2EA] <?php echo $request_cart_color ?>">
            <div class="flex justify-between px-[20px]">
                ...
                <div class="flex justify-center items-center gap-[6px] bg-white rounded-[6px] py-[4px] px-2">
                    ...
                    <p class="text-[#889BAD] text-base font-extrabold"><?php echo floor(($now - (int)$request['created_at']) / 3600) ?: 'کمتر از 1 ' ?> ساعت پیش</p>
                </div>
            </div>
            <div class="flex justify-between max-lg:flex-col max-lg:gap-2 px-[20px] my-[20px]">
                ...
                <?php if ($request['status'] === 'pending') : ?>
                    <div class="max-lg:grid max-lg:grid-cols-3 lg:flex items-center gap-2">
                        <button class="reject-btn text-base font-extrabold text-[#9AA8B7] bg-white px-4 py-3 rounded-xl cursor-pointer hover:bg-slate-100" data-request-id="<?php echo $request['ID']; ?>">رد کردن</button>
                        <button class="approve-btn text-base font-extrabold text-white bg-[#02C96F] px-4 py-3 rounded-xl cursor-pointer hover:bg-[#01B05A] max-lg:col-span-2" data-request-id="<?php echo $request['ID']; ?>">تایید و لغو سانس</button>
                    </div>
                <?php else : ?>
            ...
            <div class="flex-col items-center justify-center bg-[#F8FAFB] px-[20px] pt-2 pb-[20px] description hidden">
                ...
                <div class="flex justify-between items-center mt-[14px] mx-[20px] max-lg:flex-wrap max-lg:gap-y-4">
                    ...
                    <div class="flex">
                        <div class="flex flex-col">
                            ...
                            <p class="text-base font-extrabold text-navyBlue text-start"><?php echo $order->billing_phone ?></p>
                        </div>
