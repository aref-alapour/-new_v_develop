...
foreach ($cities_data as $city) {
    $cities_menu[] = [
        ...
        'slug' => get_the_permalink((int) $city['city_id'])
    ];
}
if (!isset($_SESSION)) {
    session_start();
...
<style>
    ...
    <div class="sms-topbar">
        با توجه به اختلالات به وجود آمده در ارسال پیامک، برای ورود به حساب کاربری از رمز ثابت استفاده بفرمائید. درصورتی که تاکنون رمز ثابت ثبت نکرده اید با پشتیبانی در ارتباط باشید.
    </div>
                            ...
                            <div class="hidden group-hover:block absolute top-full left-0 z-50">
                                <div class="border w-[262px] bg-white rounded-br-lg rounded-bl-lg shadow-lg" style="max-height: 400px;">
                                    <div class="py-5 px-3">
                                        ...
                                        <div id="cities-grid-lg" class="flex justify-between">
                                            <?php
                                            // Separate featured and non-featured cities
                                            $featured_cities = array_filter($cities_menu, function ($city) {
                                                return $city['is_featured'];
                                            });
                                            $non_featured_cities = array_filter($cities_menu, function ($city) {
                                                return !$city['is_featured'];
                                            });

                                            // Split featured cities into two columns
                                            $half = ceil(count($featured_cities) / 2);
                                            $left_column = array_slice($featured_cities, 0, $half);
                                            $right_column = array_slice($featured_cities, $half);
                                            ?>
                                            <div class="flex flex-col gap-1">
                                                <?php foreach ($left_column as $city): ?>
                                                    <a href="<?php echo esc_url($city['slug']); ?>" class="city-item-featured w-[115px] h-[38px] rounded-lg px-4 flex items-center cursor-pointer hover:bg-[#EDF2F5] hover:text-primary-500 transition-colors text-sm">
                                                        <?php echo esc_html($city['name']); ?>
                                                    </a>
                                                <?php endforeach; ?>
                                            </div>
                                            <div class="flex flex-col gap-1">
                                                <?php foreach ($right_column as $city): ?>
                                                    <a href="<?php echo esc_url($city['slug']); ?>" class="city-item-featured w-[115px] h-[38px] rounded-lg px-4 flex items-center cursor-pointer hover:bg-[#EDF2F5] hover:text-primary-500 transition-colors text-sm">
                                                        <?php echo esc_html($city['name']); ?>
                                                    </a>
                                                <?php endforeach; ?>
                                        ...
                                        <!-- Non-featured cities list for search results (hidden by default) -->
                                        <div id="city-search-results-lg" class="hidden mt-4">
                                            <div class="flex flex-col gap-1" style="max-height: 200px; overflow-y: auto;">
                                                <?php foreach ($non_featured_cities as $city): ?>
                                                    <a href="<?php echo esc_url($city['slug']); ?>" class="city-item-search w-full h-[38px] rounded-lg px-4 flex items-center cursor-pointer hover:bg-[#EDF2F5] hover:text-primary-500 transition-colors text-sm" data-city-name="<?php echo esc_attr(strtolower($city['name'])); ?>">
                                                        <?php echo esc_html($city['name']); ?>
                                                    </a>
                                                <?php endforeach; ?>
                                            </div>
                                    ...
                                    <div class="hidden group-hover:block absolute top-full left-5 z-50 mt-[15px]">
                                        <div class="border w-[262px] bg-white rounded-br-lg rounded-bl-lg shadow-lg" style="max-height: 400px;">
                                            <div class="py-5 px-3">
                                                ...
                                                <div class="w-full h-[1px] bg-[#E4EBF0] my-4"></div>
                                                <div id="cities-grid-sm" class="flex justify-between">
                                                    <?php
                                                    // Separate featured and non-featured cities
                                                    $featured_cities = array_filter($cities_menu, function ($city) {
                                                        return $city['is_featured'];
                                                ...
                                                <!-- Non-featured cities list for search results (hidden by default) -->
                                                <div id="city-search-results-sm" class="hidden mt-4">
                                                    <div class="flex flex-col gap-1" style="max-height: 200px; overflow-y: auto;">
                                                        <?php foreach ($non_featured_cities as $city): ?>
                                                            <a href="<?php echo esc_url($city['slug']); ?>" class="city-item-search w-full h-[38px] rounded-lg px-4 flex items-center cursor-pointer hover:bg-[#EDF2F5] hover:text-primary-500 transition-colors text-sm" data-city-name="<?php echo esc_attr(strtolower($city['name'])); ?>">
                                                                <?php echo esc_html($city['name']); ?>
