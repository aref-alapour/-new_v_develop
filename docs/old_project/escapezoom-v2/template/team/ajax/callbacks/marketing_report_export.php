...
/**
 * Add 3 hours and 30 minutes to server time to get local time
 */
function adjustToLocalTime($server_datetime)
{
    if (empty($server_datetime)) return '';

    $date = new DateTime($server_datetime);
    $date->modify('+3 hours +30 minutes');
    return $date->format('Y-m-d H:i:s');
}

/**
 * Subtract 3 hours and 30 minutes from local time to get server time
 * Used when user selects dates in local timezone
 */
function adjustToServerTime($local_datetime)
{
    if (empty($local_datetime)) return '';
...
try {
    ...
    if ($date_range === 'calendar' && $start_date && $end_date) {
        // User selects dates in local time, need to convert to server time
        // Subtract 3.5 hours from start date (00:00:00 local = previous day 20:30:00 server)
        // Subtract 3.5 hours from end date (23:59:59 local = same day 20:29:59 server)
        $server_start = adjustToServerTime($start_date . ' 00:00:00');
        $server_end = adjustToServerTime($end_date . ' 23:59:59');
    ...
    if ($export_type === 'csv') {
        ...
        foreach ($marketing_data as $row) {
            // Adjust server time to local time (add 3.5 hours)
