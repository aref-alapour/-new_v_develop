...
foreach ($comments as $comment) :
    ...
    // Debug: log raw data
    if ($comment_id == 1) { // Only debug for first comment to avoid spam
        error_log('Comment ID: ' . $comment_id);
        error_log('Raw comment_rating: ' . print_r($comment_ratings_raw, true));
    }

    if (is_array($comment_ratings_raw)) {
        // Map numeric keys to text keys based on the system structure
        // 1094 = فضاسازی (atmosphere), 1095 = معما (puzzle), 1096 = بازیگردانی‌واکت (acting), 1097 = برخورد پرسنل (staff), 1098 = تازگی و خلاقیت (creativity)
        $comment_ratings = [
            'acting' => isset($comment_ratings_raw[1096]) ? intval($comment_ratings_raw[1096]) / 20 : 0,
            'puzzle' => isset($comment_ratings_raw[1095]) ? intval($comment_ratings_raw[1095]) / 20 : 0,
            'atmosphere' => isset($comment_ratings_raw[1094]) ? intval($comment_ratings_raw[1094]) / 20 : 0,
            'creativity' => isset($comment_ratings_raw[1098]) ? intval($comment_ratings_raw[1098]) / 20 : 0,
            'staff' => isset($comment_ratings_raw[1097]) ? intval($comment_ratings_raw[1097]) / 20 : 0
        ];
    } else {
        // مقادیر پیش‌فرض در صورت عدم وجود داده
        $comment_ratings = [
            'acting' => 0,
            'puzzle' => 0,
            'atmosphere' => 0,
    ...
    if ($comment_id == 1) { // Only debug for first comment to avoid spam
        error_log('Mapped comment_ratings: ' . print_r($comment_ratings, true));
    }

    if ($comment['comment_date'] > COMMENT_NEW_VER_TIMESTAMP)
        $user_level = get_comment_meta($comment_id, 'user_level', true) ?: 1; // لول پلیر در لحظه ای که این کامنت رو گذاشته. (حافظه دار)
    else
        $user_level = 1;
    ...
    <div class="grid gap-x-8 px-4 py-4 border-b-[#E8EDF1] <?php echo $row_bg_class ?>" style="grid-template-columns: 5fr 1fr 6fr 3fr 4fr 1fr 1fr">
        <div class="flex items-start gap-2">
            <div class="font-bold text-base "><?php echo $author_title ?></div>
            <span class="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded" style="color:<?php echo $user_color ?>;background:<?php echo $user_background ?>;"><?php echo $user_text ?></span>
        </div>
        <button type="button" class="openModalLevel cursor-pointer flex gap-x-1" data-comment-id="<?= $comment_id ?>" data-acting="<?= $comment_ratings['acting'] ?? 0 ?>" data-puzzle="<?= $comment_ratings['puzzle'] ?? 0 ?>" data-atmosphere="<?= $comment_ratings['atmosphere'] ?? 0 ?>" data-creativity="<?= $comment_ratings['creativity'] ?? 0 ?>" data-staff="<?= $comment_ratings['staff'] ?? 0 ?>">
            <span class="font-extrabold text-base" data-subpoints='<?php echo json_encode($comment_ratings) ?>'><?php echo get_comment_meta($comment_id, 'rating', true) ?></span>
        ...
        <div class="leading-relaxed space-y-1 text-sm font-bold text-[#5E6B77]"><?php echo $comment['comment_content'] ?></div>
        <div class="text-base font-bold"><?php echo get_the_title($comment['comment_post_ID']) ?></div>
        ...
        <div class="text-base font-bold"><?php echo '0' . $user->user_login ?></div>

        <div class="flex justify-center items-start">
            <button type="button" class="openModal cursor-pointer hover:opacity-80 w-7 h-7 mx-auto transition" data-id="<?= $comment_id ?>" data-username="<?= $author_title ?>" data-comment="<?= $comment['comment_content'] ?>"
                    data-acting="<?= $comment_ratings['acting'] ?>" data-puzzle="<?= $comment_ratings['puzzle'] ?>" data-atmosphere="<?= $comment_ratings['atmosphere'] ?>" data-creativity="<?= $comment_ratings['creativity'] ?>" data-staff="<?= $comment_ratings['staff'] ?>">
