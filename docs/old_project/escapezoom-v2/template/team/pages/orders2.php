        ...
        <div class="relative">
            ...
            <div id="orderStatusMenu" class="dropdown-menu hidden absolute top-full left-0 mt-2 w-48 bg-white border border-[#E4EBF0] rounded-xl shadow-lg z-10">
                <div class="py-2">
                    ...
                    <a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" data-value="pending">در انتظار پرداخت</a>
...
<script>
    jQuery(document).ready(function($) {
        ...
        $('body').on("click", ".order_status_change", function() {
            ...
            let order_id = $button.closest('#maliModal').data('id');
            if (!order_id) {
        ...
        $('body').on("click", "#convert_complete_to_partial", function() {
            let $button = $(this);
            let originalText = $button.html();
            ...
            Swal.fire({
                title: 'آیا مطمئن هستید؟',
                text: 'آیا می‌خواهید وضعیت سفارش را به "پیش پرداخت" تغییر دهید؟',
                icon: 'warning',
                iconColor: '#F21543',
                showCancelButton: true,
                confirmButtonText: 'بله',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            'nonce': "<?php echo wp_create_nonce('team-ajax-nonce') ?>",
                            'callback': 'orders_actions',
                            'operation': 'convert_complete_to_partial',
                            'order_id': order_id,
                        },
                        dataType: "json",
                        success: function(data) {
                            ...
                            Swal.fire({
                                title: 'موفق',
                                text: data.data || 'وضعیت سفارش با موفقیت تغییر کرد',
                                icon: 'success',
                                confirmButtonColor: '#6B7280',
                                customClass: {
                                    confirmButton: 'swal2-confirm-custom'
        ...
        $(document).on("click", ".openMaliModal", function() {
            let order_id                = $(this).attr('data-id');
            let complete_change_flag    = $(this).attr('complete_change_flag') === '1';
            let order_status            = $(this).attr('data-order-status') || '';

            $("#maliModal").attr('data-id', order_id).show();

            // Show/hide the change status button based on complete_change_flag
            if (complete_change_flag) {
                $("#maliModal #convert_complete_to_partial").show();
            } else {
                $("#maliModal #convert_complete_to_partial").hide();
            }
