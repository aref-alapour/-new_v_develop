<div class="flex justify-between items-center">
    <div class="flex justify-center items-center gap-[44px]">
        <h1 class="text-base font-extrabold lg:text-2xl">درخواست ها</h1>
    </div>
    <div class="flex items-center gap-x-7 divide-x">
        <a href="<?= home_url('/team/cancellation_history/'); ?>" target="_blank" class="block text-center content-center h-12.5 w-28 px-[20px] bg-primary-500 text-white hover:bg-primary-600 font-extrabold text-base rounded-xl cursor-pointer">تاریخچه لغو</a>
        <button id="updateBtn" class="bg-slate-100 hover:bg-slate-120 rounded-lg flex items-center justify-center gap-x-1 h-12.5 w-28 px-[20px] text-[#1447E6]">
            <span id="updateBtnText">بروزرسانی</span>
            <svg id="updateBtnIcon" class="mx-0" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
...
<div id="myModal" class="hidden" style="display: none;">
    <div class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
        <form method="POST" id="cancellationForm" class="p-[30px] rounded-xl bg-white w-[234px] h-[213px] border border-[#DBE2EA] shadow-[0px_1px_0px_0px_#DBE2EA] flex flex-col items-center justify-center">
            <input type="hidden" name="request_id" id="modalRequestId" value="">
            <svg xmlns="http://www.w3.org/2000/svg" class="mx-0" width="38" height="37" viewBox="0 0 38 37" fill="none">
            ...
            <h3 class="text-base font-yekan-bold mt-4 mb-6 px-[7px] text-center">آیا از تأیید و لغو رزرو این سانس مطمئن هستید؟</h3>
            <div class="flex flex-row justify-between gap-[10px]">
                <button class="text-[#4E5C6D] text-base bg-[#EDF2F5] rounded-xl w-[78px] h-[33px] font-yekan-bold confirmStatusBtn" type="submit">
                    بله
                </button>
                <button class="text-[#4E5C6D] text-base bg-[#EDF2F5] rounded-xl w-[78px] h-[33px] font-yekan-bold closeModalBtn" type="button">
                    بستن
                </button>
            </div>
        </form>
...
<script>
    jQuery(document).ready(function($) {
        ...
        function showToast(message, type = 'success', duration = 3000) {
            ...
            const toast = $(`
                <div id="${toastId}" class="fixed top-4 right-4 ${bgColor} text-white px-6 py-4 rounded-lg shadow-lg z-50 flex items-center gap-3 min-w-[300px] max-w-[400px] transform translate-x-full transition-transform duration-300 ease-in-out">
