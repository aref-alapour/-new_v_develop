jQuery(document).ready(function ($) {
    if (typeof ProductJsObject === 'undefined') {
        return;
    }
...
<stop offset="1" stop-color="#3F7FF5"/>
        ...
        $.ajax({
            ...
            beforeSend: function () {
                if (!append) {
                    ...
                    $('#comments-list-container').html('<div class="text-center py-8"><div class="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-focus-blue"></div></div>');
    ...
    $(document).on('click', '.toggle-comment-btn', function(e) {
        ...
        if (!isExpanded) {
            ...
            if (isMobile && $scoresSection.length > 0) {
                ...
                setTimeout(() => {
                    $scoresSection.removeClass('max-h-0 opacity-0')
                                 .addClass('max-h-d1000 opacity-100 mt-4');
                }, delay);
            }
        } else {
            // حالت بسته (کمتر)
            ...
            if (isMobile && $scoresSection.length > 0) {
                $scoresSection.removeClass('max-h-d1000 opacity-100 mt-4')
                ...
                success: function (response) {
                    ...
                    setTimeout(() => {
                        ...
                        if (response.success) {
                            ...
                            if (
                                typeof window.zebline !== 'undefined' &&
                                window.zebline.event &&
                                typeof window.zebline.event.track === 'function'
                            ) {
                                window.zebline.event.track('send_comment', {
    ...
    const BuildSans = (room, day) => {
        $.ajax({
            ...
            beforeSend: function () {
                ...
                let loadingHTML = '';
                for (let i = 0; i < 4; i++) {
                    loadingHTML += '<div class="skeleton h-12 w-full rounded-10 mb-2.5"></div>';
                }
        ...
        reservableSessions.forEach((item) => {
            ...
            if (item.off_price > 0) {
                Text = `<div class="flex items-center gap-1">
                    <span class="text-blue text-lg font-bold">${new Intl.NumberFormat("fa-IR").format(item.off_price)}</span>
                    <span class="text-xs text-text-3 line-through-orange">${new Intl.NumberFormat("fa-IR").format(item.price)} تومان</span>
                </div>`;
            } else {
            ...
            out += `
                <div class="session-item" data-session-time="${item.time}" data-session-price="${item.off_price || item.price}" data-session-formatted="${hours}:${minutes}" data-is-vip="${is_vip}">
                    <div class="relative mb-2.5 flex cursor-pointer items-center justify-between rounded-10 transition-all overflow-hidden" style="border: 1px solid ${BackgroundColor}; color: ${BackgroundColor}">
        ...
        if (isDesktop) {
            ...
            const desktopMarkup = `
                ...
                <div class="relative">
                    ...
                    <div class="embla-sessions-desktop max-h-d270 pt-2.5 pb-8">
    ...
    $(document).on('click', '.session-item', function(e) {
        ...
        const d = persianDate.unix(sessionTime);
        
        const quantityHTML = `
            <!-- Header با دکمه بازگشت -->
            <div class="flex items-center justify-between mb-6">
                <span class="text-base font-bold text-slate-ui">انتخاب شما</span>
                <button type="button" class="change-session-btn bg-surface-sunken w-10 h-10 rounded-lg flex items-center justify-center">
                    <svg width="8" height="12" viewBox="0 0 8 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M6.5 1L1.5 6L6.5 11" stroke="#475569" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </button>
            </div>
            <hr class="my-4 border-t-2 border-slate-110">      
            <!-- تاریخ و ساعت -->
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-1 text-base font-extrabold text-slate-900">
                    <span>${d.format("dddd")}</span>
                    <span class="text-primary-2">${d.format("D")}</span>
                    <span>${d.format("MMMM")}</span>
                </div>
                <span class="text-2xl font-bold text-slate-900">${sessionFormatted}</span>
            </div>
            
            <!-- انتخاب تعداد بلیت -->
            <div class="quantity-selector relative mb-6 flex items-center justify-between rounded-xl border-2 px-4 py-2 transition-all text-black border-accent-450">
                <button type="button" class="btn-increase bg-accent-450 w-8 h-8 rounded-lg flex items-center justify-center disabled:!bg-slate-130">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                        <path d="M8 1V15M1 8H15" stroke="white" stroke-width="2" stroke-linecap="round"/>
                    </svg>
                </button>
                <span class="font-bold text-xl text-slate-900 flex items-baseline gap-2">
                    <span class="players-count text-3xl">${initialQuantity}</span>
                    <span>بلیت</span>
                </span>
                <button type="button" class="btn-decrease bg-slate-ui w-8 h-8 rounded-lg flex items-center justify-center disabled:!bg-slate-130">
            ...
            <!-- دکمه پرداخت -->
            <a id="quantity-checkout-btn" href="" class="flex items-center justify-center gap-2 bg-accent-450 text-white text-lg font-bold py-4 rounded-xl w-full">
        ...
        const selectedInfoText = `<span class="inline-block">${dayName}</span><span class="text-primary-500">${dayNumber}</span><span class="inline-block">${monthName}</span><span class="inline-block">${sessionFormatted}</span>`;
    ...
    function updateCheckoutUrls(sessionTime, quantity) {
        ...
        if ($quantityBox.length > 0) {
            ...
            const dayNumber = d.format("D");
            const monthName = d.format("MMMM");
            const selectedInfoText = `${dayName} <span class="text-primary-500">${dayNumber}</span> ${monthName} ${sessionFormatted}`;
            $('#mobile-selected-info').html(selectedInfoText);
        }
    }
        ...
        fetch(requestUrl, {
            ...
            .then(data => {
                ...
                for (let i = 1; i <= 15; i++) {
                    ...
                    const persianDateObj = persianDate.unix(dateTimestamp);
                    const dayNumber = persianDateObj.format('DD');
                    const dayName = persianDateObj.format('dddd');

                    datesHTML += `
                        <div class="date-btn w-d60 h-d60 rounded-lg border border-slate-110 flex flex-col justify-center items-center cursor-pointer flex-shrink-0 hover:border-blue transition text-ink-tab" data-date="${dateTimestamp}">
                            <span class="text-lg font-bold">${dayNumber}</span>
                            <span class="text-xs text-text-3">${dayName}</span>
                        </div>
                    `;
                }
    ...
    $(document).on('click', '#today-btn-desktop, #today-btn-mobile, .date-btn', function() {
        ...
        if (device === 'desktop') {
            // Reset همه دکمه‌ها
            $('#today-btn-desktop').removeClass('bg-blue text-white').addClass('bg-surface-sunken text-steel');
            $('.date-scroll-list-desktop .date-btn').removeClass('bg-blue text-white border-blue').addClass('border-slate-110 text-ink-tab');
            // برگردوندن رنگ نام روز هفته به حالت عادی
            $('.date-scroll-list-desktop .date-btn span.text-xs').removeClass('text-white').addClass('text-text-3');
            
            // فعال کردن دکمه انتخاب شده
            if ($btn.attr('id') === 'today-btn-desktop') {
                $btn.removeClass('bg-surface-sunken text-steel').addClass('bg-blue text-white');
            } else {
                $btn.removeClass('border-slate-110 text-ink-tab').addClass('bg-blue text-white border-blue');
                // تغییر رنگ نام روز هفته به سفید
                $btn.find('span.text-xs').removeClass('text-text-3').addClass('text-white');
            }
        } else {
            // Reset همه دکمه‌ها
            $('#today-btn-mobile').removeClass('bg-blue text-white').addClass('bg-surface-sunken text-steel');
            $('.date-scroll-list-mobile .date-btn').removeClass('bg-blue text-white border-blue').addClass('border-slate-110 text-ink-tab');
            // برگردوندن رنگ نام روز هفته به حالت عادی
            $('.date-scroll-list-mobile .date-btn span.text-xs').removeClass('text-white').addClass('text-text-3');
    ...
    $('.tab-btn').on('click', function(e) {
        ...
        if ($section.length) {
            ...
            $('.tab-btn').removeClass('text-primary-2 bg-white border border-primary-2').addClass('text-muted-blue bg-surface-muted');
            
            // Add active state to clicked tab (both mobile and desktop)
            $('[data-section="' + sectionId + '"]').removeClass('text-muted-blue bg-surface-muted').addClass('text-primary-2 bg-white border border-primary-2');
    ...
    $(window).on('scroll', function() {
        ...
        if (currentSection) {
            ...
            const wasActive = $activeTab.hasClass('text-primary-2');
            
            $('.tab-btn').removeClass('text-primary-2 bg-white border border-primary-2').addClass('text-muted-blue bg-surface-muted');
            $activeTab.removeClass('text-muted-blue bg-surface-muted').addClass('text-primary-2 bg-white border border-primary-2');
