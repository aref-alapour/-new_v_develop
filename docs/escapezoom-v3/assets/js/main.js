...
jQuery(document).ready(function ($) {
  ...
  $(".back-to-escape-room").on("click", function (e) {
            ...
            setTimeout(function () {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        let searchResult = `
                        <div id="lg-search-result" class="absolute z-10 mt-4 w-full rounded-lg border bg-white shadow-dropdown">
                            <div class="relative">
                                <button id="lg-search-close" class="text-red-600 hover:text-primary-600 leading-10 transition absolute text-25 left-3 top-2">×</button>
                                <div id="lg-search-result-list" class="max-h-75 divide-y divide-slate-105 overflow-y-auto px-4 py-5">
                                </div>
                            </div>
                        </div>
                        `
            ...
            setTimeout(function () {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        let searchResult = `
                        <div id="sm-search-result" class="absolute z-10 mt-4 w-full rounded-lg border bg-white shadow-dropdown">
                            <div class="relative">
                                <button id="sm-search-close" class="text-red-600 hover:text-primary-600 leading-10 transition absolute text-25 left-3 top-2">×</button>
                                <div id="sm-search-result-list" class="max-h-75 divide-y divide-slate-105 overflow-y-auto px-4 py-5">
                                </div>
                            </div>
                        </div>
                        `
    ...
    $("#open-mobile-nav").on('click', function () {
        $('#mobile-header').toggleClass("bg-slate-100 bg-white shadow-104")
        $(".mobile-menu-modal").toggleClass('hidden')
    })
    // slider-ajax
    $('body').on('click', '.filter-btn', function (e) {
        ...
        const snapshotAjaxUrl = (window.EZ_PRODUCTS_SNAPSHOT_AJAX || '/wp-admin/admin-ajax.php?action=ez_products_snapshot_swiper');
        $.ajax({
            type: 'POST',
            url: snapshotAjaxUrl,
            data: {
        ...
        radio.addEventListener('change', function() {
            ...
            radioInputs.forEach(r => {
                ...
                if (checkbox && checkbox.classList.contains('radio-checkbox')) {
                    checkbox.classList.remove('bg-blue', 'border-blue');
            ...
            if (this.checked) {
                ...
                if (checkbox && checkbox.classList.contains('radio-checkbox')) {
                    checkbox.classList.add('bg-blue', 'border-blue');
                    const svg = checkbox.querySelector('svg');
                    if (svg) {
                        svg.style.opacity = '1';
                    }
                        ...
                        response.data.forEach(function(item) {
                            if (item.ui === 'link') {
                                ...
                                htmlBuilder += '<li>' +
                                    ...
                                    '</svg>' +
                                    '<span class="font-bold text-steel">' + $('<div>').text(item.title).html() + '</span>' +
                                    '</span></a></li>';
                                ...
                                htmlBuilder += '<li><a href="' + $('<div>').text(item.url).html() + '" class="ez-search-result flex items-center justify-between" data-search-type="product" data-search-value="' + $('<div>').text(item.title).html() + '" data-url="' + $('<div>').text(item.url).html() + '">' +
                                    ...
                                    '<span class="font-bold text-sm text-steel inline-block">' + $('<div>').text(productType).html() + '</span>' +
                                    '<span class="text-slate-700 font-bold inline-block">' + $('<div>').text(item.title).html() + '</span>' +
                                    '</span></span>';
                                if (locationHtml) {
                                    ...
                                    '</svg>' +
                                    '<span class="flex items-center gap-x-2 text-steel text-4xs">' + locationHtml + '</span></span>';
                                }
                                htmlBuilder += '</a></li>';
                            }
                        });
