...
<div class="flex justify-between items-center gap-4">
    ...
    <div class="relative w-d304 h-d58" style="z-index: 99;">
        <input id="trans_user_search" class="w-d304 h-d58 border border-slate-105 bg-white rounded-xl outline-none pr-12 pl-12 py-5 text-xs font-yekan-bold text-navyBlue placeholder:text-end" placeholder="جست و جو" dir="ltr" />
        ...
        <div id="search-result-list" class="max-h-75 divide-y divide-slate-105 overflow-y-auto px-4 pt-2 pb-3" style="background: #f3f3f3; display: none; position: absolute; top: 60px; right: 0; left: 0; border-radius: 0 0 12px 12px; box-shadow: 0 8px 16px rgba(15, 23, 42, 0.12);">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-yekan-bold text-slate-ui">نتایج جستجو</span>
                <button id="search_result_list_close" class="w-6 h-6 flex items-center justify-center rounded-full hover:bg-slate-110 transition-colors" type="button">
...
if (array_intersect(['administrator', 'accounting'], wp_get_current_user()->roles)) : ?>
    ...
    <div class="flex justify-between items-center py-3 px-6 mt-7" style="border-radius: 14px;border: 1px solid var(--Border-Default, #E2E8F0);background: var(--slate-50, #F8FAFC);box-shadow: 0px 2px 0px 0px #E2E8F0;">
        <div class="flex items-center gap-3">
            ...
            <input id="trans_operation_amount" class="w-d156 h-d34 bg-white border border-slate-105 rounded-lg outline-none px-6 py-3 text-xs font-yekan-bold text-navyBlue text-end" style="width: 156px;height: 34px;direction: ltr;text-align: left;" placeholder="" />
        </div>

        <div class="flex items-center gap-3">
            <p class="text-sm font-yekan-bold">توضیح<span class=" text-pinkk">*</span></p>
            <input id="trans_operation_desc" class="w-d398 h-d34 bg-white border border-slate-105 rounded-lg outline-none text-xs font-yekan-bold text-navyBlue px-2" style="width: 398px; height: 34px;" placeholder="" />
        </div>

        <div class="relative dropdown">
            <select id="trans_operation_control" class="w-d156 h-d34 bg-white border border-slate-105 rounded-lg outline-none px-3 py-2 text-xs font-yekan-bold text-navyBlue appearance-none cursor-pointer hover:border-primary-500 focus:border-primary-500 transition-colors" style="background-image: url('data:image/svg+xml;charset=US-ASCII,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 4 5\'%3E%3Cpath fill=\'%23666\' d=\'M2 0L0 2h4zm0 5L0 3h4z\'/%3E%3C/svg%3E'); background-repeat: no-repeat; background-position: left 8px center; background-size: 12px;">
        ...
        <button id="trans_operation_control_btn" class="w-d81 h-d34 flex justify-center items-center bg-orange-500 rounded-lg cursor-pointer text-white text-xs font-yekan-heavy hover:bg-orange-600 transition-colors" style="box-shadow: 0px 2px 0px 0px #CA5608; width: 81px; height: 34px;">اعمال</button>
...
<div id="collections-owners-table-wrapper" class="relative bg-white rounded-xl overflow-hidden shadow-sm mt-6">
    <div class="flex justify-between items-center px-6 py-4 border-b border-slate-105">
    ...
    <div id="collections-owners-table-container" class="overflow-x-auto">
        <table id="collections-owners-table" class="w-full">
            <thead class="bg-surface-sunken">
                <tr>
                    <th class="px-6 py-4 text-sm font-yekan-bold text-slate-ui text-center cursor-pointer hover:bg-slate-105 transition-colors" data-sort="row">
                        ردیف <span class="sort-icon">⇅</span>
                    </th>
                    <th class="px-6 py-4 text-sm font-yekan-bold text-slate-ui text-center cursor-pointer hover:bg-slate-105 transition-colors" data-sort="full_name">
                        نام و نام خانوادگی <span class="sort-icon">⇅</span>
                    </th>
                    <th class="px-6 py-4 text-sm font-yekan-bold text-slate-ui text-center cursor-pointer hover:bg-slate-105 transition-colors" data-sort="phone">
                        شماره تلفن <span class="sort-icon">⇅</span>
                    </th>
                    <th class="px-6 py-4 text-sm font-yekan-bold text-slate-ui text-center cursor-pointer hover:bg-slate-105 transition-colors" data-sort="collection_id">
                        ID مجموعه <span class="sort-icon">⇅</span>
                    </th>
                    <th class="px-6 py-4 text-sm font-yekan-bold text-slate-ui text-center cursor-pointer hover:bg-slate-105 transition-colors" data-sort="brand_name">
                        نام برند <span class="sort-icon">⇅</span>
                    </th>
                    <th class="px-6 py-4 text-sm font-yekan-bold text-slate-ui text-center cursor-pointer hover:bg-slate-105 transition-colors" data-sort="games">
                        بازی‌ها <span class="sort-icon">⇅</span>
                    </th>
                    <th class="px-6 py-4 text-sm font-yekan-bold text-slate-ui text-center cursor-pointer hover:bg-slate-105 transition-colors" data-sort="balance">
                        آخرین موجودی <span class="sort-icon">⇅</span>
                    </th>
                    <th class="px-6 py-4 text-sm font-yekan-bold text-slate-ui text-center cursor-pointer hover:bg-slate-105 transition-colors" data-sort="last_change_date">
        ...
        function renderCollectionsTable() {
            ...
            if (collectionsOwnersData.length === 0) {
                ...
                pageData.forEach((item, index) => {
                    ...
                    html += `
                        <tr class="border-b border-slate-105 hover:bg-surface-muted transition-colors" style="background-color: ${index % 2 === 0 ? '#FFFFFF' : '#F8FAFC'};">
                            <td class="px-6 py-4 text-sm font-yekan-bold text-slate-800 text-center">${rowNumber}</td>
                            <td class="px-6 py-4 text-sm font-yekan-bold text-slate-800 text-center">${escapeHtml(item.full_name || '---')}</td>
                            <td class="px-6 py-4 text-sm font-yekan-bold text-slate-800 text-center">${escapeHtml(item.phone)}</td>
                            <td class="px-6 py-4 text-sm font-yekan-bold text-slate-800 text-center">${item.collection_id}</td>
                            <td class="px-6 py-4 text-sm font-yekan-bold text-slate-800 text-center">${item.brand_name || '---'}</td>
                            <td class="px-6 py-4 text-sm font-yekan-bold text-slate-800 text-center" style="max-width: 300px;">${item.games || '---'}</td>
                            ...
                            <td class="px-6 py-4 text-sm font-yekan-bold text-slate-800 text-center">${lastChangeDate}</td>
                        </tr>
                    `;
                });
                ...
                if (totalPages > 1) {
                    html += `<tr><td colspan="8" class="px-6 py-4">
                        <div class="flex justify-center items-center gap-2">
                            ${currentPage > 1 ? `<button onclick="goToPage(${currentPage - 1})" class="px-3 py-1 border border-slate-110 rounded-lg text-sm font-yekan-bold text-slate-ui hover:bg-surface-sunken transition-colors">قبلی</button>` : ''}
                            ${Array.from({length: totalPages}, (_, i) => i + 1).map(page => {
                                if (page === currentPage) {
                                    return `<span class="px-3 py-1 bg-primary-500 text-white rounded-lg text-sm font-yekan-bold">${page}</span>`;
                                } else if (page === 1 || page === totalPages || (page >= currentPage - 2 && page <= currentPage + 2)) {
                                    return `<button onclick="goToPage(${page})" class="px-3 py-1 border border-slate-110 rounded-lg text-sm font-yekan-bold text-slate-ui hover:bg-surface-sunken transition-colors">${page}</button>`;
                                } else if (page === currentPage - 3 || page === currentPage + 3) {
                                    return `<span class="px-3 py-1 text-slate-ui">...</span>`;
                                }
                                return '';
                            }).join('')}
                            ${currentPage < totalPages ? `<button onclick="goToPage(${currentPage + 1})" class="px-3 py-1 border border-slate-110 rounded-lg text-sm font-yekan-bold text-slate-ui hover:bg-surface-sunken transition-colors">بعدی</button>` : ''}
                        </div>
                    </td></tr>`;
                }
            }
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    $("#data-list").html(function() {
                        let out = '<div class="w-full bg-surface-sunken rounded-t-xl"><div class="grid grid-cols-[60px_180px_100px_120px_100px_120px_120px_120px_120px] gap-4 px-6 py-4 text-sm font-yekan-bold text-slate-ui"><div class="text-center">ردیف</div><div class="text-center">شماره تراکنش</div><div class="text-center">زمان درخواست</div><div class="text-center">اضافه/کسر</div><div class="text-center">مبلغ</div><div class="text-center">موجودی قبلی</div><div class="text-center">موجودی فعلی</div><div class="text-center">بابت</div><div class="text-center">وضعیت</div></div></div>';
                        for (let i = 0; i < 15; i++) {
