...
<div class="space-y-4 text-right text-sm font-yekan-bold text-navyBlue">
    ...
    <div>
        ...
        <div class="bg-gray-50 border border-slate-105 rounded-lg p-3 whitespace-pre-wrap"><?php echo esc_html( $reason !== '' ? $reason : '—' ); ?></div>
    </div>
    <div>
        <p class="font-bold text-gray-800 mb-1">جزئیات عملیات</p>
        <pre class="bg-gray-50 border border-slate-105 rounded-lg p-3 text-xs font-normal whitespace-pre-wrap break-words max-w-full max-h-80 overflow-y-auto overflow-x-hidden" style="text-wrap: auto;"><?php echo esc_html( $details !== '' ? $details : '—' ); ?></pre>
    </div>
</div>
