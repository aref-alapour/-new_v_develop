...
/**
 * هم‌سو با ez_markting_row_eligible_for_booking_recovery در
 * inc/shop/marketing/booking-recovery.php — منطق را اینجا تکرار می‌کنیم چون
 * این اسکریپت مستقیم از frontend پنل صدا زده می‌شود و اطمینان نداریم
 * تابع اصلی در همه‌ی paths لود شده باشد.
 */
if ( ! function_exists( 'ez_orders_get2_row_eligible_recover_booking_sans' ) ) {
    function ez_orders_get2_row_eligible_recover_booking_sans( array $row ) {
        $st = isset( $row['order_status'] ) ? (string) $row['order_status'] : '';
        if ( strpos( $st, 'wc-' ) === 0 ) {
            $st = substr( $st, 3 );
        }
        if ( in_array( $st, ['cancelled', 'refunded', 'conflict', 'failed', 'trash', 'on-hold', 'draft'], true ) ) {
            return false;
        }
        ...
        if ( in_array( $st, ['partially-paid', 'completed-paid'], true ) && $no_sans ) {
            return true;
        }
...
<section class="justify-center items-center mt-7 mx-auto">
    <div class="w-full py-4 rounded-t-2.5xl bg-slate-105">
    ...
    <div class="w-full h-full rounded-t-2.5xlb" id="ordersTable">
        ...
        foreach ($orders as $order) {
            ...
            <div class="orders_table_row grid <?= (++$row_index % 2 == 0) ? 'bg-gray-100/50' : ''; ?> text-center px-4 py-2.5" data-id="<?php echo $order_id ?>"
                ...
                <p class="text-base content-center text-text-3 col-order_id"><?php echo isset($order_id) ? $order_id : '---'; ?></p>
                ...
                <div class="mainContent transition-all duration-300 relative flex gap-2 justify-center">
                    ...
                    if (array_intersect(['administrator', 'supervisor', 'accounting'], $current_user_roles)) : 
                        ...
                        <?php if ( ez_orders_get2_row_eligible_recover_booking_sans( $order ) ) : ?>
                            <button type="button"
                                title="بررسی سانس: همگام‌سازی بوکینگ/مارکتینگ یا ثبت تداخل و عودت"
                                class="recover-booking-sans-btn absolute left-[-100%] top-1/2 z-10 h-7 -translate-y-1/2 cursor-pointer whitespace-nowrap rounded-md px-1.5 py-1 text-[10px] font-bold leading-tight text-white bg-teal-600 hover:opacity-85 transition"
...
<section class="flex justify-center items-center fixed inset-0 modal-bg z-50 transition-opacity" id="crmModal" data-id="" style="display: none;">
    <div class="rounded-xl bg-white border border-rail shadow-rail-lip top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 absolute p-5 w-d442">
        <div class="mb-4">
            <div class="flex justify-between items-center cursor-pointer" id="teammatesToggle">
                ...
                <svg class="transition-transform duration-300" id="teammatesArrow" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
        ...
        <div id="ownerCancellationBox" class="mt-4 p-4 bg-white hidden">
            ...
            <hr class="mb-4 bg-gray-200 h-0.5">
...
<section class="flex justify-center items-center fixed inset-0 modal--center items-center fixed inset-0 modal-bg z-50 transition-opacity" id="maliModal" data-id="" style="display: none;">
    <div class="rounded-xl bg-white border border-rail shadow-rail-lip top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 absolute p-5 w-d450">
        ...
        <div class="grid grid-cols-4 gap-x-2">
            ...
            <button class="order_status_change px-2 py-1.5 rounded-lg text-white cursor-pointer bg-slate-900 text-sm font-bold w-full" data-action="walletx">کیف پول</button>
            <button class="order_status_change px-2 py-1.5 rounded-lg text-white cursor-pointer bg-primary-500 text-sm font-bold w-full" data-action="refunded">مسترد</button>
            <button class="order_status_change px-2 py-1.5 rounded-lg text-white cursor-pointer bg-danger-hot text-sm font-bold w-full" data-action="admin-cancelled">لغو ادمین</button>
        </div>

        <!-- ردیف دوم: همان کسانی که آیکن مالی را می‌بینند (ادمین، سوپروایزر، حسابداری) -->
        <div class="grid grid-cols-2 gap-x-2 mt-2">
                <button id="convert_with_amendment" class="px-2 py-1.5 rounded-lg text-white cursor-pointer bg-secondary-600 text-sm font-bold w-full" style="display: none;">
                    تبدیل به پیش‌پرداخت (اصلاحیه)
                </button>
                <button id="openEditOrderForm" class="px-2 py-1.5 rounded-lg text-white cursor-pointer bg-blue text-sm font-bold w-full" style="display: none;">
                    ویرایش سفارش
                </button>
        </div>
        ...
        <div id="editOrderBox" class="mt-4 p-4 bg-white border border-gray-200 rounded-lg hidden">
                ...
                <hr class="mb-4 bg-gray-200 h-d1 border-0">
                ...
                <div class="flex justify-end gap-2">
                    <button id="submitEditOrder" class="bg-primary-500 text-white px-4 py-2 rounded-md text-sm font-bold h-12.5">
                        ثبت تغییرات
                    </button>
