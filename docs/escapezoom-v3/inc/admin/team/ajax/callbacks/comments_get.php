...
<div class="bg-gray-100 font-bold text-muted-blue text-sm font-yekan-bold rounded-t-xl px-4 py-3 border-b border-gray-200 mt-6 grid gap-x-8"
...
foreach ($comments as $comment) :
    ...
    <div class="border-b border-slate-105">
    <div class="grid gap-x-8 gap-y-0 px-4 py-4 pb-3 border-b-edge" style="<?php echo esc_attr( $row_style ); ?>">
        ...
        <button type="button" class="openModalLevel cursor-pointer flex gap-x-1" data-comment-id="<?= (int) $comment_id ?>" data-acting="<?= (int) ( $comment_ratings['acting'] ?? 0 ) ?>" data-puzzle="<?= (int) ( $comment_ratings['puzzle'] ?? 0 ) ?>" data-atmosphere="<?= (int) ( $comment_ratings['atmosphere'] ?? 0 ) ?>" data-creativity="<?= (int) ( $comment_ratings['creativity'] ?? 0 ) ?>" data-staff="<?= (int) ( $comment_ratings['staff'] ?? 0 ) ?>">
            ...
            <svg class="w-4 h-4 text-yellow-400 mr-1 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="leading-relaxed space-y-1 text-sm font-bold text-slate-250"><?php echo esc_html( $comment['comment_content'] ); ?></div>
    ...
    <div class="col-span-full mt-4 mb-5 mr-5 sm:mr-10 justify-self-start min-w-0">
        <div class="text-11 font-bold text-muted-blue mb-1.5 font-yekan-bold">پاسخ مجموعه</div>
        <div
            class="rounded-xl border border-slate-105 bg-white p-3 shadow-comment-card border-r-[3px] border-r-primary-2"
            role="region"
        ...
        >
            <div class="text-xs font-bold text-navyBlue font-yekan-bold mb-1.5 flex flex-wrap items-center gap-x-2 gap-y-1">
                <span class="inline-flex items-center gap-1 rounded-md bg-primary-100 px-2 py-0.5 text-11 text-primary-900">پاسخ</span>
                <span><?php echo esc_html( $reply_author_title ); ?></span>
                <span class="text-muted-blue font-normal" aria-hidden="true">·</span>
                <span class="flex gap-2 text-slate-ui"><span><?php echo esc_html( $reply_date ); ?></span><span><?php echo esc_html( $reply_time ); ?></span></span>
            </div>
            <div class="text-sm leading-relaxed font-normal text-slate-250 border-t border-dashed border-edge pt-2 mt-0.5"><?php echo esc_html( $reply_row['comment_content'] ); ?></div>
        </div>
    </div>
        <?php
    }
