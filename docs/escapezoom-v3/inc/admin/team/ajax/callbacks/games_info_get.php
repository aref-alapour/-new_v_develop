<section id="gamesSection" class="mt-7">
    <div class="w-full px-d34 py-4 rounded-t-2.5xl bg-slate-105 grid gap-4">
    ...
    if ($the_query->have_posts()) :
        while ($the_query->have_posts()) : $the_query->the_post();
            ...
            $brand_term_id    = ( ! empty( $product_brands ) && ! is_wp_error( $product_brands ) )
                ? (int) $product_brands[0]->term_id
                : 0;
            ...
            <div class="res_row grid text-sm font-yekan-bold text-grayy text-center px-d34 items-center" ${backgroundStyle}
                 style="grid-template-columns: 1fr 1.2fr 1.2fr 1fr 1.1fr 1fr 1fr 1fr 1fr 1fr">
                ...
                <p class="text-base font-yekan-bold text-navyBlue">
                    <?php
                    if ( ! empty( $product_brands ) && ! is_wp_error( $product_brands ) ) {
                        $brand_term = $product_brands[0];
                        $brand_url  = get_term_link( $brand_term );
                        if ( ! is_wp_error( $brand_url ) ) {
                            echo '<a target="_blank" rel="noopener noreferrer" href="' . esc_url( $brand_url ) . '" class="hover:underline">' . esc_html( $brand_term->name ) . '</a>';
                        } else {
                ...
                <div class="flex justify-center">
                    <a target="_blank" href="<?php the_permalink() ?>" class="w-6 h-6 bg-focus-blue rounded flex items-center justify-center">
                ...
                <div class="flex justify-center">
                    ...
                    ?>
                    <button onclick="openShortlinkModal('<?php echo esc_js($shortlink); ?>')" class="w-6 h-6 bg-health-green rounded flex items-center justify-center cursor-pointer hover:bg-health-green transition-colors" title="مشاهده لینک کوتاه">
                ...
                <?php
                if (array_intersect(['administrator', 'supervisor', 'poshtiban'], wp_get_current_user()->roles)) : ?>
                    <button onclick="openEditModal(<?php echo get_the_ID(); ?>, '<?php echo (int) $brand_term_id; ?>')" class="w-6 h-6 bg-primary-2 rounded flex items-center justify-center">
...
<div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50" style="display: none;">
    <div class="bg-white rounded-2xl p-4 shadow-2xl max-w-md w-full mx-4 relative">
        <div class="flex gap-4">
            <button id="editGameBtn" class="flex-1 bg-focus-blue text-white px-6 py-3 rounded-lg font-yekan-bold text-sm hover:bg-blue transition-colors">
                ویرایش بازی
            </button>
            <button id="editBrandBtn" class="flex-1 bg-primary-2 text-white px-6 py-3 rounded-lg font-yekan-bold text-sm hover:bg-primary-600 transition-colors">
                ویرایش برند
            </button>
        </div>
    ...
    <div class="bg-white rounded-2xl p-6 shadow-2xl max-w-md w-full mx-4 relative">
        ...
        <div class="mb-4">
            ...
            <div id="shortlinkContent" class="flex items-center gap-2">
                <a id="shortlinkLink" href="" target="_blank" rel="noopener noreferrer" class="text-sm font-yekan-bold text-focus-blue hover:underline break-all"></a>
            </div>
        </div>
        <div class="flex gap-3">
            <button id="copyShortlinkBtn" class="flex-1 bg-health-green text-white px-6 py-3 rounded-lg font-yekan-bold text-sm hover:bg-health-green transition-colors flex items-center justify-center gap-2">
...
<script>
    ...
    document.getElementById('editBrandBtn').addEventListener('click', function() {
        if (currentBrandId && currentBrandId !== '0') {
            window.open(`https://escapezoom.ir/wp-admin/term.php?taxonomy=product_brand&tag_ID=${currentBrandId}&post_type=product`, '_blank');
            closeEditModal();
        } else {
    ...
    document.getElementById('copyShortlinkBtn').addEventListener('click', function() {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(fullUrl).then(function() {
                ...
                btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg> کپی شد!';
                btn.classList.remove('bg-health-green', 'hover:bg-health-green');
                btn.classList.add('bg-wp-green');
                
                setTimeout(function() {
                    btn.innerHTML = originalText;
                    btn.classList.remove('bg-wp-green');
                    btn.classList.add('bg-health-green', 'hover:bg-health-green');
                }, 2000);
            }).catch(function(err) {
