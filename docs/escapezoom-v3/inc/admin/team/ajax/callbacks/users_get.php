...
// Function to get user level with colors (delegates to theme helper for «مجموعه دار» + points levels).
...
foreach ($users as $user) {
    ...
    $html .= '<tr class="border-b border-slate-105 hover:bg-gray-50">';
    ...
    $html .= '<a href="' . home_url('/profile/' . $user_id) . '" class="text-sm text-white bg-primary-500 hover:bg-primary-600 transition-all duration-300 rounded-lg px-3 py-1 shadow-wp-button" target="_blank">';
    $html .= 'ID: ' . $user_id;
    $html .= '</a>';
    $html .= '</td>';
    ...
    if ($can_manage_banking) {
        if (!empty($user_iban)) {
            $html .= '<button class="view-banking-btn text-blue hover:text-slate-800 transition-colors duration-200" data-user-id="' . $user_id . '" title="مشاهده اطلاعات بانکی">';
            ...
            $html .= '<button class="add-banking-btn text-primary-2 hover:text-primary-deep transition-colors duration-200" data-user-id="' . $user_id . '" title="افزودن اطلاعات بانکی">';
            $html .= '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">';
    ...
    // Edit button
    if ($can_edit) {
        $html .= '<button class="w-12 h-6 flex items-center justify-center edit-user-btn text-blue text-sm font-yekan-bold" data-user-id="' . $user_id . '" data-user-phone="' . esc_attr($user_phone) . '" data-user-first-name="' . esc_attr($user_first_name) . '" data-user-last-name="' . esc_attr($user_last_name) . '" data-user-role="' . esc_attr($user_role) . '">ویرایش</button>';
    } else {
    ...
    if ($can_create_password) {
        $html .= '<button class="w-12 h-6 flex items-center justify-center create-password-btn text-primary-2 text-sm font-yekan-bold" data-user-id="' . $user_id . '" data-user-phone="' . esc_attr($user_phone) . '">رمز ثابت</button>';
    } else {
...
if ($total_pages > 1) {
    ...
    for ($i = $start_page; $i <= $end_page; $i++) {
        $active_class = ($i == $page) ? 'bg-primary-2 text-white' : 'text-gray-700 bg-white border border-gray-300 hover:bg-gray-50';
