...
<?php
    ...
    <div class="w-full">
        ...
        <div class="w-full bg-surface-sunken rounded-t-xl">
            <div class="grid grid-cols-[60px_180px_100px_120px_100px_120px_120px_120px_120px] gap-4 px-6 py-4 text-sm font-yekan-bold text-slate-ui">
        ...
        foreach ($transactions as $key => $trans) :
            ...
            <div class="data-row w-full" style="background-color: <?php echo $key % 2 === 0 ? '#FFFFFF' : '#F8FAFC'; ?>;">
                <div class="grid grid-cols-[60px_180px_100px_120px_100px_120px_120px_120px_120px] gap-4 px-6 py-4 text-sm font-yekan-bold">
                    <div class="text-center text-slate-800 flex items-center justify-center">
                        <?php echo $row_number ?>
                    </div>
                    <div class="text-center text-slate-800 flex items-center justify-center">
                        <?php echo $trans->ID ?>
                    ...
                    <div class="text-center flex items-center justify-center">
                        <span class="inline-flex items-center justify-center w-6 h-6 rounded text-white text-xs font-yekan-heavy <?php echo $trans->amount > 0 ? 'bg-health-green' : 'bg-error' ?>" style="border-radius: 4px;">
                    ...
                    <div class="text-center flex items-center justify-center">
                        ...
                        if ($status_text == 'در حال پردازش') {
                            ...
                            $status_class = 'text-health-green';
                        } elseif ($status_text == 'لغو شد') {
                            $status_class = 'text-error';
                        } else {
                            $status_class = 'text-slate-ui';
                        }
                        ?>
        ...
        if ($total_pages > 1) { ?>
            <div class="mb-9 flex w-full items-center justify-center gap-4">
                <div class="flex gap-4 max-lg:gap-2 mt-16 justify-start max-lg:justify-center pagination">
                    ...
                    <?php if ($page_num > 1) { ?>
                        <a href="javascript:void(0)" data-page="<?php echo $page_num - 1; ?>" class="pagination-link flex items-center justify-center w-8 h-8 rounded-lg border border-slate-110 text-slate-ui hover:bg-surface-sunken transition-colors">
                    ...
                    for ($i = $start; $i <= $end; $i++) { ?>
                        <?php if ($i == $page_num) { ?>
                            <span class="flex items-center justify-center w-8 h-8 rounded-lg bg-primary-500 text-white text-sm font-yekan-bold"><?php echo $i; ?></span>
                        <?php } else { ?>
                            <a href="javascript:void(0)" data-page="<?php echo $i; ?>" class="pagination-link flex items-center justify-center w-8 h-8 rounded-lg border border-slate-110 text-slate-ui hover:bg-surface-sunken transition-colors text-sm font-yekan-bold"><?php echo $i; ?></a>
                        <?php } ?>
                    <?php } ?>
