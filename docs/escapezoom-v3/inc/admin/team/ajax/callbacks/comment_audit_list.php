...
$sortable_th = static function ( $key, $label ) use ( $sort_by, $sort_dir ) {
    ...
    return sprintf(
        '<button type="button" class="audit-sort-link inline-flex items-center gap-1 hover:text-slate-260" data-sort-by="%1$s" data-sort-dir="%2$s">%3$s <span class="text-11">%4$s</span></button>',
        esc_attr( $key ),
        esc_attr( $next_dir ),
        esc_html( $label ),
        esc_html( $arrow )
...
<div class="overflow-x-auto border border-slate-105 rounded-xl mt-4">
    <table class="min-w-full text-sm font-yekan-bold">
        <thead class="bg-gray-100 text-muted-blue">
            <tr>
        ...
        <tbody>
            ...
            <?php else : ?>
                <?php foreach ( $rows as $idx => $row ) : ?>
                    ...
                    ?>
                    <tr class="border-t border-edge hover:bg-gray-50/80">
                        ...
                        <td class="px-3 py-2 max-w-d180 truncate" title="<?php echo esc_attr( $row['product_title'] ); ?>"><?php echo esc_html( $row['product_title'] ); ?></td>
                        ...
                        <td class="px-3 py-2 max-w-d200 truncate" title="<?php echo esc_attr( $row['reason'] ); ?>"><?php echo esc_html( wp_trim_words( (string) $row['reason'], 12 ) ); ?></td>
                        <td class="px-3 py-2 align-top whitespace-nowrap">
                            <button type="button" class="audit-detail-btn text-xs text-primary-2 hover:underline cursor-pointer font-yekan-bold" data-audit-id="<?php echo (int) $row['id']; ?>">
                                مشاهده
                            </button>
                        </td>
                    </tr>
...
<?php if ( $total_pages > 1 ) : ?>
    <div class="flex flex-wrap justify-center gap-2 mt-6 mb-4 comment-audit-pagination">
        ...
        if ( $page > 1 ) {
            echo '<button type="button" class="audit-page-link px-3 py-1 rounded border border-slate-105 hover:bg-gray-50 text-sm" data-page="' . (int) ( $page - 1 ) . '">قبلی</button>';
        }
        for ( $i = $start; $i <= $end; $i++ ) {
            $cls = ( $i === $page ) ? 'bg-primary-2 text-white border-primary-2' : 'border-slate-105 hover:bg-gray-50';
            echo '<button type="button" class="audit-page-link px-3 py-1 rounded border text-sm ' . esc_attr( $cls ) . '" data-page="' . (int) $i . '">' . (int) $i . '</button>';
        }
        if ( $page < $total_pages ) {
            echo '<button type="button" class="audit-page-link px-3 py-1 rounded border border-slate-105 hover:bg-gray-50 text-sm" data-page="' . (int) ( $page + 1 ) . '">بعدی</button>';
        }
        ?>
    </div>
<?php endif; ?>
