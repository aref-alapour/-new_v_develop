...
function search_orders_by_order_id($order_id = '', $page = 1, $limit = 25)
{
    ...
    $order_id = '%' . trim(esc_sql($wpdb->esc_like($order_id))) . '%';
...
function search_orders_by_user_name($first_name = '', $page = 1, $limit = 25)
{
    ...
    $first_name = '%' . trim(esc_sql($wpdb->esc_like($first_name))) . '%';
...
function search_orders_by_user_phone($user_phone = '', $page = 1, $limit = 25)
{
    ...
    $user_phone = '%' . trim(esc_sql($wpdb->esc_like($user_phone))) . '%';
...
function search_orders_by_product_title($product_title = '', $page = 1, $limit = 25)
{
    ...
    $product_title = '%' . trim(esc_sql($wpdb->esc_like($product_title))) . '%';
    ...
    if (!isset($tab) or empty($tab) or $tab == 'orders') : // همه سفارشات
        ...
        <form method="get" action="">
                    ...
                    if ($the_query->have_posts()) :
                        while ($the_query->have_posts()) : $the_query->the_post();
                            ...
                            <tr order-id="<?php echo $order_id ?>">
                                ...
                                <td><a href="<?php echo esc_url( home_url( '/supporting?user_phone=' . rawurlencode( $order->get_billing_phone() ) ) ); ?>" title="جستجوی <?php echo esc_attr( $order->get_billing_phone() ); ?>"><?php echo esc_html( $order->get_billing_phone() ); ?></a></td>
                                ...
                                <td style="color: #00a500;font-weight: bold;font-size: 13px;text-wrap: nowrap;"><?php echo get_post_meta($order_id, '_transaction_id', true) ?: '--------' ?></td>
                                <td>
                                    <?php
                                    if ($role != 'accounting') : ?>
                                        <input type="checkbox" class="happycall" style="appearance: auto;cursor: pointer" value="1" <?php echo get_post_meta($order_id, "supporting_happycall", true) == 1 ? 'checked' : ''; ?>>
                                    <?php
                                    else: ?>
                                ...
                                <td>
                                    ...
                                    if ($role != 'poshtiban') : ?>
                                        ...
                                        if ($role == 'administrator' || $role == 'accounting' || ($role == 'shopist' && $sans_time)) { ?>
            ...
            <div id="page_num">
                ...
                <span style="float: left;display: inline-block;width: 100%;font-size: 20px;font-weight: bold;color: #9b0000;">تعداد بلیت های فروخته شده: <?php echo $quantity_all ?></span>
