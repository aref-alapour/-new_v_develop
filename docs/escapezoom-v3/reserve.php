...
$brand   = get_the_terms($id, 'product_brand')[0];
...
$axes_avg = ez_product_rating_resolve_axis_averages_for_display($id);
$decor    = $axes_avg[1094];
...
$raw_rate = ez_product_rating_overall_from_axes( $axes_avg, $type );
...
<div class="container mx-auto max-lg:pt-1 pt-20 mb-10 max-lg:mt-8">
    ...
    <div class="mb-8 flex items-center justify-between max-lg:hidden">
        <div class="flex items-center gap-5">
            ...
            <div class="flex flex-col gap-3">
                <p class="text-caption-muted text-16 space-x-1">
                ...
                <div class="flex items-center gap-x-3">
                    <span class="inline-flex h-5 w-9 content-center rounded bg-yellow-500 leading-none justify-center items-center">
                        <?= $rate_final ?>
                    </span>
        ...
        <div class="flex flex-col items-end gap-5">
            ...
            <button class="share" data-title="<?php echo $data['product']['title']; ?>" data-content="میانگین امتیاز این بازی از <?php echo $data['product']['comments']; ?> رای" data-url="<?php echo site_url('/r/' . $id); ?>">
                <span class="inline-flex items-center gap-4 text-sm font-bold text-slate-700">اشتراک با
                    دوستان
            ...
            <div class="rounded-xl bg-slate-100 w-full px-2 text-center">
    ...
    <div class="mb-8 flex gap-x-4 items-stretch lg:hidden">
        <div class="grow flex flex-col relative">
            <p class="text-caption-muted space-x-1">
                در حال تهیه بلیت <span><?php echo $type ?></span>
            </p>
            ...
            <div class="flex justify-between">
                <div class="text-caption-muted text-14">
                    <?php echo $data['brand']['hood'] ?>
                    .
                    <?php echo $data['brand']['city'] ?>
                </div>
                <div class="flex gap-x-2">
                    <span class="px-2 font-bold text-center content-center rounded-xl bg-yellow-500 leading-none text-16">
                        <?= $rate_final ?>
                    </span>
                    <span class="space-x-1 font-bold space-x-reverse text-14">
                        <span><?php echo $data['product']['comments']; ?></span>
                        <span class="text-text-3">رأی</span>
                    </span>
                </div>
            </div>
    ...
    <div class="mb-7 flex items-center justify-between gap-4 font-bold">
        <button type="button" data-timestamp="<?php echo esc_attr($lastMonth); ?>" class="max-lg:hidden last-month flex w-2/12 items-center justify-between gap-2 rounded-lg border-l border-r border-t border-l-slate-105 border-r-slate-105 border-t-slate-105 px-6 py-1 text-slate-700 shadow-98 disabled:opacity-50 max-lg:text-16" disabled>
        ...
        <button type="button" data-timestamp="<?php echo esc_attr($lastWeek); ?>" class="last-week flex w-2/12 max-lg:grow max-lg:leading-4 items-center justify-between gap-2 rounded-lg border-l border-r border-t border-l-slate-105 border-r-slate-105 border-t-slate-105 px-6 py-1 text-slate-700 shadow-98 disabled:opacity-50 max-lg:text-16" disabled>
            <svg xmlns="http://www.w3.org/2000/svg" width="7" height="12" viewBox="0 0 7 12" fill="none" class="m-0">
                <path d="M2 2L4.55 5.4C4.81667 5.75556 4.81667 6.24444 4.55 6.6L2 10" stroke="currentColor" stroke-width="3" stroke-linecap="round" />
            </svg>
            هفته پیش
        ...
        <div type="button" class="flex min-h-d41 w-4/12 items-center justify-center gap-x-4 lg:gap-x-20 rounded-lg border-l border-r border-t border-l-slate-105 border-r-slate-105 border-t-slate-105 px-6 py-1 text-slate-700 shadow-98 max-lg:flex-col max-lg:leading-none max-lg:gap-1 max-lg:text-16">
        ...
        <button type="button" data-timestamp="<?php echo esc_attr($nextWeek); ?>" class="next-week flex w-2/12 max-lg:grow max-lg:leading-4 items-center justify-between gap-2 rounded-lg border-l border-r border-t border-l-slate-105 border-r-slate-105 border-t-slate-105 px-6 py-1 text-slate-700 shadow-98 max-lg:text-16">
            هـفته بعد
            <svg xmlns="http://www.w3.org/2000/svg" width="7" height="12" viewBox="0 0 7 12" fill="none" class="m-0">
        ...
        <button type="button" data-timestamp="<?php echo esc_attr($nextMonth); ?>" class="max-lg:hidden next-month flex w-2/12 max-lg:w-4 items-center justify-between gap-2 rounded-lg border-l border-r border-t border-l-slate-105 border-r-slate-105 border-t-slate-105 px-6 py-1 text-slate-700 shadow-98 max-lg:text-16">
            مــاه بعد
            <svg xmlns="http://www.w3.org/2000/svg" width="7" height="12" viewBox="0 0 7 12" fill="none" class="m-0">
                <path d="M5 2L2.45 5.4C2.18333 5.75556 2.18333 6.24444 2.45 6.6L5 10" stroke="currentColor" stroke-width="3" stroke-linecap="round" />
            </svg>
        ...
        $("body")
            .on('click', '[data-timestamp]', function() {
                ...
                $(".selected-month").text(function() {
                    return persianDate.unix($(".selected-month").attr('data-calendar')).format("MMMM")
                })

                $(".selected-year").text(function() {
                    return persianDate.unix($(".selected-year").attr('data-calendar')).format("YYYY")
                })
            })
            ...
            .on('click', '[data-item-timestamp]', function() {
                ...
                let time = persianDate.unix(timestamp)
