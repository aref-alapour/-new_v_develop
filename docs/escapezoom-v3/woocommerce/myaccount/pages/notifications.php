...
$query = $wpdb->get_results( "SELECT * FROM notifications ORDER BY `id` DESC" );
...
<script>
    jQuery(document).ready(function ($) {
        $('body').on('click', '.notification-card', function () {
			...
			<?php if ( wp_is_mobile() ) { ?>
            $(".notification-card").find('.notification-excerpt').addClass('line-clamp-1')
            _.find('.notification-excerpt').removeClass('line-clamp-1')
			<?php }?>

            _.addClass("before:content-none").removeClass("before:content-['']")

            $(".notification-card").removeAttr('style')

            _.css({
                borderColor: _.attr('data-color'),
                boxShadow: `0px 1.5px 0 0 ${_.attr('data-color')}`
            })
