...
<div class="lg:col-span-8 2xl:col-span-9">
	<section class="rounded-2xl border border-slate-120 px-8 shadow-12 max-lg:mb-0 max-lg:rounded-none max-lg:px-0 max-lg:shadow-none py-12 max-lg:border-0 max-lg:py-0 h-auto">
		...
		<h2 class="text-base font-bold md:text-lg mb-d33">
		...
		<?php else : ?>
			<div class="flex flex-col gap-6">
				<?php foreach ( $comments as $comment ) : ?>
					...
					?>
					<div class="rounded-14 border border-rail bg-gray-20 p-5 max-lg:p-4">
						<div class="flex gap-4 items-center justify-between">
							...
							<?php else : ?>
								<div class="shrink-0 w-full sm:w-28 aspect-square rounded-xl border border-dashed border-rail bg-slate-100 flex items-center justify-center text-xs text-text-3">
									بدون تصویر
								</div>
							<?php endif; ?>
							<div class="flex-1">
							...
							<span class="text-xs text-text-3"><?php echo esc_html( $status_label ); ?></span>
						</div>
						<div class="flex justify-end gap-3">
							<button type="button" class="my-reviews-open-detail text-sm font-bold text-blue hover:underline cursor-pointer bg-transparent border-0 p-0" data-review-json="<?php echo esc_attr( $payload_json ); ?>">
								مشاهده جزئیات
							</button>
							<?php if ( $can_edit && $post ) : ?>
								<button type="button" class="my-reviews-open-edit text-sm font-bold text-accent-950 hover:underline cursor-pointer bg-transparent border-0 p-0" data-review-json="<?php echo esc_attr( $payload_json ); ?>">
									ویرایش کامنت
								</button>
							<?php endif; ?>
						</div>
			...
			<?php if ( $total_pages > 1 ) : ?>
				<nav class="flex flex-wrap justify-center items-center gap-2 mt-8" aria-label="صفحه‌بندی کامنت‌ها">
					...
					?>
					<?php if ( $page > 1 ) : ?>
						<a href="<?php echo $url_prev; ?>" class="px-3 py-1.5 rounded-lg border border-rail text-sm font-bold text-slate-700 hover:bg-white">قبلی</a>
					<?php endif; ?>
					<?php
					$start = max( 1, $page - 2 );
					...
					for ( $i = $start; $i <= $end; $i++ ) {
						if ( (int) $i === (int) $page ) {
							echo '<span class="px-3 py-1.5 rounded-lg bg-focus-blue text-white text-sm font-bold">' . (int) $i . '</span>';
						} else {
							echo '<a href="' . esc_url( add_query_arg( 'mr_page', $i, $base_url ) ) . '" class="px-3 py-1.5 rounded-lg border border-rail text-sm font-bold text-slate-700 hover:bg-white">' . (int) $i . '</a>';
						}
					}
					?>
					<?php if ( $page < $total_pages ) : ?>
						<a href="<?php echo $url_next; ?>" class="px-3 py-1.5 rounded-lg border border-rail text-sm font-bold text-slate-700 hover:bg-white">بعدی</a>
					<?php endif; ?>
				</nav>
			<?php endif; ?>
		<?php endif; ?>
		...
		<div id="my-reviews-detail-overlay" class="fixed inset-0 flex items-center justify-center p-4 bg-black/40" style="display: none; z-index: 100;" role="dialog" aria-modal="true" aria-labelledby="my-reviews-detail-title">
			<div class="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-modal overflow-hidden flex flex-col">
				<div class="flex items-center justify-between px-4 py-3 border-b border-slate-105">
					<h3 id="my-reviews-detail-title" class="text-lg font-bold text-slate-700"></h3>
				...
				<div class="p-4 overflow-y-auto text-right space-y-3">
					<p class="text-xs text-text-3" id="my-reviews-detail-status"></p>
		...
		<div id="my-reviews-edit-overlay" class="fixed inset-0 flex items-center justify-center p-4 bg-black/40" style="display: none; z-index: 100;" role="dialog" aria-modal="true">
			<div class="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-modal overflow-hidden flex flex-col">
				<div class="flex items-center justify-between px-4 py-3 border-b border-slate-105">
					<h3 class="text-lg font-bold text-slate-700">ویرایش نظر</h3>
					<button type="button" class="my-reviews-modal-close inline-flex items-center justify-center min-w-11 min-h-11 text-3xl font-light leading-none text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-xl transition-colors" aria-label="بستن">&times;</button>
				</div>
				<div class="p-4 overflow-y-auto">
					<p class="text-sm text-steel mb-4" id="my-reviews-edit-product-title"></p>
					<form class="send-comment my-reviews-edit-form" method="post">
						...
						<div class="mb-4">
							<textarea id="my-reviews-edit-content" name="content" rows="6" class="block w-full p-4 text-sm text-gray-900 border border-gray-200 outline-none rounded-xl placeholder:text-slate-300 focus:border-focus-blue focus:ring-2 focus:ring-focus-blue/20" required></textarea>
						</div>
						<div id="my-reviews-rating-root"></div>
						<button type="submit" class="my-reviews-submit w-full bg-focus-blue text-white py-3.5 rounded-xl font-bold text-base hover:bg-blue transition-colors">
							ذخیره تغییرات
						</button>
					</form>
				</div>
