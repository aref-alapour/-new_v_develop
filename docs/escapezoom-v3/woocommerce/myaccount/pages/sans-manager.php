...
<section class="border-edge lg:h-full lg:rounded-3xl lg:border lg:p-10">
    ...
    <?php if (empty($user_products) || empty($active_products)) { ?>
        ...
        <div class="h-d464 flex items-center justify-center text-3xl">
    ...
    <?php } else { ?>
        ...
        <div class="relative flex flex-col items-center justify-center">
                            ...
                            <?php foreach ($active_products as $user_product) {
                                ...
                                <div class="text-center swiper-slide"
                                    ...
                                    <?php echo wp_get_attachment_image(get_post_thumbnail_id($product->get_id()), 'large', false, [
                                        'class' => 'mx-auto mb-4 h-d45 w-d36 rounded-md object-cover lg:h-d93 lg:w-d74',
                                    ]); ?>
            ...
            <div class="bulk-actions flex gap-2 my-4 w-full">
                <!-- دکمه باز کردن -->
                <button type="button" 
                        class="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-md transition md:flex-none flex items-center justify-center" 
                        onclick="toggleAllSansesForDay(this, 'open_all_sanses')">
                    باز کردن همه سانس‌های این روز
                </button>

                <!-- دکمه بستن -->
                <button type="button" 
                        class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-md transition md:flex-none flex items-center justify-center" 
                        onclick="toggleAllSansesForDay(this, 'close_all_sanses')">
                    بستن همه سانس‌های این روز
                </button>
            </div>

            <!-- استایل اسپینر (اگر در فایل CSS ندارید اینجا قرار دهید) -->
            <style>
            .btn-spinner {
                width: 20px;
                height: 20px;
        ...
        <div id="userInfoModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 items-center justify-center">
            <div class="bg-white rounded-2xl p-6 w-88 max-w-lg mx-4 max-h-modal overflow-y-auto">
                <div id="userInfoContent" class="space-y-4">
                    <!-- User info will be populated here -->
                </div>
                <div class="w-full h-d1 bg-slate-105 my-4"></div>

                <!-- Regular cancellation button (shown when no cancellation request) -->
                <button id="cancelSansBtn" class="w-full flex justify-center items-center text-danger-hot gap-1 bg-slate-100 text-center rounded-sm">
                    لغوسانس
                ...
                <div id="cancellationRequestHandling" class="hidden">
                    <p class="text-sm font-bold text-body-soft text-center">
                        پلیر درخواست لغو این سانس را ثبت کرده است. آیا با لغو موافقت می‌کنید؟
                    </p>
                    <div class="flex gap-3 mt-4">
                        <button id="rejectCancellationBtn" class="flex-1 bg-slate-100 text-slate-200 py-2 px-4 rounded-10 font-bold text-center">
                            رد کردن
                        </button>
                        <button id="approveCancellationBtn" class="flex-1 bg-primary-500 hover:bg-primary-600 text-white py-2 px-4 rounded-10 font-bold text-center">
                            تایید و لغو سانس
                        </button>
                    </div>
                </div>
                ...
                <div id="cancellationReasonDropdown" class="mt-4 overflow-hidden transition-all duration-300 ease-in-out opacity-0" style="max-height: 0;">
                    <div class="p-4">
                        <form id="cancellationForm">
                            ...
                            <p class="text-sm font-bold text-text-3 mt-1">لطفاً یک گزینه را انتخاب کنید:</p>
                            ...
                            <div class="flex gap-2 mb-6">
                                ...
                                <p class="text-sm font-bold text-yellow-900">مجموعه‌دار عزیز، در صورت لغو کمتر از ۲۴ ساعت پیش از سانس، امتیاز و عملکرد برند شما تحت تأثیر قرار می‌گیرد.</p>
                            </div>

                            <button type="submit" class="bg-primary-500 rounded-lg h-d48 w-full shadow-primary-lip text-white">
                                ثبت درخواست
                            </button>
                        </form>
                    </div>
    ...
    jQuery(document).ready(function($) {
        ...
        $("body")
            .on('click', "[data-datepicker]", function() {
                ...
                $(this).removeClass('border-rail bg-white').addClass(
            ...
            .on('submit', ".comment_form", function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success && checkStatus === 'success') {
                            $.ajax({
                                type: "POST",
                                url: "<?php echo admin_url('admin-ajax.php') ?>",
                                data: {
                                ...
                                success: function(response) {
                                    ...
                                    if (response.success && (status === 'success' || status === 'partial_recovered')) {
                                        $(`#${sectionID}`).remove();
                                        Swal.fire({
                                            position: "bottom-start",
                                            icon: "success",
                                            text: status === 'partial_recovered' ? 'نظر با بازیابی جزئی با موفقیت ثبت شد.' : 'نظر شما با موفقیت ثبت شد.',
                                            showConfirmButton: false,
                                            timer: 2200
                                        });
                                        return;
                                    }
                                    if (response.success && status === 'already_completed') {
                                        $(`#${sectionID}`).remove();
                                        Swal.fire({
                                            position: "bottom-start",
                                            icon: "info",
                                            text: 'این نظر قبلا ثبت شده است.',
                                            showConfirmButton: false,
                                            timer: 2200
                                        });
                                        return;
                                    }
                                    if (response.success && status === 'in_progress') {
                                        Swal.fire({
                                            position: "bottom-start",
                                            icon: "info",
                                            text: 'درخواست قبلی در حال پردازش است. چند ثانیه دیگر دوباره تلاش کنید.',
                                            showConfirmButton: false,
                                            timer: 2400
                                        });
                                        return;
                                    ...
                                    Swal.fire({
                                        ...
                                        icon: "error",
                                        title: 'خطا',
                                        text: response?.data?.message || 'خطایی در ثبت دیدگاه رخ داد.',
                                        showConfirmButton: false,
                                        timer: 2400
                                    });
                                },
                        ...
                        } else if (response?.data?.status === 'outside_window') {
                            $(`#${sectionID}`).remove();
                            Swal.fire({
                                position: "bottom-start",
                                icon: "error",
                            ...
                            Swal.fire({
                                ...
                                icon: "error",
                                title: 'خطا',
                                text: response?.data?.message || 'خطایی رخ داده است.',
                                showConfirmButton: false,
                                timer: 2000
                            });
                        }
        ...
        function BuildComments() {
            ...
            $.ajax({
                ...
                data: {
                    action: "v2_ajax_handler",
                    nonce: "<?php echo wp_create_nonce('v2-ajax-nonce') ?>",
                    callback: "get_comments_order_list",
                    data: {
                                ...
                                success: function(response) {
                                    if (response.success &&
                                        ...
                                        response.data.forEach(function(
                                            ...
                                            let userLevel = parseInt(order.user_level, 10);
...
`;
        ...
        BuildComments();
        setInterval(BuildComments, 60000);
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center">
                        <div class="flex gap-3 items-center">
                            ...
                            <div class="rounded-24 h-5.5 px-2.5 leading-none content-center text-2xs text-center text-${orderData.level_color}" style="background-color: ${backgroundColor}">
                                ${orderData.level_text}
                            </div>
                        </div>
                        <a class="text-base font-extrabold text-steel" href="tel:${window.formatPhoneForTel(orderData.phone)}">${orderData.phone}</a>
                    </div>
                    ...
                    <div class="grid grid-cols-2 items-center gap-x-3.5">
                        <a href="${
                            ...
                            }" class="flex w-full text-sm gap-2 rounded-md bg-surface-pill py-1 px-4 h-8 items-center justify-center gap-2" target="_blank" rel="noopener" target="_blank">
                                <p class="text-sm font-bold text-muted-blue">مشاهده پروفایل</p>
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 19 19" fill="none">
                        ...
                        <a href="tel:${window.formatPhoneForTel(orderData.phone)}" class="flex gap-2 rounded-md bg-accent-450 py-1   px-4 h-8 items-center justify-center gap-2 w-full text-sm">
                            <p class="text-sm font-bold text-white">تماس با پلیر</p>
                            <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 25" fill="none">
                    ...
                    <div class="flex justify-between">
                        <div class="flex gap-2">
                            <p class="text-md font-bold text-text-3">کد رزرو</p>
                            <p class="text-md font-bold">${orderData.order_id}</p>
                        </div>

                        <div class="flex gap-2">
                            <p class="text-md font-bold text-text-3">تعداد</p>
                            <p class="text-md font-bold">${orderData.quantity} بلیت</p>
                        </div>
                    </div>

                    <div class="flex gap-2">
                        <p class="text-md font-bold text-text-3">تاریخ رزرو</p>
...
jQuery(document).ready(function($) {
    ...
    // تابع کمکی برای نمایش Tooltip با Tippy
    function showTooltip(element, message, theme = 'light') {
        if (!element) return;
        
        // اگر قبلاً تیپی روی این المنت بوده، نابودش کن
        if (element._tippy) {
        ...
        tippy(element, {
            content: message,
            trigger: 'manual', // فقط دستی فعال شود
            theme: theme,      // تم دلخواه (می‌توانید در css تم‌های error یا success بسازید)
            placement: 'top',
            arrow: true,
            zIndex: 99999,
            onShow(instance) {
    ...
    window.toggleAllSansesForDay = function(btn, actionType) {
        ...
        // گرفتن مقادیر
        let roomId = $('.swiper-slide-active').data('id');
        let selectedDate = $('[data-datepicker].active').data('datepicker');

        // بررسی اعتبار اولیه
        if (!roomId) {
            showTooltip(btn, "شناسه اتاق یافت نشد.", 'material');
            return;
        }
        if (!selectedDate) {
            showTooltip(btn, "تاریخ مشخص نیست.", 'material');
            return;
        }

        let actionName = (actionType === 'open_all_sanses') ? 'باز کردن' : 'بستن';
        
        // تاییدیه گرفتن از کاربر (از confirm مرورگر استفاده می‌کنیم چون blocking است)
        if (!confirm(`آیا از ${actionName} تمام سانس‌های این روز اطمینان دارید؟`)) {
            return;
        }

        // --- شروع حالت لودینگ ---
        let originalText = $btn.text();      // ذخیره متن اصلی
        let btnWidth = $btn.outerWidth();    // ذخیره عرض دکمه
        
        $btn.css('min-width', btnWidth + 'px'); // قفل کردن عرض دکمه
