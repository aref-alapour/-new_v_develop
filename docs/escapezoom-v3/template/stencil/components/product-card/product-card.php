    ...
    function ez_render_product_card(array $product, string $home_url)
    {
        ...
        <ez-product-card product-id="<?= $product['product_id']; ?>" status="<?= $status; ?>" href="<?= $href; ?>">
            <?php if (in_array($status, ['temp', 'deactivated'], true)): ?>
                <a slot="media" href="<?= $href; ?>" class="relative after:bg-white/60 after:absolute after:top-0 after:left-0 after:bottom-0 after:right-0">
                    <img alt="<?= $product_cat_alt . ($product['title'] ?? ''); ?>"
                        ...
                        class="h-d192 lg:h-d236 object-cover" src="<?= esc_url($product['image'] ?? ''); ?>"
                        style="color: transparent;">
                </a>
                <span slot="badge" class="bg-steel rounded-9 text-white w-d118 h-d34 absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 font-bold text-center text-shadow">غیرفعال</span>
            <?php elseif ($status === 'expired'): ?>
                ...
                <span slot="badge" class="bg-steel rounded-9 text-white w-d118 h-d34 absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 font-bold text-center text-shadow">اکسپایر شده</span>
            <?php elseif ($status === 'soon'): ?>
                <a slot="media" href="<?= $href; ?>">
                    <img alt="<?= $product_cat_alt . ($product['title'] ?? ''); ?>"
                        ...
                        class="h-d192 lg:h-d236 object-cover" src="<?= esc_url($product['image'] ?? ''); ?>"
                        style="color: transparent;">
                    <span class="bg-focus-blue text-white leading-none px-2.5 py-1 rounded-40 h-5 text-sm absolute top-4 right-4 font-bold">به زودی</span>
                </a>
            <?php else: ?>
                <a slot="media" href="<?= $href; ?>">
                    <img alt="<?= $product_cat_alt . ($product['title'] ?? ''); ?>"
                        ...
                        style="color: transparent;">
                    <?php if ($is_updated): ?>
                        <span class="bg-danger-hot text-white leading-none px-2.5 py-1 rounded-40 h-5 text-sm absolute top-4 right-4 font-bold">آپدیت شد</span>
                    <?php endif; ?>
                </a>
                <button type="button" slot="floating-action"
                    class="absolute bottom-2 right-2 flex h-7.5 w-7.5 items-center justify-center rounded-full bg-yellow-500/30 lg:hidden mobile-hover">
                ...
                <a slot="overlay-panel" href="<?= $href; ?>"
                    ...
                    <?php if ($product_type === 'escaperoom'): ?>
                        <div class="mx-auto flex w-[90%] items-center justify-center gap-x-1 rounded bg-white/20 px-6 py-1.5 leading-none">
                            <div class="max-lg:hidden lg:flex lg:item-center lg:justify-between w-full">
                                <span class="text-white/70 text-2xs leading-none flex items-center">امروز</span>
                    ...
                    <span class="flex items-center justify-center mx-auto rounded-xl bg-blue/40 px-2 py-0.5 relative w-[90%] h-d30 lg:hidden">
                        <button type="button"
                            class="absolute right-d6 top-d6">
            ...
            <div slot="meta" class="flex items-center justify-between my-3">
                <span class="text-base font-medium text-steel leading-none">
                    <?= $product_cat_alt; ?>
                </span>
                <?php if ($is_active): ?>
                    <span class="text-sm rounded-4 flex items-center justify-center leading-none pt-px bg-yellow-400 text-slate-900 w-d31 h-d18p5" name="rate">
            ...
            <div slot="title" class="flex items-center justify-between">
                <h3 class="w-full truncate text-base font-bold lg:text-22" title="<?= $product['title'] ?? ''; ?>" name="title">
                    <a href="<?= $href; ?>">
                        <?= $product['title'] ?? ''; ?>
                    </a>
                </h3>
            ...
            <p slot="address" class="text-4xs text-text-charcoal lg:text-2xs lg:word-spacing-nav mt-3 flex items-center gap-x-1">
            ...
            <?php if ($is_active): ?>
                <div slot="pricing" class="flex items-center justify-center gap-x-2 bg-slate-110 px-2 rounded-6 mt-3">
                    <?php if ($has_event): ?>
                        <span class="bg-danger-hot text-white rounded-40 w-8 h-4 flex items-center justify-center">
                    ...
                    <?php endif; ?>
                    <div>
                        <span class="text-steel ml-1">از</span>
                        <span>
                            <span class="ml-px text-md font-bold" name="price">
                                <?= number_format($product['price'] ?? 0); ?>
                            </span>
                            <span class="text-steel">تومان</span>
                        </span>
                    </div>
                </div>
            <?php endif; ?>
