      ...
      <Host class={`block ${is_booked ? 'opacity-60 pointer-events-none grayscale' : ''}`}>
        ...
        `}>
          {discount_price && !is_booked && (
            <div class="absolute -top-3 right-2 bg-red-500 text-white text-10 px-2 py-0.5 rounded-full shadow-sm font-yekan-bold">
              تخفیف ویژه
            </div>
          )}
            ...
            {discount_price && !is_booked ? (
              <div class="flex flex-col items-center">
                ...
                <span class="text-sm font-yekan-bold text-green-600 flex items-center gap-1">
                  ...
                  <span class="text-10 font-yekan-regular text-gray-500">تومان</span>
                </span>
              </div>
            ) : (
              <span class="text-sm font-yekan-bold text-gray-600 flex items-center gap-1">
                {is_booked ? 'رزرو شده' : (
                  <span>{this.formatPrice(price)} <span class="text-10 font-yekan-regular text-gray-400">تومان</span></span>
                )}
              </span>
            )}
          </div>
          {!is_booked && (
            <div class="mt-2 w-full pt-2 border-t border-dashed border-gray-100 flex justify-between items-center text-10 text-gray-400">
