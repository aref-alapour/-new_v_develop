...
// ۱. دریافت بازی‌هایی که کاربر سرگروه بوده (از wp_markting)
$leader_query = $wpdb->prepare("
    SELECT game_id 
    FROM wp_markting 
    WHERE customer_id = %d 
    AND order_status IN ('wc-completed', 'wc-partially-paid','wc-walletx','wc-completed-paid') 
    ORDER BY order_created_at DESC
", intval($profile_user_id));
$leader_results = $wpdb->get_col($leader_query);
$purchased_products = !empty($leader_results) ? array_map('intval', $leader_results) : [];

// ۲. دریافت بازی‌هایی که کاربر هم‌تیمی بوده (از usermeta)
$teammate_meta = get_user_meta($profile_user_id, 'teammate_products', true);
$teammate_products = (!empty($teammate_meta) && is_array($teammate_meta)) ? array_map('intval', $teammate_meta) : [];

// محاسبه تعداد کل بازی‌های انجام شده (سرگروه + هم‌تیمی)
$total_played_count = count($purchased_products) + count($teammate_products);

// بازی‌های دستی (قبل از سیستم فعلی) — از user meta ادمین
$previous_played_games = (int) get_user_meta($profile_user_id, 'ez_previous_played_games', true);
$played_count_total    = $total_played_count + max(0, $previous_played_games);

// ترکیب بازی‌ها، حذف تکراری‌ها و استخراج ۲۰ بازی آخر برای کروسل
$all_played_products = array_merge($purchased_products, $teammate_products);
$unique_played_products = array_unique($all_played_products);
$latest_20_products = array_slice($unique_played_products, 0, 20);

// دریافت HTML کروسل از snapshot (نه وب‌سرویس)
if (!empty($latest_20_products)) {
    $recent_played_response = ez_products_snapshot_swiper_by_ids($latest_20_products);
}
...
$collections = $wpdb->get_results(
	$wpdb->prepare(
		'SELECT * FROM collections WHERE user_id = %d AND active = %d AND items NOT LIKE %s',
		(int) $profile_user_id,
		1,
		'a:0:{}'
	)
...
foreach ($collections as $collection) {
    $collection_products = ez_products_snapshot_swiper_by_ids(
        is_array($collection->items) ? $collection->items : (array) unserialize($collection->items)
    );
...
get_header(); ?>
    ...
    <div class="max-lg:mb-4.5 max-lg:flex max-lg:items-center max-lg:justify-between lg:mt-5.5 lg:text-left max-lg:hidden">
        ...
        <button type="button" class="flex h-8.5 w-8.5 items-center justify-center rounded-md border border-edge shadow-13 lg:hidden">
            <svg class="mr-px" xmlns="http://www.w3.org/2000/svg" width="9" height="14" viewBox="0 0 9 14" fill="none">
    ...
    <section class="max-lg:relative max-lg:-mx-4 max-lg:h-62 max-lg:rounded-b-3xl max-lg:border-b max-lg:border-b-blue max-lg:bg-slate-100 mt-16 lg:mt-d0">
            ...
            <div class="lg:flex lg:items-center lg:gap-x-9">
                <div class="max-lg:absolute max-lg:left-1/2 max-lg:-translate-x-1/2 max-lg:-translate-y-1/2">
                    <?php echo get_avatar($data['ID'], 60, false, $data['name'], [
                        'class' => 'h-15.5 w-15.5 rounded-10 max-lg:h-20 max-lg:w-20 max-lg:border max-lg:border-2 max-lg:border-blue max-lg:drop-shad',
                    ]); ?>
                </div>
                <div>
                    ...
                    <div class="text-md text-text-3 max-lg:absolute max-lg:left-1/2 max-lg:top-30 max-lg:-translate-x-1/2 max-lg:text-center lg:hidden"><?php echo get_city_label_by_identifier($data['city']); ?> </div>
                    <div class="flex items-center">
                    <span class="hidden font-medium text-2xs max-lg:hidden">
                        <span class="ml-1.5 text-base"> <?= esc_html($data['played_count']); ?> </span> بازی های انجام داده </span>
                        <!--                        <span class="mx-3.5 h-2.5 border-l border-l-slate-110"></span>-->
                        <span class="text-2xs font-medium max-lg:absolute max-lg:right-5 max-lg:top-5 max-lg:rounded max-lg:bg-yellow-500 max-lg:px-1.5">
                ...
                <div class="z-50 hidden invite-modal">
                    ...
                    <div class="fixed bg-white w-d430 max-w-[100%] h-auto right-[50%] translate-x-[50%] top-[50%] -translate-y-[50%] p-6 rounded-xl border z-50 flex flex-col gap-4">
                        <div class="relative">
                            ...
                            <div id="search-result" class="items-center gap-x-4 rounded-10 border border-edge bg-white px-6 py-3 shadow-13 absolute top-[calc(100%+10px)] right-0 w-full hidden"></div>
                        </div>
            ...
            <div class="flex items-center max-lg:w-full max-lg:justify-center gap-x-4 max-lg:absolute max-lg:bottom-8 max-lg:left-1/2 max-lg:-translate-x-1/2">
                <?php if ((int) $profile_user_id !== get_current_user_id() && is_user_logged_in()) { ?>
                    <button id="invite-modal-button" type="button" aria-haspopup="dialog" aria-expanded="false" aria-controls="radix-:r9:" data-state="closed">
                        <div class="flex h-14 items-center gap-x-4 rounded-lg border border-edge bg-white px-4.5 shadow-13">
    ...
    <div class="grid-cols-12 pb-4 max-md:mt-6 lg:mt-10 lg:grid lg:gap-10">
        <div class="flex flex-col col-span-9 gap-10 max-3xl:col-span-8 max-2xl:col-span-7">
            <?php if (!empty($latest_20_products)): ?>
                <div>
                        ...
                        <section class="relative max-w-full py-4 md:py-5 lg:py-9 max-md:px-4">
                            <div class="relative overflow-hidden embla_normal">
                                <div class="embla__viewport">
                                    <div id="trends-rooms-slider" class="embla__container first-child:before:hidden child:before:w-px child:before:absolute child:before:bg-gradient-to-t child:before:from-white child:before:via-slate-110 child:before:to-white child:before:-right-3.5 child:lg:before:-right-6 child:before:h-full min-h-d200 child:box-content lg:min-h-d300 flex child:ml-7 md:child:ml-12  last-child:ml-0 child:relative child:shrink-0 child:grow-0 child:w-d156 md:child:w-d200 child:py-2.5"> <?= $data['recent_played']; ?> </div>
                                </div>
            ...
            <?php else: ?>
                <div class="profile-section empty-state">
                    <p>هنوز تجربه‌ای ثبت نشده است.</p>
                </div>
            <?php endif; ?>
            <?php foreach ($data['collections'] as $collection) { ?>
                <div>
                    ...
                    <div class="relative w-full max-sm:max-w-bleed-2 max-sm:w-bleed-2 max-sm:-mr-4" dir="rtl">
                        <div class="relative overflow-hidden embla_normal">
                            <div class="embla__viewport">
                                <div id="trends-rooms-slider" class="embla__container first-child:before:hidden child:before:w-px child:before:absolute child:before:bg-gradient-to-t child:before:from-white child:before:via-slate-110 child:before:to-white child:before:-right-3.5 child:lg:before:-right-6 child:before:h-full min-h-d200 child:box-content lg:min-h-d300 flex child:ml-7 md:child:ml-12 child:relative last-child:ml-0 child:shrink-0 child:grow-0 child:w-d156 md:child:w-d200 child:py-2.5"> <?php echo $collection['items']; ?> </div>
                            </div>
                            <button class="embla__button embla__button--prev trends-rooms-btn absolute right-0 top-1/2 -translate-y-115 rotate-180 z-50 cursor-pointer touch-manipulation appearance-none -mr-px hidden" type="button">
                                <svg xmlns="http://www.w3.org/2000/svg" width="30" fill="none" viewBox="0 0 30 113">
                                    <g clip-path="url(#arrow_aa)">
        ...
        <div id="ez-bio" class="col-span-3 max-3xl:col-span-4 max-2xl:col-span-5">
            ...
            <div class="flex flex-col">
                <div class="flex flex-col w-full">
                    ...
                    <div class="flex items-center justify-between mt-7.5 gap-x-4 rounded-xl border border-edge bg-white p-4.5 shadow-13">
                            ...
                            <div class="flex embla__container">
                                <?php foreach ($owner_feedback as $feedback): ?>
                                    ...
                                    <div class="w-full embla__slide shrink-0 grow-0 gap-x-4">
                                        <a href="<?= $product_url ?>" class="text-lg font-bold text-slate-700 flex shrink-0 after:content-[''] after:w-full after:bg-slate-130 after:h-2 after:rounded gap-x-6 items-center after:-mt-2"><?= $product_title ?></a>
                                        <p class="text-sm text-text-3 font-bold mt-8"><?= $feedback['owner_comment'] ?></p>
