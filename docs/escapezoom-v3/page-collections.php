...
if ( $sort == -1 ) {
	$collections = $wpdb->get_results(
		$wpdb->prepare(
			'SELECT * FROM collections WHERE active = %d AND CHAR_LENGTH(items) > 1 AND items NOT LIKE %s ORDER BY RAND() DESC LIMIT %d, %d',
			1,
			'a:0:{}',
			(int) $offset,
			(int) $items_per_page
	...
	$sort_by = ( $sort === 'recent' ) ? 'ID' : 'users';
	$collections = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT * FROM collections WHERE active = %d AND CHAR_LENGTH(items) > 1 AND items NOT LIKE %s ORDER BY {$sort_by} DESC LIMIT %d, %d",
			1,
			'a:0:{}',
			(int) $offset,
			(int) $items_per_page
                        ...
                        <a class="flex w-full h-full flex-col justify-between gap-2.5 overflow-hidden rounded-lg border border-slate-120 px-3 py-4 shadow-22 lg:gap-5 lg:rounded-3xl lg:px-5 lg:py-6 lg:border-none lg:bg-slate-700 lg:text-white lg:shadow-6 lg:[&>div]:border-none" href="<?php echo site_url( '/profile/' . $item['user'] ); ?>">
                            ...
                            <div class="grid min-w-28 grid-cols-3 gap-1 border-b border-t border-slate-100 px-2 lg:gap-3 grid-rows-2 max-lg:grid-rows-1">
                                ...
                                <?php foreach ( array_slice( $item['images'] , 0, 6) as $image) : ?>
                                    <div class="w-9 lg:w-d52 lg:[&:last-of-type>div>div]:flex max-lg:[&:nth-child(n+4)]:hidden max-lg:[&:nth-of-type(3)>div>div]:flex">
                                        <div class="relative overflow-hidden rounded-md shadow-2">

                                            <?php echo wp_get_attachment_image($image, 'thumbnail', true, [
                                                'class' => 'h-d66 w-d52 object-cover'
                                            ]);?>
                            ...
                            <div class="flex flex-col max-lg:flex-col-reverse gap-4 max-lg:gap-0">
                                <div class="inline-flex w-full items-center justify-between">
                                    ...
                                    <?php user_badge_by_level($item['user'] , 'text-xs text-primary-2 bg-white max-lg:bg-primary-2/20 max-lg:text-white p-2 max-lg:py-1 rounded-full text-nowrap'); ?>
                                </div>
