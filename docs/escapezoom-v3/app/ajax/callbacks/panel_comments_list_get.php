...
if ( ! empty( $data ) ) :
    ...
    foreach ( $data as $comment ) : ?>
        ...
        <div class="hidden lg:flex flex-col rounded-14 bg-gray-20 mb-d30" style="border: 1px solid #dbe2ea">
            <div class="flex flex-row items-center justify-between px-d30 py-4">
                <div class="flex justify-center items-center">
                    <div class="flex flex-row items-center gap-d8">
                        <span><?php
                        $bid = (int) $comment['id'];
                        if ( $bid > 0 ) {
                            $ud = get_userdata( $bid );
                    ...
                    <div class="w-d4 h-d4 bg-slate-130 rounded-full mx-4"></div>
                    <p><?php echo jdate( "Y.m.d", strtotime( $comment['comment']->comment_date ) ) ?></p>
                </div>

                <div class="flex gap-d73">
                    ...
                    <div class="flex justify-between items-center hidden">
                        <p class="text-text-3">
                            سانس
                            <span class="mr-2 text-slate-700">00:0 1401/01/01</span>
                        </p>
                    </div>
                </div>
            ...
            <div class="rounded-14 bg-white px-d30 pt-4 pb-d30" style="border: 1px solid #dbe2ea">
                <p class="text-14 font-bold mb-3"><?php echo $comment['comment']->comment_content ?></p>
                ...
                <div class="flex justify-between items-center bg-gray-20 py-2 px-4 rounded-12">

                    <div class="flex gap-2 items-center">
                        <p class="text-14 font-bold" style="color: <?php echo esc_attr( $rate_color ) ?>">
                    ...
                    <div class="flex justify-between gap-d44 ">

                        <?php foreach ( $comment['rating'] as $label => $value ) { ?>
                            <div class="flex flex-col items-center">
                                <div class="flex flex-col justify-center">
                                    <div class="flex items-center" dir="ltr">
                                        <span class="min-w-4 text-14"><?php echo esc_html( $value ) ?></span>
                                        <div class="w-d61 rounded-full bg-slate-110 dark:bg-gray-700 h-1">
                                            <div class="rounded-full bg-accent-420 h-1" style="width: <?php echo $value * 20 ?>%"></div>
                                        </div>
                                    </div>
                                </div>
                                <span class="nowrap text-14 font-bold lg:text-sm text-slate-250">
                ...
                <?php if ( count( $comment['replies'] ) > 0 ) { ?>
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex items-start gap-4">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="14" viewBox="0 0 16 14" fill="none" class="mt-2">
                ...
                <?php } else { ?>
                    <div class="flex justify-between">
                        <div class="flex">
                            <p class="reply-button text-14 font-bold text-blue cursor-pointer flex items-center gap-2" data-target-form="<?php echo esc_attr( $comment['comment']->comment_ID ) ?>">
                                پاسخ به این دیدگاه
                            ...
                            <p class="close-reply-button hidden text-14 font-bold  text-blue cursor-pointer flex items-center gap-2" data-target-form="<?php echo esc_attr( $comment['comment']->comment_ID ) ?>">
                                بستن
                    ...
                    <div class="hidden mt-4" data-form="<?php echo esc_attr( $comment['comment']->comment_ID ) ?>">
                        <form method="post" class="submit-reply-form">
                            ...
                            <textarea name="comment_content" class="w-full min-h-d100 rounded-14 border border-slate-105 p-2 resize-y" placeholder="پاسخ خود را اینجا بنویسید..."></textarea>
                            <button type="submit" class="btn rounded-8 px-d33 py-2 text-white mt-2" style="box-shadow: 0 2px 0 0 #CA5608; background: #FD7013;">
                                ارسال
                            </button>
                        </form>
                    </div>
        ...
        <div class="lg:hidden flex flex-col border-b pb-7 mb-7">
            ...
            <div class="flex flex-col bg-slate-50 rounded-xl mb-4">
                ...
                <div class="flex flex-col justify-between gap-1 p-4 pt-0" style="display: none">
                    <?php foreach ( $comment['rating'] as $label => $value ) { ?>
                        <div class="flex flex-row-reverse items-center justify-between">
                            ...
                            <span class="nowrap text-14 font-bold lg:text-sm w-d132 text-slate-250">
								<?php echo esc_html( $label ); ?>
                            </span>
                        </div>
                    <?php } ?>
                ...
                <button type="button" class="more-details bg-black/5 border-t justify-center flex items-center gap-x-3">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="10" height="6" viewBox="0 0 10 6" fill="none" class="transition duration-300">
            ...
            <?php } else { ?>
                ...
                <div class="hidden mt-4" data-form="<?php echo esc_attr( $comment['comment']->comment_ID ) ?>">
                    <form method="post" class="submit-reply-form flex items-end gap-4">
                        <input type="hidden" name="product" value="<?php echo esc_attr( $comment['product']->ID ); ?>">
                        <input type="hidden" name="comment" value="<?php echo esc_attr( $comment['comment']->comment_ID ); ?>">
                        <textarea name="comment_content" class="w-full min-h-full rounded-14 border border-slate-105 p-2 resize-y" rows="1" placeholder="پاسخ خود را اینجا بنویسید..."></textarea>
