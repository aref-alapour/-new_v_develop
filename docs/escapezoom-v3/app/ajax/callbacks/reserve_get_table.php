...
<div class="flex justify-around mb-12 text-nowrap max-lg:gap-5.5 overflow-x-auto no-scrollbar">
    <?php for ($i = 0; $i < 7; $i++) { ?>
        <button type="button" data-tab="<?php echo $i; ?>" class="bg-breserve font-extrabold border border-edge flex flex-col items-center justify-center rounded-xl p-4 leading-4 shadow-13 w-d80 h-d120 gap-1" <?php echo $i == 0 && wp_is_mobile() ? ' style="background: rgb(80, 145, 251); border-color: transparent; color: rgb(255, 255, 255);"' : '' ?>>
            <?php
            $timezone = new DateTimeZone('Asia/Tehran');
            $today = new DateTime('now', $timezone);
            $today->setTime(0, 0, 0);
            if ($time == $today->getTimestamp() && $i == 0) { ?>
                <span class="text-16">امروز</span>
                <span class="text-26">
                ...
                switch ($item->status) {
                    case "reservable": ?>
                        ...
                        <?php if ($item->off_price > 0) { ?>
                            <div data-item-timestamp="<?php echo $item->time; ?>" data-item-sell-price="<?php echo $item->off_price ?>" class="box open cursor-pointer off max-lg:h-12 h-d112 rounded-lg w-full shadow-102 overflow-hidden relative">
                                <div class="back text-white bg-blue absolute w-full h-full flex lg:flex-col text-center justify-between">
                                    ...
                                    <span class="text-md shrink-0 max-lg:flex items-center justify-center max-lg:px-2 drop-shadow-104 py-0.5 bg-black/15 w-30 lg:w-full text-center max-lg:text-22 max-lg:gap-1">
                                        ...
                                        <span class="max-lg:text-12">تومان</span>
                                    </span>
                                </div>
                        ...
                        <?php } else { ?>
                            <div data-item-timestamp="<?php echo $item->time; ?>" data-item-sell-price="<?php echo $item->price ?>" class="box open cursor-pointer max-lg:h-12.5 h-d112 rounded-lg w-full shadow-102 overflow-hidden relative">
                                <div class="back text-white bg-blue absolute w-full h-full flex lg:flex-col text-center justify-between ">
                    ...
                    <?php break;
                    case "reserving": ?>
                        <div class="box reserving cursor-wait max-lg:h-12.5 h-d112 rounded-lg w-full shadow-102 overflow-hidden relative">
                            <div class="back text-white bg-yellow-500 absolute w-full h-full flex lg:flex-col text-center justify-between">
                                <span class="text-2xl drop-shadow-104 flex grow w-full lg:justify-center items-center max-lg:px-2">
                                    <?php echo date('H:i', $item->time) ?>
                                </span>
                    ...
                    <?php break;
                    case "reserved": ?>
                        <div class="box reserved cursor-not-allowed max-lg:h-12.5 h-d112 rounded-lg w-full shadow-102 overflow-hidden relative">
                            <div class="back text-white bg-secondary-500 absolute w-full h-full flex lg:flex-col text-center justify-between">
                                <span class="text-2xl drop-shadow-104 flex grow w-full lg:justify-center items-center max-lg:px-2">
                                    <?php echo date('H:i', $item->time) ?>
                                </span>
                                <span class="text-md shrink-0 max-lg:flex items-center justify-center max-lg:px-2 drop-shadow-104 py-2 bg-black/15 w-30 lg:w-full max-lg:text-22">
                    ...
                    <?php break;
                    case "non_reservable": ?>
                        <div class="box closed cursor-not-allowed max-lg:h-12.5 h-d112 rounded-lg w-full shadow-102 overflow-hidden relative">
                            <div class="back text-textColor bg-gray-50 absolute w-full h-full flex lg:flex-col text-center justify-between">
