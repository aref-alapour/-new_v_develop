...
$comment_id      = (int) sanitize_text_field( $_POST['comment'] ) ?: - 1;
$product_id      = (int) sanitize_text_field( $_POST['product'] ) ?: - 1;
...
$role = match ( get_user_role( $user->ID ) ) {
    'sans_manager' => 'sans_manager',
    'compiler' => 'user_ebtal'
};

$products = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}postmeta WHERE `meta_key` LIKE %s AND `meta_value` LIKE %d", $role, $user->ID ), ARRAY_A );

$active_products = [];
foreach ( $products as $product ) {
    if ( get_post_type( $product['post_id'] ) == 'product' ) {
        $active_products[] = (int) $product['post_id'];
    }
}

if ( empty( $products ) || empty( $active_products ) ) {
    wp_send_json_error( 'شما هیچ اتاق فعالی برای نمایش ندارید.' );
}

if ( ! in_array( $product_id, $active_products ) ) {
    wp_send_json_error( 'این کامنت متعلق به شما نیست!' );
}

$comment = wp_insert_comment( [
    'comment_post_ID'  => $product_id,
    'comment_author'   => get_user_by( 'id', $user->ID )->user_login,
...
$reply = '<div class="flex flex-col gap-4 max-lg:hidden">
	<div class="text-black font-bold text-lg">پاسخ مجموعه</div>
	<div class="flex items-start gap-4">
		...
		<span class="text-gray-500">' . $comment_content . '</span>
	</div>
</div>

<div class="mb-2 mb-5 p-4 rounded lg:hidden" style="background: #F3f9fc">
