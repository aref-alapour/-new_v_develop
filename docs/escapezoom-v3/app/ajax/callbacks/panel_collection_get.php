...
$collections = $wpdb->get_results(
    $wpdb->prepare(
        "SELECT * FROM collections WHERE user_id = %d AND ID = %d LIMIT 1",
        (int) $user->ID,
        $collection_ID
    )
);
...
$collection_type_key = isset( $collection->type ) ? (string) $collection->type : '';
if ( function_exists( 'ez_collection_normalize_type_key' ) ) {
    $collection_type_key = ez_collection_normalize_type_key( $collection_type_key );
}
...
<div class="grid grid-cols-2 lg:grid-cols-12 mb-5 max-lg:mt-5">
    ...
    <div class="col-span-2 lg:col-span-4 max-lg:order-3">
        <div class="flex items-center gap-x-4 mb-4 lg:mb-0 rounded-10 border border-edge px-6 py-3 shadow-13 relative">
            <span>
                <svg class="shrink-0" xmlns="http://www.w3.org/2000/svg" width="18" height="19" viewBox="0 0 18 19" fill="none">
...
<?php if ($collection_city_id <= 0) { ?>
    <div class="relative mb-6">
        <div class="absolute inset-0 bg-white/70 backdrop-blur-[2px] z-30 rounded-2xl border border-slate-120"></div>
        <div class="relative z-40 bg-white rounded-2xl border border-slate-120 p-4 flex flex-col gap-3">
            <div class="text-sm text-slate-600">برای فعال شدن این کالکشن، شهر مورد نظر را انتخاب کنید.</div>
            <div class="relative ez-city-combobox" data-cities='<?php echo wp_json_encode($cities_flat, JSON_UNESCAPED_UNICODE); ?>' data-collection-type="<?php echo esc_attr($collection_type_key); ?>">
                <input id="collection-city-input" class="ez-city-input text-gray-900 block w-full border-0 p-1.5 text-sm shadow-13 outline-none ring-1 ring-inset ring-gray-100 placeholder:text-right placeholder:text-slate-200 focus:shadow-none focus:ring-2 focus:ring-inset focus:ring-primary-500 h-12 py-2 px-6 rounded-2xl" placeholder="شهر را جستجو کنید..." type="text" autocomplete="off">
                <input id="collection-city-id" class="ez-city-id" type="hidden" value="0">
                <input id="collection-term-id" class="ez-collection-term-id" type="hidden" value="0">
            ...
            <div id="collection-city-term-error" class="text-xs text-red-500 hidden">
                برای این شهر، دسته‌بندی نوع کالکشن تعریف نشده است.
            </div>
            <div class="text-xs text-red-500">
                <span class="font-bold">*</span>
                با انتخاب شهر، بازی هایی که متعلق به شهر انتخابی شما نباشند، حذف میشوند.
            </div>
            <button type="button" id="collection-city-submit" class="bg-primaryColor p-3 text-white rounded-2xl shadow-primary-3 shadow-13 w-full">
                ثبت شهر کالکشن
            </button>
        </div>
    </div>
            ...
            <div class="col-span-1 lg:border lg:rounded-xl border-b max-lg:flex max-lg:justify-between">
                <div class="p-4 flex items-center gap-4 max-lg:py-">
                    <?php echo get_the_post_thumbnail( $item, 'large', [
                        'class' => 'max-lg:hidden w-d62 rounded-xl',
                        'style' => 'height:70px',
                    ] ) ?>

                    <?php echo get_the_post_thumbnail( $item, 'large', [
                        'class' => 'lg:hidden w-d30 rounded-xl',
                    ] ) ?>
                    <div>
...
?>
    <div class="h-d300 text-lg flex justify-center items-center text-slate-350">
...
<script>
    jQuery(document).ready(function ($) {
        ...
        let collectionSearchXhr = null;
        $("#search-field").on('input', function () {
            const value = (($(this).val() || '') + '').trim();

            if (collectionSearchDebounceTimer) {
                clearTimeout(collectionSearchDebounceTimer);
                collectionSearchDebounceTimer = null;
            }
            if (collectionSearchXhr && collectionSearchXhr.readyState !== 4) {
                collectionSearchXhr.abort();
                collectionSearchXhr = null;
            }

            if (value.length >= 2) {
                if (<?php echo (int)$collection_city_id; ?> <= 0) {
                    Toast.fire({ icon: 'error', title: 'ابتدا شهر کالکشن را مشخص کنید' })
                    $("#search-result").addClass('hidden').removeClass('flex')
                    return;
                }
                collectionSearchDebounceTimer = setTimeout(function () {
                    $("#search-result").addClass('flex').removeClass('hidden');
                    collectionSearchXhr = $.ajax({
                        type: 'POST',
                        url: "<?php echo admin_url( 'admin-ajax.php' ) ?>",
                        data: {
                            'action': 'v2_ajax_handler',
                            'nonce': "<?php echo wp_create_nonce( 'v2-ajax-nonce' ) ?>",
                            'callback': 'panel_collection_search',
                            'collection': '<?php echo (int)$collection_ID; ?>',
                            'term': value
                        },
                        beforeSend: function () {
...
<img src="${item.image}" alt="${item.title}" style="width: 20px; height: 24px; border-radius: 4px;">
                                ...
                                $("#search-result").text(typeof res.data === 'string' ? res.data : 'خطا در جست و جو')
                            } else {
                        ...
                        error: function (_, status) {
                            ...
                            $("#search-result").text('خطا در جست و جو')
                        },
                        complete: function () {
                            collectionSearchXhr = null;
                        }
                ...
                $("#search-result").addClass('hidden').removeClass('flex').empty()
            }
        })
        ...
        const ezNormalizeTypeKey = (typeKey) => {
        ...
        const ezResolveTermIdForCity = (city, typeKey) => {
            ...
            const normalizedType = ezNormalizeTypeKey(typeKey);
            const label = EZ_TYPE_TO_LABEL[normalizedType];
            if (!label) return 0;
        ...
        // Legacy collection: city combobox + set city + cleanup mismatched items
        const initCityCombobox = ($root) => {
            if (!$root || !$root.length) return;
            const cities = JSON.parse($root.attr('data-cities') || '[]');
            const typeKey = ($root.attr('data-collection-type') || '').toString();
            const $input = $root.find('.ez-city-input');
            const $hidden = $root.find('.ez-city-id');
            const $term = $root.find('.ez-collection-term-id');
            const $list = $root.find('.ez-city-list');
            const $submit = $('#collection-city-submit');
            const $termError = $('#collection-city-term-error');
            ...
            const syncSelectionState = (id, fallbackName = '') => {
                const cityId = parseInt(id, 10) || 0;
                const city = cities.find(c => parseInt(c.id, 10) === cityId) || null;
                if (!city) {
                    $hidden.val(0);
                    $term.val(0).attr('data-error', 'missing-term');
                    $termError.removeClass('hidden');
                ...
                $hidden.val(cityId);
                $input.val(fallbackName || city.name || '');

                const termId = ezResolveTermIdForCity(city, typeKey);
                $term.val(termId);
                if (termId <= 0) {
                    $term.attr('data-error', 'missing-term');
                    $termError.removeClass('hidden');
                    setSubmitDisabled(true);
                } else {
                    $term.removeAttr('data-error');
                    $termError.addClass('hidden');
            ...
            $input.on('blur', function () {
                const q = ($input.val() || '').trim();
                if (!q) {
                    syncSelectionState(0);
                    return;
                }
                const city = cities.find(c => (c.name || '').trim() === q);
                syncSelectionState(city ? city.id : 0, q);
            });

            // If city id is prefilled (e.g. partial rerender/autofill), ensure term_id is synced.
            syncSelectionState($hidden.val(), ($input.val() || '').trim());

            $(document).off('click.ezCollectionCity').on('click.ezCollectionCity', function (e) {
                if (!$root.is(e.target) && $root.has(e.target).length === 0) close();
            });
        };

        initCityCombobox($('#collection-city-input').closest('.ez-city-combobox'));

        $("#collection-city-submit").on('click', function () {
            const cityId = parseInt($("#collection-city-id").val(), 10) || 0;
            const termId = parseInt($("#collection-term-id").val(), 10) || 0;
            if (!cityId) {
                Toast.fire({ icon: 'error', title: 'لطفا شهر را انتخاب کنید' })
                return;
            }
            if (termId <= 0) {
                Toast.fire({ icon: 'error', title: 'برای این شهر، نوع کالکشن تعریف نشده است.' })
                return;
            }

            const $btn = $("#collection-city-submit");
            ...
            $.ajax({
                ...
                data: {
                    'action': 'v2_ajax_handler',
                    'nonce': "<?php echo wp_create_nonce( 'v2-ajax-nonce' ) ?>",
                    'callback': 'panel_collection_set_city',
                    'collection': '<?php echo (int)$collection_ID; ?>',
                    'city_id': cityId,
                    'collection_term_id': termId
                },
                beforeSend: function () {
                    $btn.attr('disabled', 'disabled')
                        .addClass('opacity-50 cursor-not-allowed')
                        .html('<div class="spinner" style="border-color: #FFFFFF; border-width: 3px; width: 28px; margin-inline: auto"></div>');
                },
                success: function (res) {
                    if (res && res.success) {
                        const removed = res.data && typeof res.data === 'object' ? (res.data.removed_count || 0) : 0;
                        const msg = removed > 0
                            ? `شهر ثبت شد. ${removed} بازی نامرتبط حذف شد.`
                            : 'شهر ثبت شد.';
                        Toast.fire({ icon: 'success', title: msg })

                        // Refresh current collection view
                        $.ajax({
                            type: 'POST',
                            url: "<?php echo admin_url( 'admin-ajax.php' ) ?>",
                            data: {
                        ...
                        const err = res && res.data ? res.data : 'خطا در ثبت شهر';
                ...
                error: function () {
                    Toast.fire({ icon: 'error', title: 'خطا در ثبت شهر' })
                    $btn.removeAttr('disabled')
                        .removeClass('opacity-50 cursor-not-allowed')
                        .text(btnText);
                }
