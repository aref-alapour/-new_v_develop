            ...
            foreach ($sanses[$day_type] as $sans) :
                ...
                if (! ($t1 < $firstTimeTs && $t2 > $firstTimeTs)) :
                    if ($firstTimeTs >= $auto_disable) :
                        ...
                        if ( $order_obj && isset( $order_obj['status'] ) && (int) $order_obj['status'] === 2 ) : // بسته شده توسط مجموعه دار
            ?>
                        ...
                        <?php
                        elseif ( $order_obj && isset( $order_obj['status'] ) && (int) $order_obj['status'] === 1 ) : // رزرو شده توسط یوزر
                        ?>
...
if ($data->type == 'get_sanses') { // display for api
    ...
    foreach ($days_time_arr as $key => $time_res) :
        // روز نمایشی: از ساعت ۸ امروز تا ۸ فردا — سانس‌های بامداد فردا (مثل ۰۲:۰۰) ته امروز نمایش داده می‌شوند
        $time_res_next = $time_res + 86400;
        $day_type      = get_day_type2($time_res);

        // فقط کلیدهای واقعی schedule: normals و holidays (closed در schedule وجود ندارد)
        $schedule_key = ($day_type === 'closed' || ! isset($sanses[$day_type])) ? null : $day_type;

        // بامداد فردا (قبل از ۰۸:۰۰) با همان برنامهٔ روز نمایشی — عادی فقط سانس‌های عادی بامداد، تعطیل فقط سانس‌های تعطیل بامداد
        $reservation_data[$key] = [];
        $day_slots = []; // [ ['ts' => timestamp, 'sans' => ..., 'day_type' => ... ], ... ]

        // سانس‌های همان روز از ۰۸:۰۰ به بعد (عادی یا تعطیل بسته به نوع روز)
        ...
        // سانس‌های بامداد فردا (قبل از ۰۸:۰۰) از همان schedule روز نمایشی — نه از نوع روز بعد
        if ($schedule_key !== null && ! empty($sanses[$schedule_key])) {
            foreach ($sanses[$schedule_key] as $sans) {
                $t = $sans['time'];
                $h = (int) substr($t, 0, 2);
    ...
    <div class="container">
        <div class="row">
            ...
            foreach ($sanses[$day_type] as $sans) :
                ...
                if ( $order_obj && isset( $order_obj['status'] ) && (int) $order_obj['status'] === 1 ) : // بسته شده توسط مجموعه دار
            ?>
                ...
                <?php
                elseif ( $order_obj && isset( $order_obj['status'] ) && (int) $order_obj['status'] === 2): // رزرو شده توسط یوزر
                ?>
